...
import { loadSkillModels, validateDocsMetadata } from './lib/ops-skill-model.mjs';
...
function renderTable(headers, rows) {
  ...
  for (const row of rows) {
    lines.push(`| ${row.map(cell => toStringValue(cell).replace(/\|/g, '\\|')).join(' | ')} |`);
  }
  return lines.join('\n');
}
...
function buildReadme(model) {
  ...
  if (inputs.length > 0) {
    ...
    sections.push(
      renderTable(
        ...
        inputs.map(input => [input?.name, input?.type, input?.required, input?.description])
      )
    );
    sections.push('');
  }
  ...
  if (outputs.length > 0) {
    ...
    sections.push(
      renderTable(
        ...
        outputs.map(output => [output?.name, output?.type, output?.description])
      )
    );
    sections.push('');
  }
  ...
  if (variants.length > 0) {
    ...
    sections.push(
      renderTable(
        ['Variant', 'Prompt File', 'Description', 'Cost Factor', 'Latency (ms)'],
  ...
  sections.push('');
  sections.push('This skill is available through the core-skills plugin and can be:');
...
function main() {
  ...
  if (errors.length > 0) {
    printErrorsAndExit(errors.map(error => `missing required metadata: ${error}`));
    return;
  }
