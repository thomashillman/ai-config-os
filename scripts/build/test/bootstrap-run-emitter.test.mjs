...
const EMITTER_PATH = join(__dirname, '../../../adapters/claude/lib/bootstrap-run-emitter.mjs');
...
function make404FailureRun(overrides = {}) {
  return {
    ...
    phases: [
      ...
      { phase: 'worker_package_fetch', result: 'error', duration_ms: 250, error_code: 'WORKER_PACKAGE_NOT_PUBLISHED' },
    ],
    ...overrides,
  };
}
...
test('validateRun: accepts minimal valid run', () => {
  const result = validateRun({ run_id: 'r1', started_at: '2026-01-01T00:00:00Z', status: 'success', phases: [] });
  assert.equal(result.ok, true);
});

test('validateRun: rejects missing run_id', () => {
  const result = validateRun({ started_at: '2026-01-01T00:00:00Z', status: 'success', phases: [] });
...
test('validateRun: rejects invalid status', () => {
  const result = validateRun({ run_id: 'r1', started_at: '2026-01-01T00:00:00Z', status: 'running', phases: [] });
  assert.equal(result.ok, false);
  assert.match(result.error, /status/);
});

test('validateRun: rejects forbidden field "token"', () => {
  const result = validateRun({ run_id: 'r1', started_at: '2026-01-01T00:00:00Z', status: 'success', phases: [], token: 'secret' });
  assert.equal(result.ok, false);
  assert.match(result.error, /token.*not permitted/i);
});

test('validateRun: rejects forbidden field "authorization"', () => {
  const result = validateRun({ run_id: 'r1', started_at: '2026-01-01T00:00:00Z', status: 'success', phases: [], authorization: 'Bearer x' });
  assert.equal(result.ok, false);
});
...
test('all phases have phase, result, and duration_ms', () => {
  ...
  for (const run of runs) {
    for (const phase of run.phases) {
      assert.ok(typeof phase.phase === 'string' && phase.phase.length > 0, 'phase.phase must be non-empty string');
      assert.ok(['ok','error','skipped'].includes(phase.result), `phase.result must be ok|error|skipped, got ${phase.result}`);
      assert.ok(typeof phase.duration_ms === 'number' && phase.duration_ms >= 0, 'phase.duration_ms must be non-negative number');
    }
  }
});
