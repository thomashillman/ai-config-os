...
function makeTask({
  ...
  state = "active",
  progress = { completed_steps: 1, total_steps: 5 },
...
test("scenario 1: MomentumView shows limited strength and upgrade opportunity for pasted_diff task", () => {
  ...
  const contract = makeContract({
    ...
    upgradeExplanation: {
      ...
      unlocks:
        "Full repository access enables call site verification, dependency impact analysis, and related test inspection",
...
test("scenario 2: after route upgrade to local_repo, finding confidence upgrades to high", () => {
  ...
  assert.equal(
    upgraded[0].provenance.confidence_basis,
...
test("scenario 2: MomentumView shows full strength and no upgrade opportunity for local_repo task", () => {
  ...
  const task = makeTask({
    taskId: "task_journey_002",
    route: "local_repo",
    findings: [finding],
    nextAction: "Write fix",
  });
  const contract = makeContract({
    taskId: "task_journey_002",
    routeId: "local_repo",
...
test("scenario 3: confidence grows pasted_diff → github_pr → local_repo and never decreases", () => {
  ...
  const stageValues = [finding, ...afterPR, ...afterLocal].map(
    (f) => f.provenance.confidence,
  );
...
test("scenario 4: buildMomentumShelf ranks active tasks with upgrade opportunity highest", () => {
  ...
  assert.ok(
    withUpgradeEntry,
    "pasted_diff task with upgrade should appear in shelf",
  );
