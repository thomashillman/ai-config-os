...
test('Bootstrap Workflow Integration', async (t) => {
  await t.test('complete flow: build → package → extract → install', async () => {
  ...
  await t.test('session-start.sh selects correct path based on bootstrap result', () => {
  ...
  await t.test('package version mismatch detection (prevents partial updates)', () => {
  ...
  await t.test('timing: fast path <10s, slow path ~25-60s', () => {
    ...
    assert.ok(
      fastPathMs < 10000,
      'fast path should complete in <10s'
    );
    assert.ok(
      slowPathMs > 25000,
      'slow path typically takes 25-60s'
    );
    assert.ok(
      slowPathMs < 60000,
      'slow path should not exceed 60s'
    );
  });
});
