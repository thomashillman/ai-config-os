...
test('Materialise Bootstrap Extraction', async (t) => {
  ...
  await t.test('writes version marker for idempotence', () => {
    ...
    try {
      ...
      assert.strictEqual(cached, version, 'version marker contains correct version');
    } finally {
      rmSync(tmpCache, { recursive: true, force: true });
    }
  });

  await t.test('is idempotent: rerun with same version skips re-extraction', () => {
  ...
  await t.test('directory structure matches cache layout', () => {
    ...
    try {
      ...
      assert.ok(
        existsSync(join(baseDir, 'skills')),
