...
async function loadWorkerWithFixtures(registryFixture, pluginFixture) {
  ...
  function transpileTree(current) {
    for (const entry of readdirSync(current)) {
      ...
      if (stat.isDirectory()) { transpileTree(absolute); continue; }
      ...
      const rewritten = transpiled.outputText.replace(/(from\s+['"])(\.\.?\/[^'".]+)(['"])/g, '$1$2.js$3');
  ...
  return {
    ...
    platform_definitions: {
      'claude-code': {
        ...
        capabilities: {
          'fs.read': { status: 'supported', confidence: 'high', source: 'manual-test' },
          'fs.write': { status: 'supported', confidence: 'high', source: 'manual-test' },
          'shell.exec': { status: 'supported', confidence: 'high', source: 'manual-test' },
          'git.read': { status: 'supported', confidence: 'high', source: 'manual-test' },
          'network.http': { status: 'supported', confidence: 'high', source: 'manual-test' },
          'mcp.client': { status: 'supported', confidence: 'high', source: 'manual-test' },
        },
      ...
      'claude-web': {
        ...
        default_package: 'api',
        capabilities: {
          'fs.read': { status: 'unknown', confidence: 'low', source: 'inference' },
          'shell.exec': { status: 'unsupported', confidence: 'medium', source: 'inference' },
          'network.http': { status: 'supported', confidence: 'medium', source: 'vendor-doc' },
          'ui.prompt-only': { status: 'supported', confidence: 'high', source: 'vendor-doc' },
        },
      ...
      'claude-ios': {
        ...
        capabilities: {
          'network.http': { status: 'supported', confidence: 'medium', source: 'vendor-doc' },
          'shell.exec': { status: 'unsupported', confidence: 'medium', source: 'inference' },
          'fs.read': { status: 'unsupported', confidence: 'medium', source: 'inference' },
        },
      },
    },
    skills: [
      {
        ...
        invocation: null,
        tags: ['git', 'core'],
        capabilities: { required: ['shell.exec', 'git.read'], optional: ['fs.write'], fallback_mode: 'none' },
        compatibility: {
          'claude-code': { status: 'supported', mode: 'native', package: 'skill' },
          'claude-web': { status: 'excluded', mode: 'excluded', package: 'api', notes: 'No shell access.' },
          'claude-ios': { status: 'excluded', mode: 'excluded', package: 'api', notes: 'No shell access.' },
        },
        platforms: [],
      ...
      },
      {
        ...
        invocation: null,
        tags: ['review'],
        capabilities: { required: [], optional: ['fs.read', 'git.read'], fallback_mode: 'prompt-only' },
        compatibility: {
          'claude-code': { status: 'supported', mode: 'native', package: 'skill' },
...
describe('capability discovery API', () => {
  ...
  describe('/v1/capabilities/platform/:platform — happy path', () => {
    test('returns capability profile for claude-code', async () => {
      const worker = await loadWorkerWithFixtures(makeRegistry(), loadJson(PLUGIN_PATH));
      const { status, body } = await call(worker, '/v1/capabilities/platform/claude-code');
      ...
      assert.ok(Array.isArray(body.capabilities.supported), 'supported must be an array');
      assert.ok(Array.isArray(body.capabilities.unsupported), 'unsupported must be an array');
      assert.ok(Array.isArray(body.capabilities.unknown), 'unknown must be an array');
      assert.ok(body.capabilities.supported.includes('shell.exec'), 'claude-code supports shell.exec');
      assert.ok(body.capabilities.supported.includes('fs.read'), 'claude-code supports fs.read');
    });

    test('returns capability profile for claude-web', async () => {
      const worker = await loadWorkerWithFixtures(makeRegistry(), loadJson(PLUGIN_PATH));
      const { status, body } = await call(worker, '/v1/capabilities/platform/claude-web');
    ...
    test('returns capability profile for claude-ios', async () => {
      const worker = await loadWorkerWithFixtures(makeRegistry(), loadJson(PLUGIN_PATH));
      const { status, body } = await call(worker, '/v1/capabilities/platform/claude-ios');
    ...
    test('includes capability_detail with status, confidence and source per cap', async () => {
      const worker = await loadWorkerWithFixtures(makeRegistry(), loadJson(PLUGIN_PATH));
      const { body } = await call(worker, '/v1/capabilities/platform/claude-code');
    ...
    test('includes platform notes when present', async () => {
      const worker = await loadWorkerWithFixtures(makeRegistry(), loadJson(PLUGIN_PATH));
      const { body } = await call(worker, '/v1/capabilities/platform/claude-web');
      assert.ok(typeof body.notes === 'string' && body.notes.length > 0, 'notes present for claude-web');
    });

    test('omits notes field when platform has no notes', async () => {
      const worker = await loadWorkerWithFixtures(makeRegistry(), loadJson(PLUGIN_PATH));
      const { body } = await call(worker, '/v1/capabilities/platform/claude-ios');
    ...
    test('returns capabilities from the real compiled registry', async () => {
      ...
      const worker = await loadWorkerWithFixtures(registry, loadJson(PLUGIN_PATH));
      const { status, body } = await call(worker, '/v1/capabilities/platform/claude-code');

      assert.equal(status, 200);
      assert.equal(body.manifest_version, registry.version);
      assert.ok(body.capabilities.supported.length > 0, 'at least one supported capability');
    });
  });
  ...
  describe('/v1/capabilities/platform/:platform — validation', () => {
    test('returns 404 for unknown platform', async () => {
      const worker = await loadWorkerWithFixtures(makeRegistry(), loadJson(PLUGIN_PATH));
      const { status, body } = await call(worker, '/v1/capabilities/platform/nonexistent');
    ...
    test('blocks path traversal attempts', async () => {
      const worker = await loadWorkerWithFixtures(makeRegistry(), loadJson(PLUGIN_PATH));
      const { status, body } = await call(worker, '/v1/capabilities/platform/..%2F..%2Fetc%2Fpasswd');
    ...
    test('blocks slash-injected platform names', async () => {
      const worker = await loadWorkerWithFixtures(makeRegistry(), loadJson(PLUGIN_PATH));
      const { status } = await call(worker, '/v1/capabilities/platform/claude-code/extra');
  ...
  describe('CORS headers', () => {
    test('capability platform response includes CORS headers', async () => {
      const worker = await loadWorkerWithFixtures(makeRegistry(), loadJson(PLUGIN_PATH));
      const { headers } = await call(worker, '/v1/capabilities/platform/claude-web');
    ...
    test('compatible skills response includes CORS headers', async () => {
      const worker = await loadWorkerWithFixtures(makeRegistry(), loadJson(PLUGIN_PATH));
      const { headers } = await call(worker, '/v1/skills/compatible?caps=network.http');
    ...
    test('error responses include CORS headers (404)', async () => {
      const worker = await loadWorkerWithFixtures(makeRegistry(), loadJson(PLUGIN_PATH));
      const { status, headers } = await call(worker, '/v1/capabilities/platform/does-not-exist');
    ...
    test('OPTIONS preflight returns 204 with full CORS headers', async () => {
      ...
      const res = await worker.fetch(
        new Request('https://worker.test/v1/capabilities/platform/claude-web', { method: 'OPTIONS' }),
        baseEnv
      );
  ...
  describe('caching headers', () => {
    test('platform response has immutable cache-control', async () => {
      const worker = await loadWorkerWithFixtures(makeRegistry(), loadJson(PLUGIN_PATH));
      const { headers } = await call(worker, '/v1/capabilities/platform/claude-code');

      const cc = headers.get('cache-control') ?? '';
      assert.ok(cc.includes('immutable'), 'cache-control must include immutable');
      assert.ok(cc.includes('max-age=31536000'), 'cache-control must include 1-year max-age');
    });
  ...
  describe('/v1/skills/compatible — happy path', () => {
    test('returns skills with no required capabilities for any cap set', async () => {
      const worker = await loadWorkerWithFixtures(makeRegistry(), loadJson(PLUGIN_PATH));
      const { status, body } = await call(worker, '/v1/skills/compatible?caps=network.http');
      ...
      assert.ok(ids.includes('code-review'), 'code-review is compatible with any caps');
    });
    ...
    test('excludes skills whose required caps are not in request', async () => {
      ...
      const { body } = await call(worker, '/v1/skills/compatible?caps=network.http');

      const ids = body.skills.map(s => s.id);
      assert.ok(!ids.includes('git-ops'), 'git-ops excluded when shell.exec absent');
    });

    test('includes skill when all required caps are satisfied', async () => {
      const worker = await loadWorkerWithFixtures(makeRegistry(), loadJson(PLUGIN_PATH));
      const { body } = await call(worker, '/v1/skills/compatible?caps=shell.exec,git.read,network.http');

      const ids = body.skills.map(s => s.id);
      assert.ok(ids.includes('git-ops'), 'git-ops included when all required caps present');
    ...
    test('each skill entry has required fields', async () => {
      const worker = await loadWorkerWithFixtures(makeRegistry(), loadJson(PLUGIN_PATH));
      const { body } = await call(worker, '/v1/skills/compatible?caps=shell.exec,git.read');
    ...
    test('each compatible skill includes full pre-computed compatibility matrix', async () => {
      ...
      assert.ok(codeReview.compatibility['claude-code'], 'compat for claude-code present');
      assert.ok(codeReview.compatibility['claude-web'], 'compat for claude-web present');
      assert.ok(codeReview.compatibility['claude-ios'], 'compat for claude-ios present');
    });
    ...
    test('includes requested_capabilities in response', async () => {
      const worker = await loadWorkerWithFixtures(makeRegistry(), loadJson(PLUGIN_PATH));
      const { body } = await call(worker, '/v1/skills/compatible?caps=network.http,fs.read');
    ...
    test('deduplicates repeated capability IDs in request', async () => {
      const worker = await loadWorkerWithFixtures(makeRegistry(), loadJson(PLUGIN_PATH));
      const { status, body } = await call(worker, '/v1/skills/compatible?caps=network.http,network.http');

      assert.equal(status, 200);
      // Deduplicated — requested_capabilities has only one entry
      assert.equal(body.requested_capabilities.filter(c => c === 'network.http').length, 1);
    });
  ...
  describe('/v1/skills/compatible — validation', () => {
    ...
    test('returns 400 when caps param is empty string', async () => {
      const worker = await loadWorkerWithFixtures(makeRegistry(), loadJson(PLUGIN_PATH));
      const { status, body } = await call(worker, '/v1/skills/compatible?caps=');
    ...
    test('returns 400 for invalid capability ID format', async () => {
      const worker = await loadWorkerWithFixtures(makeRegistry(), loadJson(PLUGIN_PATH));
      const { status, body } = await call(worker, '/v1/skills/compatible?caps=UPPERCASE');
    ...
    test('returns 400 for capability IDs with no dot separator', async () => {
      const worker = await loadWorkerWithFixtures(makeRegistry(), loadJson(PLUGIN_PATH));
      const { status, body } = await call(worker, '/v1/skills/compatible?caps=nodot');
  ...
  describe('data consistency', () => {
    test('compatible skills: required caps are always a subset of requested', async () => {
      const worker = await loadWorkerWithFixtures(makeRegistry(), loadJson(PLUGIN_PATH));
      const { body } = await call(worker, '/v1/skills/compatible?caps=network.http,shell.exec');

      const capSet = new Set(['network.http', 'shell.exec']);
      for (const skill of body.skills) {
        for (const req of skill.capabilities.required) {
          assert.ok(capSet.has(req), `${skill.id} requires '${req}' which is not in requested caps`);
        }
      }
    });
  ...
  describe('edge cases', () => {
    test('registry with no skills returns empty compatible list gracefully', async () => {
      ...
      const worker = await loadWorkerWithFixtures(emptyRegistry, loadJson(PLUGIN_PATH));
      const { status, body } = await call(worker, '/v1/skills/compatible?caps=network.http');
    ...
    test('registry with no platform_definitions returns platform synthesised from compat blocks', async () => {
      ...
      const worker = await loadWorkerWithFixtures(noDefs, loadJson(PLUGIN_PATH));
      const { status, body } = await call(worker, '/v1/capabilities/platform/claude-web');
    ...
    test('caps param with only whitespace returns 400', async () => {
      const worker = await loadWorkerWithFixtures(makeRegistry(), loadJson(PLUGIN_PATH));
      const { status } = await call(worker, '/v1/skills/compatible?caps=%20,%20');
      assert.equal(status, 400);
    });
    ...
    test('skill with missing compatibility block is included with empty compat', async () => {
      ...
      const worker = await loadWorkerWithFixtures(registry, loadJson(PLUGIN_PATH));
      const { status, body } = await call(worker, '/v1/skills/compatible?caps=shell.exec,git.read');
