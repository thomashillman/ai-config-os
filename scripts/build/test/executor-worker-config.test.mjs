...
const executorWranglerPath = path.join(
  repoRoot,
  "worker/executor/wrangler.toml",
);
...
test("executor Worker: wrangler.toml exists", () => {
  assert.ok(
    fs.existsSync(executorWranglerPath),
    "worker/executor/wrangler.toml must exist",
  );
});
...
test("executor Worker: wrangler.toml has required fields", () => {
  ...
  assert.equal(
    ...
    "name must be ai-config-os-executor",
  );
...
test("executor Worker: wrangler.toml has KV binding for MANIFEST_KV", () => {
  ...
  assert.ok(
    manifestKv.id,
    "MANIFEST_KV binding must have an id (can be placeholder)",
  );
});
...
test("executor Worker: wrangler.toml has R2 binding for ARTEFACTS_R2", () => {
  ...
  assert.ok(
    artefactsR2.bucket_name,
    "ARTEFACTS_R2 binding must have a bucket_name",
  );
});
...
test("executor Worker: wrangler.toml has staging environment config", () => {
  ...
  assert.equal(
    config.env.staging.vars.ENVIRONMENT,
    "staging",
    'Staging ENVIRONMENT must be "staging"',
  );
});
...
test("executor Worker: package.json exists", () => {
  ...
  assert.ok(
    ...
    "worker/executor/package.json must exist",
  );
});
...
test("executor Worker: tsconfig.json exists and is valid", () => {
  ...
  assert.ok(
    ...
    "worker/executor/tsconfig.json must exist",
  );
...
test("executor Worker: README.md exists and documents Phase 1", () => {
  ...
  assert.match(
    content,
    /health_check/,
    "README must document health_check tool",
  );
  assert.match(
    content,
    /not.*support|NOT.*support|unavailable/i,
    "README must document what is not supported",
  );
});
