...
function validateBootstrapRun(payload) {
  ...
  for (const k of Object.keys(payload)) {
    if (FORBIDDEN.has(k.toLowerCase()))
      return {
        ok: false,
        error: `Field '${k}' is not permitted in a run record`,
      };
  }
  ...
  if (!["success", "failure", "partial"].includes(payload.status))
    return {
      ok: false,
      error: "Field 'status' must be one of: success, failure, partial",
    };
...
function sanitizeRecord(v) {
  ...
  if (isObj(v)) {
    const r = {};
    for (const [k, val] of Object.entries(v)) r[k] = sanitizeRecord(val);
    return r;
  }
  return v;
}
...
function validateObservabilitySettings(input) {
  ...
  for (const [f, b] of Object.entries(BOUNDS)) {
    const v = input[f];
    if (v === undefined) {
      result[f] = DEFAULT_SETTINGS[f];
      continue;
    }
    if (typeof v !== "number" || !Number.isInteger(v)) {
      errors.push(`Field '${f}' must be an integer`);
      continue;
    }
    if (v < b.min || v > b.max) {
      errors.push(
        `Field '${f}' must be between ${b.min} and ${b.max} (got ${v})`,
      );
      continue;
    }
    result[f] = v;
...
async function writeObservabilitySettings(kv, s) {
  await kv.put(KV_SETTINGS_KEY, JSON.stringify(s));
}

// --- storage ---
function runSummaryR2Key(id) {
  return `observability/runs/${id}/summary.json`;
}
function runMetaKvKey(id) {
  return `observability:run:${id}:meta`;
}
...
async function writeBootstrapRun(run, kv, r2) {
  ...
  return {
    run_id: san.run_id,
    summary_key: runSummaryR2Key(san.run_id),
    kv_meta_key: runMetaKvKey(san.run_id),
  };
}
async function listBootstrapRuns(kv, limit = 20) {
  const raw = await kv.get(INDEX_KV);
  if (!raw) return [];
  let idx;
  try {
    idx = JSON.parse(raw);
  ...
  for (const id of idx.slice(0, limit)) {
    const m = await kv.get(runMetaKvKey(id));
    if (m) {
      try {
        summaries.push(JSON.parse(m));
      } catch {}
    }
  }
  return summaries;
...
async function getBootstrapRun(runId, r2) {
  const obj = await r2.get(runSummaryR2Key(runId));
  if (!obj) return null;
  try {
...
async function getLatestRunSummary(kv) {
  const id = await kv.get(LATEST_KV);
  if (!id) return null;
  const m = await kv.get(runMetaKvKey(id));
  if (!m) return null;
  try {
...
function badReq(msg) {
  return jsonResp({ error: { code: "bad_request", message: msg } }, 400);
}
function nf(msg) {
  return jsonResp({ error: "Not Found", message: msg }, 404);
}
...
async function readBody(request) {
  try {
    ...
    return { ok: false, response: badReq("Invalid JSON body") };
  }
}
...
async function handleRunCreate(request, env) {
  ...
  if (!env.kv || !env.r2)
    return errEnvelope(
      "observability.runs.create",
      "Storage not configured",
      {
        code: "storage_unavailable",
        message: "KV or R2 not bound",
        hint: "Check bindings",
      },
      503,
    );
  try {
    ...
    return jsonResp(
      envelope("observability.runs.create", "Bootstrap run recorded", {
    ...
    return errEnvelope(
      ...
      "Failed to persist run",
      {
        ...
        message: "Failed to persist run record",
        hint: "Retry",
      },
      500,
    );
...
async function handleRunList(request, env) {
  if (!env.kv)
    return jsonResp(
      envelope("observability.runs.list", "No runs recorded yet", {
        runs: [],
        latest: null,
        count: 0,
      }),
  ...
  const [runs, latest] = await Promise.all([
    listBootstrapRuns(env.kv, limit),
    getLatestRunSummary(env.kv),
  ]);
  ...
  return jsonResp(
    envelope(
      ...
      enrichedLatest
        ? `Latest run: ${enrichedLatest.status}. ${runs.length} run(s).`
        : `${runs.length} run(s) recorded.`,
      { runs: enriched, latest: enrichedLatest, count: runs.length },
      {
        meta: {
          interpretation: {
            attention_required: enriched.some((r) => r.attention_required),
            failure_reason_summary:
              enrichedLatest?.failure_reason_summary ?? null,
          },
        },
      },
    ),
...
async function handleRunGet(runId, env) {
  ...
  return jsonResp(
    envelope(
      "observability.runs.get",
      `Run ${runId}: ${enriched.status}`,
      { run: enriched },
      {
        meta: {
          interpretation: {
            attention_required: enriched.attention_required,
            failure_reason_summary: enriched.failure_reason_summary,
          },
        },
      },
    ),
...
async function handleSettingsPut(request, env) {
  ...
  if (!v.ok)
    return jsonResp({ error: "Validation failed", details: v.errors }, 400);
  if (!env.kv)
    return errEnvelope(
      "observability.settings.put",
      "Storage not configured",
      {
        code: "storage_unavailable",
        message: "KV not bound",
        hint: "Check bindings",
      },
      503,
    );
  ...
  return jsonResp(
    envelope("observability.settings.put", "Settings saved", {
      settings: v.value,
    }),
  );
}
...
function makeR2() {
  ...
  return {
    ...
    get: async (k) => (k in s ? { text: async () => s[k] } : null),
  };
}
function makeEnv(opts = {}) {
  return { kv: opts.kv ?? makeKv(), r2: opts.r2 ?? makeR2() };
}
...
test('POST runs: returns 400 for forbidden field "token"', async () => {
  const env = makeEnv();
  const req = mockRequest("http://w/v1/observability/runs", {
    ...VALID_RUN,
    token: "secret",
  });
  const resp = await handleRunCreate(req, env);
...
test("GET runs: respects limit query parameter", async () => {
  ...
  for (let i = 1; i <= 5; i++) {
    await handleRunCreate(
      mockRequest("http://w/v1/observability/runs", {
        ...VALID_RUN,
        run_id: `r${i}`,
      }),
      env,
  ...
  const resp = await handleRunList(
    mockRequest("http://w/v1/observability/runs?limit=2", null),
    env,
  );
...
test("GET runs: summaries do not contain raw phase arrays", async () => {
  const env = makeEnv();
  await handleRunCreate(
    mockRequest("http://w/v1/observability/runs", VALID_RUN),
    env,
  );
  const resp = await handleRunList(
    mockRequest("http://w/v1/observability/runs", null),
    env,
  );
...
test("PUT settings: persists valid settings", async () => {
  const env = makeEnv();
  const req = mockRequest("http://w/v1/observability/settings", {
    ...DEFAULT_SETTINGS,
...
test("PUT settings: returns 503 when KV not configured", async () => {
  ...
  const req = mockRequest(
    "http://w/v1/observability/settings",
    DEFAULT_SETTINGS,
  );
  const resp = await handleSettingsPut(req, env);
  assert.equal(resp.status, 503);
