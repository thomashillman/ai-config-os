...
describe("materialise.sh install command", () => {
  test(
    "install command requires a cached version file",
    SHELL_TEST_OPTIONS,
    () => {
      ...
      try {
        ...
        assert.notEqual(
          result.status,
          0,
          "install should fail when no cached version exists",
        );
        assert.ok(
  ...
  test(
    "install copies skills from cache to ~/.claude/skills",
    SHELL_TEST_OPTIONS,
    () => {
      const tmpHome = mkdtempSync(join(tmpdir(), "materialise-install-"));
      const tmpProject = mkdtempSync(join(tmpdir(), "materialise-project-"));
      ...
      try {
        ...
        writeFileSync(
          join(cacheDir, "skills", "skill-a", "SKILL.md"),
          "# Skill A",
        );

        mkdirSync(join(cacheDir, "skills", "skill-b"), { recursive: true });
        writeFileSync(
          join(cacheDir, "skills", "skill-b", "SKILL.md"),
          "# Skill B",
        );
        ...
        assert.ok(
          existsSync(join(skillsDir, "skill-a", "SKILL.md")),
          "skill-a should be installed",
        );
        assert.ok(
          existsSync(join(skillsDir, "skill-b", "SKILL.md")),
          "skill-b should be installed",
        );

        // Verify version marker was written
        assert.ok(
          existsSync(join(skillsDir, ".version")),
  ...
  test(
    "install is idempotent (no-op when version matches)",
    SHELL_TEST_OPTIONS,
    () => {
      const tmpHome = mkdtempSync(join(tmpdir(), "materialise-install-"));
      const tmpProject = mkdtempSync(join(tmpdir(), "materialise-project-"));
