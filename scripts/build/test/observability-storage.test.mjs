...
function runSummaryR2Key(runId) {
  return `observability/runs/${runId}/summary.json`;
}
function runMetaKvKey(runId) {
...
async function listBootstrapRuns(kv, limit = 20) {
  ...
  for (const runId of runIds) {
    ...
    if (!meta) continue;
    try {
      summaries.push(JSON.parse(meta));
    } catch {}
  }
  return summaries;
}
...
test("runSummaryR2Key: produces correct path", () => {
  assert.equal(
    runSummaryR2Key("run-abc"),
    "observability/runs/run-abc/summary.json",
  );
});
...
test("listBootstrapRuns: respects limit parameter", async () => {
  ...
  const r2 = makeR2();
  for (let i = 1; i <= 5; i++)
    await writeBootstrapRun(makeRun(`r${i}`), kv, r2);
