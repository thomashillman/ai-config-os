...
test("createMomentumEngine requires taskStore with progressEvents", () => {
  assert.throws(
    () => createMomentumEngine({ taskStore: {} }),
    /progressEvents/,
  );
});
...
test("startReviewRepositoryTask with narrator produces narration and records observation", () => {
  ...
  const observer = new MomentumObserver({
    progressEventStore: store.progressEvents,
  });
  ...
  assert.ok(
    result.narration.headline.length > 0,
    "headline should not be empty",
  );
  assert.equal(result.narration.strength.level, "degraded");
  assert.equal(
    result.narration.upgrade,
    null,
    "should not claim blocked upgrade",
  );
  assert.match(
    result.narration.progress,
...
test("buildTaskReadinessView with narrator does not produce upgrade narration when host lacks required capability", () => {
  ...
  assert.equal(view.narration, null);
  assert.match(
    view.readiness.upgrade_unavailable_reason,
    /Upgrade unavailable due to missing capability:/,
  );
});
...
test("engine.buildShelf ranks tasks with narrator-produced headlines", () => {
  ...
  const shelf = engine.buildShelf({
    tasks,
    currentCapabilities: strongProfile(),
  });
