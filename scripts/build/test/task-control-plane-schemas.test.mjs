...
test("T002 control-plane schemas exist with versioned ids and schema_version guards", () => {
  for (const fileName of Object.values(schemaFiles)) {
    ...
    assert.ok(
      schema.$id.includes("/contracts/v1/"),
      `${fileName} should be versioned under /contracts/v1/`,
    );
    assert.ok(
      schema.properties?.schema_version,
      `${fileName} must define schema_version`,
    );
    assert.equal(
      schema.properties.schema_version.const,
      "1.0.0",
      `${fileName} schema_version must be pinned to 1.0.0`,
    );
  }
});
