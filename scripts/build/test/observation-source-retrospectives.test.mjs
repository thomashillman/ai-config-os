...
test("mapRetrospectiveToObservations with empty artifact", () => {
  ...
  assert.equal(
    observations.length,
    0,
    "Empty artifact produces no observations",
  );
});
...
test("mapRetrospectiveToObservations maps friction signals to observations", () => {
  ...
  assert.equal(
    observations[0].metadata.description,
    "No skill for db schema lookup",
  );
  ...
  assert.equal(
    observations[1].metadata.description,
    "Tool re-invoked twice due to incomplete output",
  );
...
test("mapRetrospectiveToObservations maps skill recommendations to observations", () => {
  ...
  assert.equal(
    obs.metadata.rationale,
    "Repeated schema lookups during session",
  );
