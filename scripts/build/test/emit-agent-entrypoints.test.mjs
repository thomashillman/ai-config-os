...
test('emit-agent-entrypoints includes base doctrine in both root files and keeps surfaces isolated', () => {
  ...
  try {
    ...
    writeFixtureFile(repoDir, 'shared/agent-doctrine/surfaces/claude.md', 'Claude-only guidance');
    writeFixtureFile(repoDir, 'shared/agent-doctrine/surfaces/codex.md', 'Codex-only guidance');
    ...
    assert.equal(claude.includes('Codex-only guidance'), false, 'Codex guidance must not leak into CLAUDE.md');
    assert.equal(codex.includes('Claude-only guidance'), false, 'Claude guidance must not leak into AGENTS.md');
  } finally {
...
test('emit-agent-entrypoints emits size warnings when configured threshold is exceeded', () => {
  const repoDir = mkdtempSync(join(tmpdir(), 'agent-entrypoints-size-warning-'));

  try {
    writeFixtureFile(repoDir, 'shared/agent-doctrine/base/10-first.md', '# Base\n\n' + 'x'.repeat(200));
    writeFixtureFile(repoDir, 'shared/agent-doctrine/surfaces/claude.md', 'Claude-only guidance');
    writeFixtureFile(repoDir, 'shared/agent-doctrine/surfaces/codex.md', 'Codex-only guidance');
    writeFixtureFile(repoDir, 'shared/agent-doctrine/repos/ai-config-os/10-first.overlay.md', 'Overlay A');
