...
import {
  TOOL_REGISTRY,
  getToolList,
} from "../../../runtime/mcp/tool-registry.mjs";
  ...
  for (const name of EXPECTED_TOOLS) {
    test(`registry contains '${name}'`, () => {
      assert.ok(
        ...
        `TOOL_REGISTRY missing entry for '${name}'`,
      );
    });
  }
});
...
describe("TOOL_REGISTRY — dispatch: mcp_add", () => {
  ...
  test("handles missing args array gracefully", () => {
    ...
    TOOL_REGISTRY.mcp_add.run(
      { runScript },
      { name: "srv", command: "node server.js" },
    );
  ...
  test("delegates to taskService.startReviewRepositoryTask with remapped keys", () => {
    ...
    const result = TOOL_REGISTRY.task_start_review_repository.run(
      { taskService },
      {
        ...
        goal: "review it",
        route_inputs: { pr: 42 },
        capability_profile: { mode: "local-cli" },
      },
    );
...
describe("TOOL_REGISTRY — dispatch: task_get_readiness", () => {
  test("wraps taskService.getReadiness result in readiness key", () => {
    const taskService = {
      getReadiness: (id) => ({ task_id: id, is_ready: true }),
    };
    const result = TOOL_REGISTRY.task_get_readiness.run(
      { taskService },
      { task_id: "t3" },
    );
  ...
  test("each entry has name, description, and inputSchema", () => {
    for (const entry of getToolList()) {
      ...
      assert.equal(
        typeof entry.description,
        "string",
        "entry.description must be string",
      );
      assert.equal(
        typeof entry.inputSchema,
        "object",
        "entry.inputSchema must be object",
      );
    }
  });

  test("entry names match TOOL_REGISTRY keys exactly", () => {
    const listNames = getToolList()
      .map((e) => e.name)
      .sort();
