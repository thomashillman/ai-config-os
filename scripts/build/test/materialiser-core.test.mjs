...
test("Materialiser Core", async (t) => {
  ...
  function createTestPackage() {
    ...
    writeFileSync(
      join(tmp, "skills", "skill-one", "SKILL.md"),
      "---\nskill: skill-one\n---\nContent one",
    );
    writeFileSync(
      join(tmp, "skills", "skill-two", "SKILL.md"),
      "---\nskill: skill-two\n---\nContent two",
    );
  ...
  await t.test(
    "readPackageMetadata: should fail if package root does not exist",
    () => {
  ...
  await t.test(
    "readPackageMetadata: should fail if plugin.json missing",
    () => {
      const tmp = mkdtempSync(join(tmpdir(), "materialise-test-"));
  ...
  await t.test(
    "readPackageMetadata: should fail if plugin.json is invalid JSON",
    () => {
      const tmp = mkdtempSync(join(tmpdir(), "materialise-test-"));
  ...
  await t.test(
    "readPackageMetadata: should fail if version field missing",
    () => {
      const tmp = mkdtempSync(join(tmpdir(), "materialise-test-"));
      mkdirSync(join(tmp, ".claude-plugin"), { recursive: true });
      writeFileSync(
  ...
  await t.test(
    "validatePackageContents: should succeed for valid package",
    () => {
      const pkg = createTestPackage();
  ...
  await t.test(
    "validatePackageContents: should reject path traversal attempts",
    () => {
      const pkg = createTestPackage();
  ...
  await t.test(
    "validatePackageContents: should reject missing skill files",
    () => {
      const pkg = createTestPackage();
  ...
  await t.test(
    "materializePackage: should extract all skills to destination",
    () => {
      const pkg = createTestPackage();
      ...
      try {
        ...
        const content = readFileSync(
          join(dest, "skills", "skill-one", "SKILL.md"),
          "utf8",
        );
  ...
  await t.test("materializePackage: should support dryRun option", () => {
    ...
    try {
      ...
      assert.ok(
        ...
        "No files should be created in dryRun",
      );
    } finally {
      rmSync(pkg, { recursive: true, force: true });
      rmSync(dest, { recursive: true, force: true });
  ...
  await t.test(
    "materializePackage: should create destination if needed",
    () => {
      const pkg = createTestPackage();
  ...
  await t.test(
    "Security: should reject complex path traversal patterns",
    () => {
      const pkg = createTestPackage();
  ...
  await t.test(
    "Security: should reject directories when files expected",
    () => {
      const pkg = createTestPackage();
  ...
  await t.test(
    "Security: materializePackage should refuse to extract from untrusted packages",
    () => {
      const pkg = createTestPackage();
      const dest = mkdtempSync(join(tmpdir(), "materialise-dest-"));
