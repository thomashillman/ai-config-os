...
describe("loadProbeResults — edge cases", () => {
  ...
  test("probe with error-status cap → treated as unsupported", () => {
    ...
    try {
      ...
      assert.ok(
        !result.supported.has("shell.exec"),
        "error status must be treated as unsupported",
      );
  ...
  test("capability absent from probe results → treated as unsupported", () => {
    ...
    try {
      ...
      assert.ok(result.supported.has("fs.read"));
      assert.ok(
        !result.supported.has("git.read"),
        "absent cap must be treated as unsupported",
      );
    } finally {
      rmSync(dir, { recursive: true, force: true });
    }
  ...
  test("valid manifest → skills array returned", () => {
    ...
    try {
      writeFileSync(
        ...
        JSON.stringify({
          version: "0.5.4",
          skills: [makeSkill({ id: "skill-a" }), makeSkill({ id: "skill-b" })],
        }),
      );
...
describe("filterSkills — end-to-end with temp files", () => {
  test("probe missing → all skills available + warning", () => {
    ...
    try {
      ...
      assert.equal(
        ...
        2,
        "all skills should be available when probe is missing",
      );
  ...
  test("claude-ssh probe does not crash or downgrade everything when platform is recognised", () => {
    ...
    try {
      ...
      assert.deepEqual(
        result.degraded.map((skill) => skill.id),
        ["ssh-degraded"],
      );
      assert.deepEqual(
        result.excluded.map((skill) => skill.id),
        ["ssh-fallback"],
      );
    ...
    try {
      ...
      writeFileSync(
        ...
        JSON.stringify({
          version: "0.5.4",
          platforms: ["claude-code"],
          skills: [makeSkill({ id: "remote-shell", required: ["shell.exec"] })],
        }),
      );
      ...
      assert.equal(result.registry_platform_hint, "claude-code");
      assert.equal(
        result.available.map((skill) => skill.id)[0],
        "remote-shell",
      );
      assert.equal(result.warning, null);
    } finally {
...
describe("formatSummary", () => {
  test("all buckets populated → includes all counts", () => {
    ...
    assert.ok(
      summary.includes("3 available"),
      `expected "3 available" in: ${summary}`,
    );
    assert.ok(
      summary.includes("1 degraded"),
      `expected "1 degraded" in: ${summary}`,
    );
    assert.ok(
      summary.includes("2 excluded"),
      `expected "2 excluded" in: ${summary}`,
    );
    assert.ok(
      summary.includes("1 unavailable"),
      `expected "1 unavailable" in: ${summary}`,
    );
  });
  ...
  test("empty buckets omitted from summary", () => {
    ...
    assert.ok(
      ...
      "degraded should be omitted when empty",
    );
    assert.ok(
      !summary.includes("excluded"),
      "excluded should be omitted when empty",
    );
    assert.ok(
      !summary.includes("unavailable"),
      "unavailable should be omitted when empty",
    );
  });
});
