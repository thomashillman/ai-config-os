...
const SUPPORTED_RUNTIME_PLATFORMS = [
  ...
  },
  {
    ...
    selection: {
      type: "runtime",
      env: { IDEA_HOME: "/Applications/IntelliJ IDEA.app" },
    },
...
const COMPILE_TIME_ONLY_PLATFORMS = [
  {
    ...
    selection: {
      type: "compile-time",
      story:
        "compile-time package selection only until Claude Desktop exposes a distinct runtime signal",
    },
    expectedSurface: "desktop-app",
  },
];
...
function ensureFreshRegistry() {
  ...
  assert.equal(
    result.status,
    0,
    `compile.mjs must succeed before parity assertions`,
  );
  registryBuilt = true;
}
...
describe("platform/probe parity contract", () => {
  ...
  test(
    "runtime-detectable platforms resolve to YAML and registry definitions with matching surface hints",
    {
