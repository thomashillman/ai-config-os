...
function createScaffoldFixture() {
  ...
  if (existsSync(join(REPO_ROOT, "schemas", "skill.schema.json"))) {
    copyFileSync(
      join(REPO_ROOT, "schemas", "skill.schema.json"),
...
test("new-skill.mjs does not change VERSION, package.json, or plugin.json", () => {
  ...
  try {
    ...
    const pluginBefore = readFileSync(
      join(fixture, "plugins", "core-skills", ".claude-plugin", "plugin.json"),
      "utf8",
    );
    ...
    const result = spawnSync(
      process.execPath,
      [NEW_SKILL_MJS, FIXTURE_SCAFFOLD_A, "--no-link"],
      {
        cwd: fixture,
        encoding: "utf8",
      },
    ...
    assert.ok(
      skillContent.includes(`skill: ${FIXTURE_SCAFFOLD_A}`),
      "SKILL.md should have skill name in frontmatter",
    );
    assert.ok(
      !skillContent.includes("{{SKILL_NAME}}"),
      "Template placeholders should be replaced",
    );

    // Assert manifest was updated
    const manifest = readFileSync(
      join(fixture, "shared", "manifest.md"),
      "utf8",
    );
    assert.ok(
      manifest.includes(FIXTURE_SCAFFOLD_A),
...
test("new-skill.mjs --no-link creates skill without symlinks (portable)", () => {
  ...
  try {
    const result = spawnSync(
      process.execPath,
      [NEW_SKILL_MJS, FIXTURE_SCAFFOLD_B, "--no-link"],
      {
        cwd: fixture,
        encoding: "utf8",
      },
    ...
    assert.ok(
      existsSync(skillDir),
      "Skill directory should exist in shared/skills/",
    );
    assert.ok(existsSync(join(skillDir, "SKILL.md")), "SKILL.md should exist");
    ...
    assert.ok(
      ...
      "Symlink must NOT exist when --no-link is used",
    );
  } finally {
    rmSync(fixture, { recursive: true, force: true });
  }
...
test(
  "new-skill.mjs creates symlink on Unix (optional)",
  { skip: process.platform === "win32" ? "symlinks need Unix" : false },
  () => {
    const fixture = createScaffoldFixture();
    ...
    try {
      const result = spawnSync(
        process.execPath,
        [NEW_SKILL_MJS, FIXTURE_SCAFFOLD_C],
        {
          cwd: fixture,
          encoding: "utf8",
        },
      ...
      assert.ok(
        existsSync(join(symlinkPath, "SKILL.md")),
        "Symlink should resolve to skill with SKILL.md",
      );
    } finally {
      rmSync(fixture, { recursive: true, force: true });
    }
...
test("new-skill.mjs rejects duplicate skill name", () => {
  ...
  try {
    ...
    const result1 = spawnSync(
      process.execPath,
      [NEW_SKILL_MJS, FIXTURE_SCAFFOLD_D, "--no-link"],
      {
        cwd: fixture,
        encoding: "utf8",
      },
...
test("new-skill.mjs does not change VERSION (version invariance)", () => {
  ...
  try {
    ...
    const result = spawnSync(
      process.execPath,
      [NEW_SKILL_MJS, FIXTURE_SCAFFOLD_E, "--no-link"],
      {
        cwd: fixture,
        encoding: "utf8",
      },
...
test("release build has consistent provenance in claude-code, registry, and cursor", () => {
  const result = spawnSync(process.execPath, [COMPILE_MJS, "--release"], {
    ...
    env: {
      ...process.env,
      GITHUB_SHA: "prov-test-sha",
      GITHUB_RUN_ID: "prov-test-run",
    },
  });
  ...
  assert.equal(
    pluginJson.source_commit,
    "prov-test-sha",
    "claude-code: source_commit",
  );
  ...
  assert.ok(
    existsSync(cursorMetaPath),
    "Cursor .emit-meta.json MUST be emitted in release mode",
  );
  const cursorMeta = JSON.parse(readFileSync(cursorMetaPath, "utf8"));
  assert.equal(cursorMeta.version, expectedVersion, "cursor meta version");
  assert.ok(cursorMeta.built_at, "cursor: built_at required");
  assert.equal(cursorMeta.build_id, "prov-test-run", "cursor: build_id");
  assert.equal(
    cursorMeta.source_commit,
    "prov-test-sha",
...
test("local build has no provenance in claude-code, registry, or cursor", () => {
  ...
  assert.equal(
    pluginJson.built_at,
    undefined,
    "claude-code: no built_at in local",
  );
  assert.equal(
    pluginJson.build_id,
    undefined,
    "claude-code: no build_id in local",
  );
  assert.equal(
    pluginJson.source_commit,
    undefined,
    "claude-code: no source_commit in local",
  );
  ...
  assert.equal(
    registryJson.built_at,
    undefined,
    "registry: no built_at in local",
  );
  assert.equal(
    registryJson.build_id,
    undefined,
    "registry: no build_id in local",
  );
  assert.equal(
    registryJson.source_commit,
    undefined,
    "registry: no source_commit in local",
  );
  ...
  assert.ok(
    existsSync(cursorMetaPath),
    "Cursor .emit-meta.json MUST be emitted in local mode",
  );
  const cursorMeta = JSON.parse(readFileSync(cursorMetaPath, "utf8"));
  assert.equal(cursorMeta.built_at, undefined, "cursor: no built_at in local");
  assert.equal(cursorMeta.build_id, undefined, "cursor: no build_id in local");
  assert.equal(
    cursorMeta.source_commit,
    undefined,
    "cursor: no source_commit in local",
  );
});
