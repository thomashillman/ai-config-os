...
const GENERATE_COMMANDS = new URL(
  "../../../adapters/claude/generate-commands.mjs",
  import.meta.url,
).href;
...
function makeTempEnv(skills = [], probeOverrides = {}, modelHint = "") {
  const tmp = join(
    tmpdir(),
    `gc-test-${Date.now()}-${Math.random().toString(36).slice(2)}`,
  );
  ...
  writeFileSync(
    ...
    JSON.stringify(makeManifest(skills)),
  );
...
async function loadModule() {
  const { selectVariant, buildCommandContent, generateCommands } = await import(
    GENERATE_COMMANDS
  );
...
describe("selectVariant", () => {
  ...
  test("5. defaults to sonnet when no model hint and no matching variant", async () => {
    const { selectVariant } = await loadModule();
    const skill = makeSkill({
      variants: { sonnet: { prompt_file: "p.md" }, fallback_chain: ["sonnet"] },
    });
...
describe("buildCommandContent", () => {
  test("7. includes skill name as H1", async () => {
    ...
    assert.ok(
      content.includes("# security-review"),
      "Should have H1 with skill name",
    );
  });
  ...
  test("8. includes description", async () => {
    ...
    assert.ok(
      content.includes("OWASP security review."),
  ...
  test("9. includes body content", async () => {
    ...
    assert.ok(
      content.includes("Full instructions here."),
      "Should include body",
    );
  });
  ...
  test("10. embeds generated marker comment", async () => {
    ...
    assert.ok(
      content.includes("generated by ai-config-os"),
...
describe("generateCommands", () => {
  test("11. writes one file per available/degraded skill", async () => {
    ...
    assert.ok(
      existsSync(join(env.commandsDir, "skill-a.md")),
      "skill-a.md should exist",
    );
    assert.ok(
      existsSync(join(env.commandsDir, "skill-b.md")),
      "skill-b.md should exist",
    );
  ...
  test("12. skips excluded and unavailable skills", async () => {
    ...
    const skills = [
      ...
      makeSkill({
        id: "skill-gone",
        capabilities: { required: ["mcp.client"], optional: [] },
      }),
    ];
    ...
    assert.ok(
      existsSync(join(env.commandsDir, "skill-ok.md")),
      "available skill should be written",
    );
    assert.ok(
      !existsSync(join(env.commandsDir, "skill-excluded.md")),
      "excluded skill should not be written",
    );
    assert.ok(
      !existsSync(join(env.commandsDir, "skill-gone.md")),
      "unavailable skill should not be written",
    );
  ...
  test("14. cleans stale generated commands on re-run", async () => {
    ...
    writeFileSync(
      join(env.cacheDir, "latest.json"),
      JSON.stringify(makeManifest(skillsSecond)),
    );
    ...
    assert.ok(
      existsSync(join(env.commandsDir, "skill-keep.md")),
      "kept skill should still exist",
    );
    assert.ok(
      !existsSync(join(env.commandsDir, "skill-stale.md")),
  ...
  test("16. selects haiku variant when probe surface is mobile", async () => {
    ...
    const content = readFileSync(
      join(env.commandsDir, "skill-mobile.md"),
      "utf8",
    );
    assert.ok(
      content.includes("variant: haiku"),
      "Mobile surface should use haiku variant",
    );
