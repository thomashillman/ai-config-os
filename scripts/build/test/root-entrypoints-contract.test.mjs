...
function getSizeWarnings(files, thresholdBytes) {
  return files
    ...
    .map(
      ({ file, size }) =>
        `[warn] ${file} is ${size} bytes (> ${thresholdBytes})`,
    );
}
...
test("generation sync (root files stale check)", () => {
  ...
  assert.match(
    claude,
    /^> Generated file\. Edit doctrine fragments, not this file\./m,
  );
  assert.ok(
  ...
  assert.equal(
    plugin.version,
    version,
    "claude plugin version must be in sync with VERSION",
  );
  assert.equal(
    agentsVersion,
    version,
    "codex AGENTS version header must be in sync with VERSION",
  );
});
...
test("deterministic output (run generator twice, assert byte-identical)", () => {
  ...
  assert.deepEqual(
    second,
    first,
    "compile output should be byte-identical across consecutive runs",
  );
});
...
test("composition (base doctrine appears in both surfaces where expected)", () => {
  ...
  for (const phrase of doctrineAnchors) {
    assert.ok(
      claude.includes(phrase),
      `CLAUDE.md should include doctrine anchor: ${phrase}`,
    );
    assert.ok(
      agents.includes(phrase),
      `AGENTS.md should include doctrine anchor: ${phrase}`,
    );
  }
...
test("separation (ai-config-os local rules absent from base-only external materialisation)", () => {
  ...
  assert.ok(
    claude.includes(localOnlyMarker),
    "CLAUDE.md should retain repo-local operational guidance",
  );
  assert.equal(
    agents.includes(localOnlyMarker),
    false,
    "AGENTS.md should not include repo-local operational guidance",
  );
});
...
test("materialisation fixture (writes valid CLAUDE.md and AGENTS.md into temp target repo)", () => {
  ...
  try {
    ...
    assert.ok(
      writtenAgents.includes(`# Version: ${version}`),
      "AGENTS.md should carry the expected version header",
    );
...
test("size sanity warning (non-failing warning threshold for oversized root entrypoints)", () => {
  ...
  assert.ok(
    warnings.length >= 1,
    "low threshold should emit at least one warning",
  );
  assert.equal(
    warnings.every((warning) => warning.startsWith("[warn] ")),
    true,
  );
});
