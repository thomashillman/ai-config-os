...
function captureDistOutput() {
  const tmpDir = join(
    tmpdir(),
    `dist-capture-${Date.now()}-${Math.random().toString(36).slice(2)}`,
  );
...
function compareDistSnapshots(snap1, snap2) {
  ...
  return {
    hashes1,
    hashes2,
    match: JSON.stringify(hashes1) === JSON.stringify(hashes2),
  };
}
...
test("release builds are deterministic (excluding timestamps)", () => {
  ...
  const plugin1 = JSON.parse(
    readFileSync(
      join(snap1, "clients", "claude-code", ".claude-plugin", "plugin.json"),
      "utf8",
    ),
  );
  const registry1 = JSON.parse(
    readFileSync(join(snap1, "registry", "index.json"), "utf8"),
  );
  ...
  const plugin2 = JSON.parse(
    readFileSync(
      join(snap2, "clients", "claude-code", ".claude-plugin", "plugin.json"),
      "utf8",
    ),
  );
  const registry2 = JSON.parse(
    readFileSync(join(snap2, "registry", "index.json"), "utf8"),
  );
  ...
  try {
    ...
    assert.ok(
      plugin1.build_id && plugin2.build_id,
      "Both builds should have build_id",
    );
    assert.equal(
      plugin1.build_id,
      plugin2.build_id,
      "build_id should match (same env)",
    );
    assert.equal(
      plugin1.source_commit,
      plugin2.source_commit,
      "source_commit should match (same env)",
    );
  } finally {
    rmSync(snap1, { recursive: true, force: true });
    rmSync(snap2, { recursive: true, force: true });
...
test("release builds differ only in provenance when env changes", () => {
  ...
  try {
    ...
    assert.notEqual(
      plugin1.source_commit,
      plugin2.source_commit,
      "Plugin source_commit should differ",
    );
    assert.notEqual(
      plugin1.build_id,
      plugin2.build_id,
      "Plugin build_id should differ",
    );
    assert.notEqual(
      registry1.source_commit,
      registry2.source_commit,
      "Registry source_commit should differ",
    );
    assert.notEqual(
      registry1.build_id,
      registry2.build_id,
      "Registry build_id should differ",
    );
    ...
    assert.deepEqual(
      plugin1NonProv,
      plugin2NonProv,
      "Plugin should differ only in provenance fields",
    );
    ...
    assert.deepEqual(
      registry1NonProv,
      registry2NonProv,
      "Registry should differ only in provenance fields",
    );
  } finally {
    rmSync(snap1, { recursive: true, force: true });
    rmSync(snap2, { recursive: true, force: true });
...
test("release build populates all provenance fields consistently", () => {
  ...
  const plugin = JSON.parse(
    readFileSync(
      join(DIST_DIR, "clients", "claude-code", ".claude-plugin", "plugin.json"),
      "utf8",
    ),
  );
  ...
  assert.equal(
    plugin.build_id,
    env.GITHUB_RUN_ID,
    "Plugin build_id should match env",
  );
  assert.equal(
    plugin.source_commit,
    env.GITHUB_SHA,
    "Plugin source_commit should match env",
  );
  assert.equal(
    registry.build_id,
    env.GITHUB_RUN_ID,
    "Registry build_id should match env",
  );
  assert.equal(
    registry.source_commit,
    env.GITHUB_SHA,
    "Registry source_commit should match env",
  );
});
