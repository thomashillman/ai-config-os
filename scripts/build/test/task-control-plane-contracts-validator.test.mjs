...
test('validateContract accepts all new T002 kinds with canonical payloads', () => {
  ...
  assert.equal(validateContract('effectiveExecutionContract', effectiveExecutionContract), effectiveExecutionContract);
  ...
  assert.equal(validateContract('findingsLedgerEntry', findingsEntry), findingsEntry);
  assert.equal(validateContract('handoffToken', handoffToken), handoffToken);
  assert.equal(validateContract('continuationPackage', continuationPackage), continuationPackage);
});

test('validateContract rejects adversarial shape drift for new T002 kinds', () => {
  assert.throws(() => validateContract('portableTaskObject', { ...baseTask(), schema_version: '2.0.0' }), /Invalid portableTaskObject/);
  assert.throws(() => validateContract('taskRouteDefinition', { ...baseTaskRoute(), unknown_field: true }), /Invalid taskRouteDefinition/);
  assert.throws(() => validateContract('effectiveExecutionContract', {
    schema_version: '1.0.0',
    task_id: 'task_review_repository_001',
    task_type: 'review_repository',
  ...
  assert.throws(() => validateContract('handoffToken', {
    schema_version: '1.0.0',
    token_id: 'handoff_001',
    task_id: 'task_review_repository_001',
    issued_at: '2026-03-12T12:00:00.000Z',
