...
function getArtefacts() {
  ...
  const claudeCodePlugin = JSON.parse(
    readFileSync(claudeCodePluginPath, "utf8"),
  );

  const registryPath = join(REPO_ROOT, "dist", "registry", "index.json");
  const registry = JSON.parse(readFileSync(registryPath, "utf8"));

  const cursorMetaPath = join(
    REPO_ROOT,
    "dist",
    "clients",
    "cursor",
...
test("cursor skills directory and .emit-meta.json exist with expected shape", () => {
  ...
  assert.ok(
    /^\d+\.\d+\.\d+$/.test(cursorMeta.version),
    "meta.version should be semver",
  );
  assert.ok(cursorMeta.skills_count > 0, "skills_count should be positive");
  const dirs = readdirSync(cursorSkillsDir, { withFileTypes: true }).filter(
    (d) => d.isDirectory(),
  );
  assert.equal(
    dirs.length,
...
test("cursor skills contain at least one registry skill with valid frontmatter", () => {
  ...
  assert.ok(
    overlap.length > 0,
    "At least one registry skill should be emitted under cursor/skills",
  );
  ...
  assert.ok(
    md.includes(`name: ${sampleId}`),
    "Frontmatter should include name matching folder",
  );
});
...
test("registry lists expected platforms", () => {
  ...
  assert.ok(
    ...
    "Registry should have platforms array",
  );
  assert.ok(
  ...
  const hasCommonPlatforms = registry.platforms.some(
    (p) => p === "claude-code" || p === "cursor",
  );
  assert.ok(
    hasCommonPlatforms,
    "Registry should include common platforms (claude-code or cursor)",
  );
});
...
test("all registry skills have compatibility matrix", () => {
  ...
  assert.ok(
    ...
    "Registry should have skills array",
  );
  assert.ok(
    registry.skills.length > 0,
    "Registry should have at least one skill",
  );
...
test("claude-code plugin.json has correct version and structure", () => {
  ...
  assert.ok(
    /^\d+\.\d+\.\d+$/.test(claudeCodePlugin.version),
    "Version should be semver",
  );
  assert.ok(claudeCodePlugin.skills, "Plugin should have skills array");
  assert.ok(
    Array.isArray(claudeCodePlugin.skills),
    "Plugin skills should be array",
  );
  ...
  for (const skill of claudeCodePlugin.skills) {
    ...
    if (skill.name && skill.path && !skill.path.includes(skill.name)) {
      failures.push(
        `  ${skill.name}: path '${skill.path}' should include skill name`,
      );
    }
  }
  assert.equal(
    failures.length,
    0,
    `${failures.length} plugin skill field issue(s):\n${failures.join("\n")}`,
  );
});
...
test("runtime docs are emitted with deterministic artifact hashes", () => {
  ...
  assert.ok(
    existsSync(toolRegistryPath),
    "runtime tool-registry.json must exist",
  );
  assert.ok(
    existsSync(taskRouteDefinitionsPath),
    "runtime task-route-definitions.json must exist",
  );
  assert.ok(
    existsSync(taskRouteInputDefinitionsPath),
    "runtime task-route-input-definitions.json must exist",
  );
  ...
  assert.equal(
    ...
    1,
    "runtime manifest schemaVersion should be 1",
  );
  assert.ok(
    runtimeManifest.documents,
    "runtime manifest should contain documents mapping",
  );
  assert.ok(
    runtimeManifest.artifactHashes,
    "runtime manifest should contain artifactHashes",
  );
  ...
  for (const relativePath of docPaths) {
    assert.ok(
      ...
      "document path should be non-empty",
    );
    const absolutePath = join(REPO_ROOT, "dist", relativePath);
    assert.ok(
      existsSync(absolutePath),
      `document should exist: ${relativePath}`,
    );
    ...
    if (relativePath === runtimeManifest.documents.manifest) {
      ...
      const expectedManifestHash =
        hashManifestWithRedactedSelfHash(runtimeManifest);
      assert.equal(
  ...
  const emittedTaskRouteDefinitions = JSON.parse(
    readFileSync(taskRouteDefinitionsPath, "utf8"),
  );
  const emittedTaskRouteInputDefinitions = JSON.parse(
    readFileSync(taskRouteInputDefinitionsPath, "utf8"),
  );
  const sourceTaskRouteDefinitions = loadTaskRouteDefinitions(
    join(REPO_ROOT, "runtime", "task-route-definitions.yaml"),
  );
  const sourceTaskRouteInputDefinitions = loadTaskRouteInputDefinitions(
    join(REPO_ROOT, "runtime", "task-route-input-definitions.yaml"),
  );
