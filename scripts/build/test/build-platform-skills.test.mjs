...
describe("buildPlatformSkillsAndCheckZeroEmit", () => {
  it("groups skills by platform when emit=true", () => {
    ...
    const compatMatrix = new Map([
      [
        "a",
        new Map([
          ["claude-code", { status: "supported", emit: true }],
          ["cursor", { status: "excluded", emit: false }],
        ]),
      ],
      [
        "b",
        new Map([
          ["claude-code", { status: "supported", emit: true }],
          ["cursor", { status: "supported", emit: true }],
        ]),
      ],
    ]);

    const { platformSkills, zeroEmitSkills } =
  ...
  it("detects zero-emit non-deprecated skills", () => {
    ...
    const compatMatrix = new Map([
      [
        "orphan",
        new Map([["claude-code", { status: "excluded", emit: false }]]),
      ],
    ]);

    const { zeroEmitSkills } = buildPlatformSkillsAndCheckZeroEmit(
      compatMatrix,
      skillById,
    );
  ...
  it("produces log lines for each skill", () => {
    ...
    const { logLines } = buildPlatformSkillsAndCheckZeroEmit(
      compatMatrix,
      skillById,
    );
