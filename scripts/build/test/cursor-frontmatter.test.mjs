...
`;
  const skill = { skillName: 'my-skill', frontmatter: {} };
  const out = transformSkillMdForCursor(raw, skill, undefined);
  assert.ok(out.includes('user-invocable: false'), 'body note should mention the flag');
  assert.ok(out.includes('Claude Code'), 'note should name Claude Code semantics');
  const fm = out.match(/^---\n([\s\S]*?)\n---\n/);
  assert.ok(fm, 'frontmatter block present');
  assert.ok(!fm[1].includes('user-invocable'), 'user-invocable must not remain in YAML frontmatter');
...
`;
  assert.throws(
    () =>
      transformSkillMdForCursor(raw, { skillName: `${longId}x`, frontmatter: {} }, undefined),
    /exceeds 64/
  );
});
...
`;
  assert.throws(
    () => transformSkillMdForCursor(raw, { skillName: 'Bad_Name', frontmatter: {} }, undefined),
    /pattern/
  );
});
