...
test("adapter-contract: adapter implements required methods", () => {
  ...
  assert.ok(
    typeof adapter.syncTools === "function",
    "Should have syncTools method",
  );
  assert.ok(
    typeof adapter.getTool === "function",
    "Should have getTool method",
  );
  assert.ok(
...
test("adapter-contract: init() is async and returns structured result", async () => {
  ...
  assert.ok(
    resolved.success === true || resolved.success === false,
...
test("adapter-contract: syncTools() returns structured sync result", async () => {
  ...
  assert.ok(
    result.applied === true || result.applied === false,
    "Should have applied field",
  );
...
test("adapter-contract: getTool() returns tool state or null", async () => {
  ...
  assert.ok(
    result === null || (typeof result === "object" && result.name),
    "Should return null or tool",
  );
});
...
test("adapter-contract: cleanup() handles shutdown gracefully", async () => {
  ...
  assert.ok(
    result.cleaned === true || result.cleaned === false,
    "Should have cleaned field",
  );
...
test("adapter-contract: syncTools supports partial tool list", async () => {
  ...
  assert.ok(
    !result.applied === false || result.applied === true,
