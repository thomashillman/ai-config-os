...
function runFetch({ homeDir, workerUrl, token = "test-token" }) {
  return new Promise((resolveFetch) => {
    ...
    proc.on("error", (error) =>
      finish({ status: null, stdout, stderr: error.message }),
    );
...
describe("sync loop fetch contract (etag + version pointer)", () => {
  test(
    "initial fetch stores both payload version and ETag",
    SHELL_TEST_OPTIONS,
    async () => {
      const seen = [];
      await withMockServer(
        ...
        async (baseUrl) => {
          ...
          assert.equal(
            JSON.parse(readFileSync(files.latest, "utf8")).version,
            "1.0.0",
          );
  ...
  test(
    "subsequent request sends If-None-Match and 304 keeps version pointer unchanged",
    SHELL_TEST_OPTIONS,
    async () => {
      let callCount = 0;
      const ifNoneMatch = [];
      ...
      await withMockServer(
        ...
        async (baseUrl) => {
          const homeDir = makeHome();
          assert.equal(
            (await runFetch({ homeDir, workerUrl: baseUrl })).status,
            0,
          );
  ...
  test(
    "200 with new ETag updates payload and version pointer atomically",
    SHELL_TEST_OPTIONS,
    async () => {
      let callCount = 0;
      ...
      await withMockServer(
        (req, res) => {
          ...
          if (callCount === 1) {
            res.setHeader("ETag", '"etag-v1"');
            res.end(
              JSON.stringify({ version: "1.0.0", skills: [{ id: "a" }] }),
            );
          } else {
            res.setHeader("ETag", '"etag-v2"');
            res.end(
              JSON.stringify({ version: "2.0.0", skills: [{ id: "b" }] }),
            );
          }
        },
        async (baseUrl) => {
  ...
  test(
    "failure path does not partially update etag/version state",
    SHELL_TEST_OPTIONS,
    async () => {
      await withMockServer(
        (req, res) => {
        ...
        async (baseUrl) => {
          ...
          assert.equal(readFileSync(files.version, "utf8"), "1.0.0");
          assert.equal(
            existsSync(join(files.cacheDir, "latest.json.tmp")),
            false,
          );
          assert.equal(
            existsSync(join(files.cacheDir, "latest.etag.tmp")),
            false,
          );
          assert.equal(
            existsSync(join(files.cacheDir, "latest.version.tmp")),
            false,
          );
        },
      );
