...
test("Tool Inefficiencies Observation Source", async (t) => {
  ...
  await t.test("setup: import adapter", async () => {
    const mod = await safeImport(
      "../../../runtime/lib/observation-sources/tool-inefficiencies.mjs",
      import.meta.url,
    );
    readToolInefficiencies = mod.readToolInefficiencies;
    assert.ok(
      typeof readToolInefficiencies === "function",
      "should export readToolInefficiencies function",
    );
  });

  await t.test("missing file returns empty array", async () => {
    const observations = await readToolInefficiencies({
      filePath: inefficienciesPath,
    });
    assert.deepEqual(
      observations,
      [],
      "should return empty array when file does not exist",
    );
  });
  ...
  await t.test("reads tool_error events", async () => {
    ...
    assert.equal(
      observations[0].type,
      "tool_error",
      "should preserve event type",
    );
    assert.equal(
      observations[0].tool_name,
      "Bash",
      "should map tool to tool_name",
    );
    assert.equal(
      observations[0].metadata.source_type,
      "tool_inefficiencies",
      "should preserve source type",
    );
    assert.equal(
      observations[0].metadata.snippet,
      "error message",
      "should preserve snippet",
    );
  });
  ...
  await t.test("reads loop_suspected events", async () => {
    ...
    assert.equal(
      observations[0].metadata.call_count,
      15,
      "should preserve call_count",
    );
  });
  ...
  await t.test("preserves all metadata fields", async () => {
    ...
    assert.equal(
      obs.metadata.session_id,
      "abc123",
      "should preserve session_id in metadata",
    );
    assert.equal(
      obs.metadata.source_type,
      "tool_inefficiencies",
      "should mark source_type",
    );
  });
