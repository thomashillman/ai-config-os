...
test("toToolResponse returns envelope on success", async () => {
  const { toToolResponse } =
    await import("../../../runtime/mcp/tool-response.mjs");
...
test("toToolResponse handles error without stdout", async () => {
  ...
  assert.equal(result.isError, true);
  assert.equal(
    parseEnvelope(result).data.diagnostics.raw_output,
    "command failed",
  );
});
...
test("handler unknown tool returns isError true with helpful message", async () => {
  const { createCallToolHandler } =
    await import("../../../runtime/mcp/handlers.mjs");
  const { toToolResponse, toolError } =
    await import("../../../runtime/mcp/tool-response.mjs");
...
test("handler mcp_add with invalid name returns tool error", async () => {
  ...
  const result = await handler({
    params: {
      name: "mcp_add",
      arguments: { name: "!!!invalid!!!", command: "bash" },
    },
  });
...
test("handler mcp_add with unsafe command returns tool error", async () => {
  ...
  const result = await handler({
    params: {
      name: "mcp_add",
      arguments: { name: "demo", command: "../evil.sh" },
    },
  });
...
test("assertRuntimePrereqsWith passes when bash check succeeds", async () => {
  const { assertRuntimePrereqsWith } =
    await import("../../../runtime/mcp/runtime-prereqs.mjs");
...
test("resolveEffectiveOutcomeContract uses runnable script paths in routes", async () => {
  const { resolveEffectiveOutcomeContract } =
    await import("../../../runtime/lib/outcome-resolver.mjs");

  const syncContract = resolveEffectiveOutcomeContract({
    toolName: "sync_tools",
    executionChannel: "mcp",
  });
  assert.equal(syncContract.preferredRoute.id, "runtime/sync.sh");

  const listContract = resolveEffectiveOutcomeContract({
    toolName: "list_tools",
    executionChannel: "mcp",
  });
...
test("resolveEffectiveOutcomeContract keeps route id path-only and places flags in args", async () => {
  ...
  const mcpList = resolveEffectiveOutcomeContract({
    toolName: "mcp_list",
    executionChannel: "mcp",
  });
  ...
  const mcpAdd = resolveEffectiveOutcomeContract({
    toolName: "mcp_add",
    executionChannel: "mcp",
  });
  ...
  const mcpRemove = resolveEffectiveOutcomeContract({
    toolName: "mcp_remove",
    executionChannel: "mcp",
  });
...
test("handler resolve_outcome_contract returns contract JSON", async () => {
  ...
  const result = await handler({
    params: {
      ...
      arguments: { tool_name: "list_tools" },
    },
  });
