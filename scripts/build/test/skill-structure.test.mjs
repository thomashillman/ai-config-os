...
const VALID_CAPABILITY_IDS = new Set([
  'fs.read', 'fs.write',
  'shell.exec', 'shell.long-running',
  'git.read', 'git.write',
  'network.http', 'browser.fetch',
  'mcp.client', 'env.read',
    ...
    for (const { name, dir, frontmatter } of skills) {
      ...
      for (const [model, variant] of Object.entries(variants)) {
        ...
        if (!existsSync(fullPath)) {
          failures.push(`  ${name}: variants.${model}.prompt_file not found: ${variant.prompt_file}`);
        }
      }
    }
    assert.equal(failures.length, 0, `Missing prompt files:\n${failures.join('\n')}`);
  });
    ...
    for (const { name, frontmatter } of skills) {
      ...
      for (const dep of deps) {
        ...
        if (!knownSkillNames.has(dep.name)) {
          failures.push(`  ${name}: dependencies.skills references unknown skill: ${dep.name}`);
        }
      }
    }
    assert.equal(failures.length, 0, `Unknown skill dependencies:\n${failures.join('\n')}`);
  });
