...
test("Step 4.1: Valid pair formation and constraint filtering", async (t) => {
  await t.test("forms valid pairs and filters by quality floor", () => {
    ...
    assert.ok(
      ["standard", "premium"].includes(
        result.execution_selection.resolved_model_path.model_tier,
      ),
    );
  });
  ...
  await t.test("filters by reliability floor", () => {
    ...
    assert.ok(!result.error);
    assert.ok(
      ["above_floor", "high_margin"].includes(
        result.execution_selection.selection_basis.reliability_posture,
      ),
    );
  });
...
test("Step 4.2: Pair-cost derivation", async (t) => {
  await t.test("derives cost deterministically from route and model", () => {
    const route1 = {
      ...sampleRoute,
      effective_capabilities: {
        ...sampleRoute.effective_capabilities,
        artifact_completeness: "repo_complete",
      },
    };
    const route2 = {
  ...
  await t.test(
    "reflects broader scope (lower cost) for fuller artifacts",
    () => {
      const routeFull = {
        ...sampleRoute,
        effective_capabilities: {
      ...
      assert.ok(!result2.error);
      assert.equal(
        result1.execution_selection.resolved_model_path.provider,
        "anthropic",
      );
    },
  );
...
test("Step 4.3: Tie-break order", async (t) => {
  await t.test("breaks ties by evidence depth", () => {
    ...
    assert.equal(
      result.execution_selection.selected_route.effective_capabilities
  ...
  await t.test("breaks ties by reliability margin", () => {
    const input = {
      ...
      model_candidates: [
        {
          ...sampleModel,
          model_id: "model-meets",
          cost_basis: "cost_balanced",
          reliability_margin: "meets_floor",
        },
        {
          ...sampleModel,
          model_id: "model-above",
          cost_basis: "cost_balanced",
          reliability_margin: "above_floor",
        },
        {
    ...
    assert.equal(
      result.execution_selection.resolved_model_path.model_id,
      "model-high",
    );
  });
  ...
  await t.test("breaks ties by latency risk", () => {
    const input = {
      ...
      model_candidates: [
        {
          ...sampleModel,
          model_id: "model-interactive-safe",
        ...
        {
          ...sampleModel,
          model_id: "model-interactive-tolerable",
          cost_basis: "cost_efficient",
          reliability_margin: "high_margin",
