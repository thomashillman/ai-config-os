...
describe('validateSkillPolicy — hook rules', () => {
  ...
  function allExcluded() {
    return Object.fromEntries(
      nonHookPlatforms.map(pid => [pid, { mode: 'excluded', notes: 'No hook surface' }])
    );
  }
  ...
  test('hook skill missing one exclusion → error mentioning that platform', () => {
    ...
    const fm = { type: 'hook', capabilities: { required: [], optional: [] }, platforms };
    const { errors } = validateSkillPolicy(fm, 'my-hook');
    assert.ok(errors.some(e => e.includes('claude-ios')), `Expected error about claude-ios, got: ${errors}`);
  });
  ...
  test('hook skill with a non-hook platform entry but wrong mode → error', () => {
    ...
    const fm = { type: 'hook', capabilities: { required: [], optional: [] }, platforms };
    const { errors } = validateSkillPolicy(fm, 'my-hook');
    assert.ok(errors.some(e => e.includes('cursor')), `Expected error about cursor, got: ${errors}`);
  });

  test('hook skill with no platforms block → error', () => {
    const fm = { type: 'hook', capabilities: { required: [], optional: [] } };
    const { errors } = validateSkillPolicy(fm, 'my-hook');
    assert.ok(errors.some(e => e.includes("'platforms' block")), `Expected platforms-block error, got: ${errors}`);
  });
...
describe('validateSkillPolicy — capability rules', () => {
  ...
  test('overlapping required and optional → error', () => {
    const fm = {
      type: 'prompt',
      capabilities: { required: ['git.read', 'fs.write'], optional: ['git.read'] },
    };
    const { errors } = validateSkillPolicy(fm, 'x');
  ...
  test('unknown platform with knownPlatforms set → error', () => {
    ...
    const { errors } = validateSkillPolicy(fm, 'x', new Set(['claude-code', 'cursor']));
...
describe('validatePlatformPolicy', () => {
  ...
  test('id mismatches filename → error', () => {
    const { errors } = validatePlatformPolicy({ id: 'claude-web' }, 'claude-code');
    assert.ok(errors.some(e => e.includes('claude-web') && e.includes('claude-code')));
  });
