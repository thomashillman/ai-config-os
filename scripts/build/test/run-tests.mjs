...
const DIST_READ_PATTERN =
  /DIST_DIR|['"`]dist\/clients|['"`]dist\/registry|['"`]dist\/runtime/;
  ...
  allTestFiles.map(
    (filePath) =>
      readFile(filePath, "utf8")
        .then((content) => ({
          filePath,
          isDistWriter: DIST_WRITE_PATTERN.test(content),
        }))
...
const { exitCode: pureExitCode, durationMs: pureDurationMs } =
  await runTestGroup(pureTests, parallelism);
...
console.log(
  `[run-tests] Phase 2: ${distTests.length} dist tests (concurrency=1)`,
);
const { exitCode: distExitCode, durationMs: distDurationMs } =
  await runTestGroup(distTests, 1);
