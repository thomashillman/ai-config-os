...
test("portability: detect absolute paths on any platform", () => {
  ...
  assert.equal(
    isAbsolutePath(winAbs) || isAbsolutePath(winAbs2),
    true,
    "Should detect Windows absolute",
  );
  assert.equal(
    isAbsolutePath("relative/path"),
    false,
    "Should detect relative",
  );
});
...
test("portability: normalize line endings", () => {
  ...
  assert.equal(
    normalizedCrlf.split("\n").length,
    3,
    "CRLF should be normalized to LF",
  );
  assert.equal(
    normalizedMixed.split("\n").length,
    3,
    "Mixed should be normalized to LF",
  );
...
test("portability: extract file extension reliably", () => {
  ...
  for (const filename of filenames) {
    ...
    if (typeof ext !== "string") {
      failures.push(
        `  '${filename}': expected string extension, got ${typeof ext}`,
      );
    }
  }
  assert.equal(
    failures.length,
    0,
    `${failures.length} extension type issue(s):\n${failures.join("\n")}`,
  );
});
...
function createEnvGetter(env) {
  return (key) => {
    if (platform() === "win32") {
      ...
      const found = Object.keys(env).find(
