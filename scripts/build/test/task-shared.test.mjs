...
import {
  TaskConflictError,
  TaskNotFoundError,
  summariseFindingsProvenance,
  createReadinessView,
} from "../../../runtime/lib/task-shared.mjs";
...
test("summariseFindingsProvenance counts status occurrences", () => {
  ...
  assert.deepEqual(summariseFindingsProvenance(findings), {
    verified: 2,
    hypothesis: 1,
  });
});
...
test("createReadinessView computes stronger_route_available", () => {
  ...
  assert.equal(
    createReadinessView(onPr, [], upgradeAvailableContract).readiness
  ...
  assert.equal(
    createReadinessView(onPr, [], blockedUpgradeContract).readiness
      .stronger_route_available,
    false,
  );
  ...
  assert.equal(
    createReadinessView(onLocal, [], upgradeAvailableContract).readiness
      .stronger_route_available,
    false,
  );
  ...
  assert.equal(
    createReadinessView(otherType, [], upgradeAvailableContract).readiness
      .stronger_route_available,
    false,
  );
});
...
test("createReadinessView does not claim upgrade when contract route is still degraded", () => {
  ...
  const degradedContract = {
    selected_route: {
      ...
      missing_capabilities: ["local_repo"],
    },
  };
  assert.equal(
    createReadinessView(task, [], degradedContract).readiness
      .stronger_route_available,
    false,
  );
});
