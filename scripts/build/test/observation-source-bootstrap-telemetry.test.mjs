...
function createTempDir() {
  const dir = join(tmpdir(), `bootstrap-telemetry-test-${Date.now()}-${Math.random().toString(36).slice(2)}`);
...
test('loadBootstrapTelemetry: skips malformed JSON lines', async () => {
  ...
  writeFileSync(logFile, `${validLine}\n{not valid json\n${validLine}\n`, 'utf8');

  const events = await loadBootstrapTelemetry({ home: tempDir });

  assert.equal(events.length, 2, 'should skip malformed line and load valid ones');
...
test('loadBootstrapTelemetry: creates unique event_id for each event', async () => {
  ...
  assert.notEqual(events[0].event_id, events[1].event_id, 'event IDs should be unique');
