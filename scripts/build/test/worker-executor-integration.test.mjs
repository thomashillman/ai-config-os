...
function validateContract(payload) {
  ...
  assert.equal(
    result.valid,
    true,
    `response contract mismatch: ${JSON.stringify(result.errors)}`,
  );
}
...
function startWorkerProxy({ executorPort, timeoutMs = 50, outputLimit = 80 }) {
  ...
  const server = http.createServer(async (req, res) => {
    ...
    try {
      const executorRes = await fetch(
        `http://127.0.0.1:${executorPort}/execute`,
        {
          method: "POST",
...
test("worker-executor integration: valid signed request -> allowed tool -> successful response", async () => {
  ...
  const proxy = await startWorkerProxy({
    executorPort: exec.port,
    timeoutMs: 5000,
  });

  try {
    const { response, json } = await sendProxyRequest(
      proxy.port,
      fixtures.valid,
    );
    ...
    proxy.server.close();
    await Promise.all([
      once(exec.server, "close"),
      once(proxy.server, "close"),
    ]);
  }
});
...
function startPhase1ServiceBindingProxy({ executorPort, timeoutMs = 15000 }) {
  const server = http.createServer(async (req, res) => {
    ...
    try {
      const executorRes = await fetch(
        `http://127.0.0.1:${executorPort}/v1/execute`,
        {
          method: "POST",
          headers: {
            "content-type": "application/json",
...
test("Phase 1 service binding: valid health_check request succeeds", async () => {
  ...
  const binding = await startPhase1ServiceBindingProxy({
    executorPort: executor.port,
  });

  try {
    const response = await fetch(
      `http://127.0.0.1:${binding.port}/v1/execute`,
      {
        method: "POST",
        headers: {
          "content-type": "application/json",
    ...
    await Promise.all([
      once(executor.server, "close"),
      once(binding.server, "close"),
    ]);
  }
});
