...
import { resolveSkillPlatform, resolveAll } from '../lib/resolve-compatibility.mjs';
...
test('resolveSkillPlatform: required capability unsupported → excluded', () => {
  ...
  assert.ok(result.unsupported.includes('shell.exec'), 'Should list unsupported capability');
...
test('resolveSkillPlatform: optional capability unsupported → still supported', () => {
  ...
  assert.equal(result.status, 'supported', 'Should be supported (optional does not block)');
  assert.equal(result.emit, true, 'Should emit');
});
...
test('resolveSkillPlatform: unknown capability → unverified, defaults to native', () => {
  ...
  assert.equal(result.emit, false, 'Should not emit (unverified without allow_unverified)');
  assert.ok(result.unknown.includes('unknown.capability'), 'Should list unknown capability');
});
...
test('resolveSkillPlatform: platform override mode: excluded forces exclusion', () => {
  ...
  assert.equal(result.emit, false, 'Should not emit');
  assert.ok(result.notes.includes('hook surface'), 'Should include override note');
});
...
test('resolveSkillPlatform: no capabilities defined → supported', () => {
  ...
  assert.equal(result.status, 'supported', 'Should be supported (no required caps)');
  assert.equal(result.emit, true, 'Should emit');
});
...
test('resolveAll: produces compatibility matrix for multiple skills and platforms', () => {
  ...
  assert.equal(skill1Claude.status, 'supported', 'skill1 should be supported on claude-code');
  ...
  assert.equal(skill2Cursor.status, 'unverified', 'skill2 should be unverified on cursor');
  assert.equal(skill2Cursor.emit, false, 'skill2 should not emit on cursor (unverified)');
});
...
test('resolveSkillPlatform: degraded mode with fallback_mode none → native', () => {
  ...
  assert.equal(result.mode, 'native', 'Should revert to native (degraded not allowed)');
});
...
test('resolveSkillPlatform: multiple unsupported capabilities → all listed in result', () => {
  ...
  assert.ok(result.unsupported.includes('shell.exec'), 'Should include shell.exec');
...
test('resolveSkillPlatform: mixed unknown and unsupported → excluded (unsupported takes precedence)', () => {
  ...
  assert.equal(result.status, 'excluded', 'Should be excluded (unsupported takes precedence)');
  assert.equal(result.emit, false, 'Should not emit');
  assert.ok(result.unsupported.includes('known.unsupported'), 'Should list unsupported');
});
