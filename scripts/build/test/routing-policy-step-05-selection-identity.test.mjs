...
const sampleExecutionSelection = {
  ...
  selection_reason: 'route: local_repo; model: anthropic/claude-sonnet-4-6; cost: cost_balanced; reliability: high_margin',
};
  ...
  await t.test('sorts allowed_task_classes for consistency', () => {
    const sel1 = {
      ...
      selected_route: {
        ...
        effective_capabilities: {
          ...
          allowed_task_classes: ['patch_review', 'repository_review', 'artifact_review'],
        },
      },
    };
    ...
    const sel2 = {
      ...
      selected_route: {
        ...
        effective_capabilities: {
          ...sampleExecutionSelection.selected_route.effective_capabilities,
          allowed_task_classes: ['artifact_review', 'patch_review', 'repository_review'],
        },
      },
    };
    ...
    assert.deepEqual(proj1.selected_route.effective_capabilities.allowed_task_classes, proj2.selected_route.effective_capabilities.allowed_task_classes);
  });
});
...
test('Step 5.2: Digest stability and revision triggers', async t => {
  ...
  await t.test('produces different digest when resolved_model_path changes', () => {
    const modified = {
      ...sampleExecutionSelection,
      resolved_model_path: {
...
test('Step 5.3: Identity change detection', async t => {
  ...
  await t.test('returns false for identical selections', () => {
    assert.ok(!hasIdentityChanged(sampleExecutionSelection, sampleExecutionSelection));
  });
});
...
test('Step 5.4: Identity enrichment', async t => {
  ...
  await t.test('preserves original fields', () => {
    ...
    assert.equal(enriched.selected_route.route_id, sampleExecutionSelection.selected_route.route_id);
    assert.equal(enriched.resolved_model_path.provider, sampleExecutionSelection.resolved_model_path.provider);
  });
});
...
test('Step 5.6: Action allowlist for lightweight references', async t => {
  ...
  await t.test('rejects disallowed actions', () => {
    assert.ok(!isActionAllowedLightweightReference('execution_selection_created'));
    assert.ok(!isActionAllowedLightweightReference('execution_selection_replaced'));
  });
});
