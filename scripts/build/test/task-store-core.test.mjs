...
class StubProgressEventStore {
  ...
  listByTaskId(taskId) { return this.events.filter(e => e.taskId === taskId); }
}

function stubTransitionPortableTaskState({ task, nextState, nextAction, updatedAt, progress }) {
  return {
    ...task,
...
describe('TaskStore.update', () => {
  test('merges changes and increments version', () => {
    const store = makeTaskStore();
    store.create(makeTask());
    const result = store.update('task-001', { expectedVersion: 1, changes: { goal: 'Updated' } });
...
describe('TaskStore.transitionState', () => {
  test('transitions state via stub and emits state_change progress event', () => {
    ...
    assert.ok(events.some(e => e.type === 'state_change'), 'should emit state_change event');
  });
  ...
  test('throws TaskConflictError on version mismatch', () => {
    ...
    store.create(makeTask());
    assert.throws(
      () => store.transitionState('task-001', { expectedVersion: 99, nextState: 'complete' }),
      TaskConflictError
    );
  });
  ...
  test('throws TaskNotFoundError for unknown taskId', () => {
    const store = makeTaskStore();
    assert.throws(
      () => store.transitionState('nonexistent', { expectedVersion: 1, nextState: 'complete' }),
      TaskNotFoundError
    );
  });
});
...
describe('TaskStore.appendFinding', () => {
  ...
  test('throws TaskConflictError on version mismatch', () => {
    ...
    store.create(makeTask());
    assert.throws(
      () => store.appendFinding('task-001', { expectedVersion: 99, finding: {}, updatedAt: '' }),
      TaskConflictError
    );
  });
  ...
  test('throws TaskNotFoundError for unknown taskId', () => {
    const store = makeTaskStore();
    assert.throws(
      () => store.appendFinding('nonexistent', { expectedVersion: 1, finding: {}, updatedAt: '' }),
      TaskNotFoundError
    );
  });
});
...
describe('TaskStore.selectRoute', () => {
  ...
  test('throws TaskConflictError on version mismatch', () => {
    ...
    store.create(makeTask());
    assert.throws(
      () => store.selectRoute('task-001', { routeId: 'x', expectedVersion: 99, selectedAt: '' }),
      TaskConflictError
    );
  });
  ...
  test('throws TaskNotFoundError for unknown taskId', () => {
    const store = makeTaskStore();
    assert.throws(
      () => store.selectRoute('nonexistent', { routeId: 'x', expectedVersion: 1, selectedAt: '' }),
      TaskNotFoundError
    );
  });
});
...
describe('TaskStore.listProgressEvents', () => {
  ...
  test('throws TaskNotFoundError for unknown taskId', () => {
    const store = makeTaskStore();
    assert.throws(() => store.listProgressEvents('nonexistent'), TaskNotFoundError);
  });
  ...
  test('returns events scoped to the correct task', () => {
    ...
    store.transitionState('task-A', { expectedVersion: 1, nextState: 'paused', updatedAt: '' });
...
describe('maxSnapshots', () => {
  test('caps snapshot count to maxSnapshots, keeping most recent', () => {
    ...
    assert.ok(!snapshots.some(s => s.snapshot_version === 1), 'oldest snapshot should be evicted');
  });
