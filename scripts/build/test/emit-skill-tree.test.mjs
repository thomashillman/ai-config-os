...
test('emitSkillFolder writes transformed SKILL.md and copies prompts', () => {
  ...
  try {
    ...
    emitSkillFolder({
      ...
      transformSkillMd: (raw, s) => raw.replace(/^---\n/, `---\nname: ${s.skillName}\n`),
    });

    const outMd = readFileSync(join(distSkills, 'demo-skill', 'SKILL.md'), 'utf8');
    assert.ok(outMd.includes('name: demo-skill'));
    assert.ok(existsSync(join(distSkills, 'demo-skill', 'prompts', 'extra.md')));
  } finally {
