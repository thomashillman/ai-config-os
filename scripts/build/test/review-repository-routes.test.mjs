...
test('runtime loader overrides are supported for deterministic tests and integration wiring', () => {
  setReviewRepositoryRouteRuntimeLoaders({
    routeDefinitionsLoader: () => ({
      taskTypes: {
        review_repository: {
          routes: [{ route_id: 'pasted_diff', required_capabilities: [], equivalence_level: 'degraded' }],
        },
      },
    }),
  ...
  try {
    assert.deepEqual(getReviewRepositoryRoutes().map((route) => route.route_id), ['pasted_diff']);
    assert.deepEqual(getRequiredInputsForReviewRepositoryRoute('pasted_diff'), ['diff_text']);
  } finally {
    resetReviewRepositoryRouteRuntimeLoaders();
  }
});
...
test('getRequiredInputsForReviewRepositoryRoute rejects routes not declared in canonical route definitions', () => {
  setReviewRepositoryRouteRuntimeLoaders({
    routeDefinitionsLoader: () => ({
      taskTypes: {
        review_repository: {
          routes: [{ route_id: 'local_repo', required_capabilities: [], equivalence_level: 'equal' }],
        },
      },
    }),
    routeInputDefinitionsLoader: () => ({
