...
test("T015 startReviewRepositoryTask creates active task with deterministic weak route", () => {
  ...
  assert.equal(
    started.effective_execution_contract.selected_route.route_id,
    "github_pr",
  );
  ...
  assert.ok(
    events.some(
      (event) =>
        event.type === "state_change" && event.metadata?.phase === "start",
    ),
  );
});
...
test("T016 resumeReviewRepositoryTask upgrades to local_repo and transitions findings provenance", () => {
  ...
  assert.ok(
    events.some(
      (event) =>
        event.type === "route_selected" && event.metadata?.phase === "resume",
    ),
  );
});
...
test("T017 buildTaskReadinessView exposes readiness, route history, and provenance summary", () => {
  ...
  assert.match(
    readiness.readiness.upgrade_unavailable_reason,
