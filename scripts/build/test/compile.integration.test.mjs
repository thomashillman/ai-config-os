...
test("compile.mjs --validate-only exits 0 against real skills", () => {
  ...
  assert.equal(
    ...
    0,
    `Compiler exited with status ${compileResult.status}`,
  );
...
test("compile.mjs --validate-only reports skills and platforms loaded", () => {
  ...
  assert.ok(
    compileResult.stdout.includes("Validated:"),
    'Expected "Validated:" summary line',
  );
  assert.ok(
    compileResult.stdout.includes("Loaded"),
    'Expected "Loaded N platform(s)" line',
  );
});
