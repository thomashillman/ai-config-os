...
test('Manifest Update Utility', async (t) => {
  await t.test('extractDescription: should extract first line from description pipe', () => {
    const skillContent = `---
skill: test-skill
description: |
...
Body content...`;
  ...
  await t.test('extractDescription: should return fallback if no description field', () => {
    const skillContent = `---
skill: test-skill
type: prompt
status: stable
...
`;
      ...
      assert(
        updated.includes('| \`new-skill\` | A brand new skill | \`shared/skills/new-skill/SKILL.md\` |'),
      ...
      assert(updated.includes('existing-skill'), 'Existing skill should remain');
    } finally {
      rmSync(tmpDir, { recursive: true, force: true });
    }
  });

  await t.test('updateManifestWithSkill: should preserve manifest structure', () => {
    const tmpDir = mkdtempSync(join(tmpdir(), 'manifest-test-'));
...
`;
      ...
      assert(updated.includes('## Workflows'), 'Workflows heading should exist');
      assert(updated.includes('Content after workflows'), 'Content after workflows should exist');
    } finally {
      rmSync(tmpDir, { recursive: true, force: true });
    }
  });

  await t.test('updateManifestWithSkill: should fail if ## Skills heading missing', () => {
    const tmpDir = mkdtempSync(join(tmpdir(), 'manifest-test-'));
...
`;
  ...
  await t.test('updateManifestWithSkill: should fail if table header malformed', () => {
    const tmpDir = mkdtempSync(join(tmpdir(), 'manifest-test-'));
