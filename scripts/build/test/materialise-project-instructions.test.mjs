...
const SCRIPT_PATH = join(
  ...
  "scripts",
  "build",
  "materialise-project-instructions.mjs",
);
...
describe("materialise-project-instructions CLI", () => {
  test("writes CLAUDE.md and AGENTS.md using overlay directory", () => {
    ...
    try {
      ...
      assert.ok(
        firstBaseHeading,
        "expected doctrine base to include a heading",
      );
      assert.match(
        claude,
        new RegExp(
          `^${firstBaseHeading.replace(/[.*+?^${}()|[\]\\]/g, "\\$&")}`,
          "m",
        ),
      );
  ...
  test("supports repeatable --overlay-file entries", () => {
    const target = mkdtempSync(
      join(tmpdir(), "proj-instructions-overlay-file-"),
    );
    const baseFile = join(target, "overlay-base.md");
  ...
  test("does not include ai-config-os local rules unless explicitly overlayed", () => {
    const target = mkdtempSync(
      join(tmpdir(), "proj-instructions-local-rules-"),
    );
    ...
    try {
      ...
      const claudeWithoutOverlay = readFileSync(
        join(target, "CLAUDE.md"),
        "utf8",
      );
      const agentsWithoutOverlay = readFileSync(
  ...
  test("fails fast when an option value is missing", () => {
    const target = mkdtempSync(
      join(tmpdir(), "proj-instructions-missing-value-"),
    );
