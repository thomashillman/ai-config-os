...
test('maps user_response progress event to observation response', () => {
  ...
  assert.equal(observation.metadata.narration_event_id, 'evt_1_narration_onstart_1234567890');
...
test('filters out unknown event types', () => {
  ...
  const observations = mapMomentumProgressEvents([narrationEvent, unknownEvent]);
...
test('handles follow_up response with text', () => {
  ...
  assert.equal(observation.metadata.follow_up_text, 'What about security implications?');
});
