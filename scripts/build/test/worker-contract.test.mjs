...
const capabilitySchema = loadJson(
  resolve(REPO_ROOT, "shared/contracts/capability.schema.json"),
);
const envelopeSchema = loadJson(
  resolve(REPO_ROOT, "shared/contracts/response-envelope.schema.json"),
);
...
async function createTask(worker, task, requestEnv = env) {
  const response = await worker.fetch(
    makeAuthorizedJsonRequest("POST", "/v1/tasks", task),
    requestEnv,
  );
  assert.equal(
    response.status,
    201,
    `expected task ${task.task_id} to be created`,
  );
...
const env = {
  ...
  ENVIRONMENT: "test",
  HANDOFF_TOKEN_SIGNING_KEY: "test-signing-key",
};

async function makeSignedHandoffToken({
  tokenId,
  taskId,
  issuedAt,
  expiresAt,
  ...
  const { canonicalHandoffTokenPayload, signCanonicalHandoffTokenPayload } =
    await import(HANDOFF_SERVICE_FILE_URL);
...
describe("worker endpoint contract", () => {
  test("health/manifest/client/skill versions all match dist registry version", async () => {
    ...
    const clientLatest = await responseJson(
      worker,
      "/v1/client/claude-code/latest",
    );
    const skillId = registry.skills[0]?.id;
    assert.ok(
      skillId,
      "registry must include at least one skill for /v1/skill/:id test",
    );
  ...
  test("client payload artifact metadata aligns with registry skill entries", async () => {
    ...
    assert.deepEqual(
      clientLatest.body.plugin_json,
      plugin,
      "client payload plugin_json must equal emitted plugin",
    );

    const registrySkillsById = new Map(
      registry.skills.map((skill) => [skill.id, skill]),
    );
    for (const pluginSkill of clientLatest.body.plugin_json.skills) {
      const registrySkill = registrySkillsById.get(pluginSkill.name);
      assert.ok(
        registrySkill,
        `plugin skill '${pluginSkill.name}' must exist in registry.skills`,
      );
      assert.equal(
        pluginSkill.version,
  ...
  test("unknown route/client/skill errors stay structured and never expose version fields", async () => {
    ...
    for (const sample of [unknownRoute, unknownClient, unknownSkill]) {
      ...
      assert.ok(
        !("version" in sample.body),
        "error payload must not include version field",
      );
    }
  });
  ...
  test("regression: actionable failure when registry/plugin fixture versions diverge", async () => {
    const registry = loadJson(REGISTRY_PATH);
    const plugin = loadJson(PLUGIN_PATH);
    const worker = await loadWorkerWithFixtures(registry, {
      ...plugin,
      version: "0.0.0-fixture-mismatch",
    });
  ...
  test("manifest/artifact endpoints resolve through version pointers when storage bindings exist", async () => {
    ...
    const manifest = await responseJsonWithEnv(
      worker,
      "/v1/manifest/latest",
      envWithStorage,
    );
    const outcomes = await responseJsonWithEnv(
      worker,
      "/v1/outcomes/latest",
      envWithStorage,
    );
    const routes = await responseJsonWithEnv(
      worker,
      "/v1/routes/latest",
      envWithStorage,
    );
    const tools = await responseJsonWithEnv(
      worker,
      "/v1/tools/latest",
      envWithStorage,
    );
  ...
  test("task control plane endpoints cover success and deterministic continuation replay", async () => {
    ...
    const getRes = await worker.fetch(
      makeAuthorizedRequest(`/v1/tasks/${task.task_id}`),
      env,
    );
    assert.equal(getRes.status, 200);

    const transitionRes = await worker.fetch(
      makeAuthorizedJsonRequest("PATCH", `/v1/tasks/${task.task_id}/state`, {
        expected_version: 1,
        next_state: "active",
        next_action: "collect_more_context",
    ...
    const routeSelectionRes = await worker.fetch(
      makeAuthorizedJsonRequest(
        "POST",
        `/v1/tasks/${task.task_id}/route-selection`,
        {
          expected_version: 2,
          route_id: "local_repo",
    ...
    const answerRes = await worker.fetch(
      makeAuthorizedJsonRequest(
        "POST",
        `/v1/tasks/${task.task_id}/questions/q-1/answer`,
        {
          expected_version: 3,
    ...
    const dismissRes = await worker.fetch(
      makeAuthorizedJsonRequest(
        "POST",
        `/v1/tasks/${task.task_id}/questions/q-2/dismiss`,
        {
          expected_version: 4,
    ...
    const routesRes = await worker.fetch(
      makeAuthorizedRequest(`/v1/tasks/${task.task_id}/available-routes`),
      env,
    );
    ...
    const continuationRes1 = await worker.fetch(
      makeAuthorizedJsonRequest(
        "POST",
        `/v1/tasks/${task.task_id}/continuation`,
        continuationReqBody,
      ),
      env,
    );
    ...
    assert.equal(
      continuationBody1.data.continuation_package.handoff_token_id,
      "handoff_001",
    );
    assert.equal(
      continuationBody1.data.continuation_package.created_at,
      "2026-03-12T12:04:00.000Z",
    );

    const continuationRes2 = await worker.fetch(
      makeAuthorizedJsonRequest(
        "POST",
        `/v1/tasks/${task.task_id}/continuation`,
        continuationReqBody,
    ...
    const progressEventsRes = await worker.fetch(
      makeAuthorizedRequest(`/v1/tasks/${task.task_id}/progress-events`),
      env,
    );
    ...
    const readinessRes = await worker.fetch(
      makeAuthorizedRequest(`/v1/tasks/${task.task_id}/readiness`),
      env,
    );
    ...
    const snapshotsRes = await worker.fetch(
      makeAuthorizedRequest(`/v1/tasks/${task.task_id}/snapshots`),
      env,
    );
    ...
    const snapshotVersion2Res = await worker.fetch(
      makeAuthorizedRequest(`/v1/tasks/${task.task_id}/snapshots/2`),
      env,
    );
  ...
  test("task list endpoint rejects invalid query params and caps large limits", async () => {
    ...
    const cappedResponse = await worker.fetch(
      makeAuthorizedRequest("/v1/tasks?limit=500"),
      envWithKv,
    );
  ...
  test("task endpoint failures: malformed payloads, auth, not found, conflict and token errors", async () => {
    ...
    const unauthorized = await worker.fetch(
      new Request("https://example.test/v1/tasks", { method: "POST" }),
      env,
    );
    assert.equal(unauthorized.status, 401);

    const missingSigningKeyRes = await worker.fetch(
      makeAuthorizedJsonRequest(
        "POST",
        "/v1/tasks/task_missing_key_001/continuation",
        {
          handoff_token: {
            schema_version: "1.0.0",
    ...
    const malformed = await worker.fetch(
      makeAuthorizedJsonRequest(
        "POST",
        "/v1/tasks/task_review_repository_001/route-selection",
        {
          expected_version: "x",
          route_id: "local_repo",
          selected_at: "2026-03-12T12:02:00.000Z",
    ...
    const duplicateCreateRes = await worker.fetch(
      makeAuthorizedJsonRequest("POST", "/v1/tasks", task),
      env,
    );
    assert.equal(duplicateCreateRes.status, 409);

    const staleRouteSelection = await worker.fetch(
      makeAuthorizedJsonRequest(
        "POST",
        `/v1/tasks/${task.task_id}/route-selection`,
        {
    ...
    const missingTask = await worker.fetch(
      makeAuthorizedRequest("/v1/tasks/missing_task_001"),
      env,
    );
    assert.equal(missingTask.status, 404);

    const missingTaskReadiness = await worker.fetch(
      makeAuthorizedRequest("/v1/tasks/missing_task_001/readiness"),
      env,
    );
    ...
    const invalidTokenRes = await worker.fetch(
      makeAuthorizedJsonRequest(
        "POST",
        `/v1/tasks/${task.task_id}/continuation`,
        {
    ...
    const expiredTokenRes = await worker.fetch(
      makeAuthorizedJsonRequest(
        "POST",
        `/v1/tasks/${task.task_id}/continuation`,
        {
    ...
    const firstReplayUse = await worker.fetch(
      makeAuthorizedJsonRequest(
        "POST",
        `/v1/tasks/${task.task_id}/continuation`,
        {
    ...
    const replayConflict = await worker.fetch(
      makeAuthorizedJsonRequest(
        "POST",
        `/v1/tasks/${task.task_id}/continuation`,
        {
  ...
  test("runtime.capabilities returns canonical contract envelope describing surface", async () => {
    ...
    const res = await worker.fetch(
      makeAuthorizedRequest("/v1/runtime/capabilities"),
      env,
    );
    assert.equal(res.status, 200);
  ...
  test("contract schema validation: tasks.list, tasks.get, tasks.events, tasks.available_routes all pass envelope schema", async () => {
    ...
    const getRes = await worker.fetch(
      makeAuthorizedRequest(`/v1/tasks/${taskId}`),
      env,
    );
    assert.equal(getRes.status, 200);
    ...
    const eventsRes = await worker.fetch(
      makeAuthorizedRequest(`/v1/tasks/${taskId}/progress-events`),
      env,
    );
    ...
    const routesRes = await worker.fetch(
      makeAuthorizedRequest(`/v1/tasks/${taskId}/available-routes`),
      env,
    );
    assert.equal(routesRes.status, 200);
  ...
  test("golden path: agent discovers surface, lists tasks, drills into detail, gets route advice", async () => {
    ...
    assert.ok(
      capBody.data.available_resources.includes("tasks.list"),
    ...
    const createRes = await worker.fetch(
      makeAuthorizedJsonRequest("POST", "/v1/tasks", newTask),
      env,
    );
    const createBody = await createRes.json();
    const taskId = createBody.data.task.task_id;
    ...
    assert.ok(
      "urgency" in getBody.meta,
      "meta.urgency must be present for agent decision-making",
    );
    assert.ok(
      "best_next_route" in getBody.meta,
      "meta.best_next_route must be present for agent decision-making",
    );
    ...
    assert.ok(
      typeof routesBody.data.best_next_route === "string" ||
        routesBody.data.best_next_route === null,
    );
