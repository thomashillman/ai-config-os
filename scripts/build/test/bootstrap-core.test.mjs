...
const { resolveProviderContext } = await safeImport(
  "../../../adapters/bootstrap/provider-context.mjs",
  import.meta.url,
);
const { executeBootstrap } = await safeImport(
  "../../../adapters/bootstrap/core.mjs",
  import.meta.url,
);

function createRunner(resultsByPhase) {
  return (command) => {
    const phase = Object.entries(resultsByPhase).find(([, matcher]) =>
      command.join(" ").includes(matcher.match),
    );
    if (!phase) {
...
describe("bootstrap provider context", () => {
  test("maps Claude remote to a fully-capable adapter context", () => {
    ...
    assert.match(
      context.commands.remote_install.join(" "),
      /materialise\.sh bootstrap/,
    );
  });
...
describe("bootstrap core execution", () => {
  test("runs only install-critical work inline and defers non-critical work by default", () => {
    ...
    assert.ok(
      events.some(
        (event) =>
          event.phase === "acquire_remote_bundle" && event.result === "ok",
      ),
    );
    assert.equal(
      events.some(
        (event) => event.phase === "validate" && event.deferred === false,
      ),
      false,
    );
  });
  ...
  test("falls back to local materialization when remote install fails", () => {
    ...
    assert.ok(
      result.errors.some((error) => error.code === "REMOTE_BUNDLE_FAILED"),
    );
    assert.ok(
      events.some(
        (event) => event.phase === "materialize_local" && event.result === "ok",
      ),
    );
    assert.ok(
      events.some(
        (event) => event.phase === "install_target" && event.result === "ok",
      ),
    );
  });
  ...
  test("returns non-zero only when the install path fails", () => {
    ...
    assert.ok(
      result.errors.some((error) => error.code === "INSTALL_TARGET_FAILED"),
    );
  });
  ...
  test("strict mode runs deferred phases inline and preserves schema invariants", () => {
    ...
    assert.ok(
      events.some(
        (event) => event.phase === "probe_runtime" && event.result === "ok",
      ),
    );
  ...
  test("skips deferred phases when their required capability is disabled", () => {
    ...
    assert.ok(
      events.some(
        (event) => event.phase === "validate" && event.result === "skipped",
      ),
    );
  });
  ...
  test("writes telemetry events to the bootstrap JSONL log by default", () => {
    ...
    try {
      ...
      assert.equal(exitCode, 0);
      assert.ok(
        result.telemetry_path,
        "telemetry path should be reported in the result",
      );
      assert.ok(
        existsSync(result.telemetry_path),
        "telemetry log should be created",
      );
