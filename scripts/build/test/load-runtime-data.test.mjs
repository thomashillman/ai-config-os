...
test("loadRouteDefinitions returns object with taskTypes", () => {
  ...
  assert.ok(
    typeof result.taskTypes === "object",
    "taskTypes should be an object",
  );
...
test("loadRouteDefinitions parity: matches runtime loadTaskRouteDefinitions", async () => {
  const { loadTaskRouteDefinitions } = await import(
    new URL(
      "../../../runtime/lib/task-route-definition-loader.mjs",
      import.meta.url,
    ).href
  );
...
test("loadRouteInputDefinitions parity: matches runtime loadTaskRouteInputDefinitions", async () => {
  const { loadTaskRouteInputDefinitions } = await import(
    new URL("../../../runtime/lib/task-route-input-loader.mjs", import.meta.url)
      .href
  );
  const runtimeResult = loadTaskRouteInputDefinitions(
    TASK_ROUTE_INPUT_DEFINITIONS_PATH,
  );
  const buildResult = loadRouteInputDefinitions(
    TASK_ROUTE_INPUT_DEFINITIONS_PATH,
  );
