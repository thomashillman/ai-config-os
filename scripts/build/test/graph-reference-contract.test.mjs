...
function validateGraphReferences(graph, sourceLabel) {
  const routes = [...(graph.routes ?? [])].sort((a, b) =>
    a.id.localeCompare(b.id),
  );
  const outcomes = [...(graph.outcomes ?? [])].sort((a, b) =>
    a.id.localeCompare(b.id),
  );
  const tools = [...(graph.tools ?? [])].sort((a, b) =>
    a.id.localeCompare(b.id),
  );
  const skills = [...(graph.skills ?? [])].sort((a, b) =>
    a.id.localeCompare(b.id),
  );
  ...
  for (const route of routes) {
    assert.ok(
      route.outcomeId,
      `${sourceLabel}: routes.${route.id}.outcomeId missing`,
    );
    assert.ok(
...
function loadWorkflowGraph(skillIds, toolIds) {
  ...
  for (const file of files) {
    ...
    const steps = Array.isArray(workflow.execution_flow)
...
describe("graph reference contract — live artefacts and runtime modules", () => {
  test("registry, plugin, workflows, and runtime route/outcome definitions resolve with no dangling IDs", () => {
    ...
    const registry = readJson(
      join(REPO_ROOT, "dist", "registry", "index.json"),
    );
    ...
    const registrySkillIds = (registry.skills ?? [])
      .map((skill) => skill.id)
      .sort();
    const pluginSkillIds = (plugin.skills ?? [])
      .map((skill) => skill.name)
      .sort();
    const toolIds = parseToolRegistryIds(
      join(REPO_ROOT, "runtime", "tool-registry.yaml"),
    );
    ...
    assert.ok(
      toolIds.length > 0,
      "Expected runtime/tool-registry.yaml to declare at least one tool",
    );

    const workflowGraph = loadWorkflowGraph(registrySkillIds, toolIds);
    assert.ok(
      workflowGraph,
      "Expected at least one workflow graph to validate",
    );
        ...
        validateGraphReferences(
          {
            ...
            outcomes: [
              { id: "o1", refs: { tools: ["missing-tool"], skills: [] } },
            ],
        ...
        validateGraphReferences(
          {
            routes: [{ id: "r1", outcomeId: "o1" }],
            outcomes: [
              { id: "o1", refs: { tools: [], skills: ["missing-skill"] } },
            ],
            tools: [{ id: "t1" }],
