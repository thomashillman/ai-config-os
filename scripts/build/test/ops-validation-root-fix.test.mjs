...
describe("ops generate-docs strict mode", () => {
  test("generate-docs fails when required metadata is missing", () => {
    ...
    try {
      ...
      const result = spawnSync(
        "node",
        [
          join(REPO_ROOT, "scripts", "build", "generate-docs.mjs"),
          "--repo-root",
          repoPath,
        ],
        { encoding: "utf8" },
  ...
  test("generate-docs succeeds and keeps multiline description content", () => {
    ...
    try {
      ...
      mkdirSync(join(repoPath, "shared", "skills", "docs-ok", "prompts"), {
        recursive: true,
      });
      writeFileSync(
        join(repoPath, "shared", "skills", "docs-ok", "prompts", "balanced.md"),
        "prompt",
      );
      ...
      const readme = readFileSync(
        join(repoPath, "shared", "skills", "docs-ok", "README.md"),
        "utf8",
      );
  ...
  test(
    ...
    () => {
      ...
      try {
        ...
        writeFileSync(
          join(fixture, "ops", "test-skills.sh"),
          "#!/usr/bin/env bash\nset -euo pipefail\nexit 1\n",
        );
        writeFileSync(
        ...
        assert.notEqual(
          result.status,
          0,
          "expected non-zero exit with failing checks",
        );
