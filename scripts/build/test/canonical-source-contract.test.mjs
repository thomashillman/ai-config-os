...
test("Canonical Source Contract", async (t) => {
  await t.test("should read skill definitions only from shared/skills/", () => {
    ...
    const sharedSkills = readdirSync(SKILLS_DIR).filter(
      (f) => !f.startsWith("."),
    );
    assert(
      sharedSkills.length > 0,
      "shared/skills/ must contain at least one skill",
    );
  });
  ...
  await t.test("should have shared/skills/ as the only source of truth", () => {
    ...
    for (const skillName of sharedSkills) {
      ...
      if (!existsSync(skillPath)) {
        failures.push(
          `  ${skillName}: missing SKILL.md in shared/skills/${skillName}/`,
        );
      }
    }
    assert(
      failures.length === 0,
      `${failures.length} skill(s) missing SKILL.md:\n${failures.join("\n")}`,
    );
  });

  await t.test(
    "should not enumerate skills from plugins/core-skills/skills/",
    () => {
      ...
      const pluginsSkills = readdirSync(PLUGINS_SKILLS_DIR).filter(
        (f) => !f.startsWith("."),
      );
  ...
  await t.test("should have consistent skill names in shared/skills/", () => {
    ...
    for (const skill of skills) {
      ...
      if (skillsByName[lower]) {
        failures.push(
          `  '${skill}' conflicts with '${skillsByName[lower]}' (case-insensitive duplicate)`,
        );
      } else {
    ...
    assert(
      failures.length === 0,
      `${failures.length} duplicate skill name(s):\n${failures.join("\n")}`,
    );
  });
});
