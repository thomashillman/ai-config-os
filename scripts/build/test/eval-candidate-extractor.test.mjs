...
test("extractEvalCandidates counts signal types", () => {
  ...
  const capGapCandidate = candidates.find(
    (c) => c.signal_type === "capability_gap",
  );
...
test("extractEvalCandidates filters out single-occurrence signals", () => {
  ...
  const assumptionCandidate = candidates.find(
    (c) => c.signal_type === "assumption_failure",
  );
...
test("extractEvalCandidates identifies skill recommendation gaps", () => {
  ...
  const gapCandidate = candidates.find(
    (c) => c.signal_type === "capability_gap",
  );
