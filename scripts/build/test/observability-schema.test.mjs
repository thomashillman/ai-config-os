...
const FORBIDDEN_FIELD_NAMES = new Set([
  ...
  'credential', 'credentials', 'api_key', 'apikey', 'private_key',
  'privatekey', 'auth',
]);
...
function validatePhase(p) {
  ...
  return { ok: true, value: { phase: p.phase, result: p.result, duration_ms: p.duration_ms, ...(p.error_code !== undefined ? { error_code: p.error_code } : {}) } };
}
function validateBootstrapRun(payload) {
  if (!isObject(payload)) return { ok: false, error: 'Payload must be a JSON object' };
  const forbidden = hasForbiddenField(payload);
  if (forbidden !== null) return { ok: false, error: `Field '${forbidden}' is not permitted in a run record` };
  ...
  const optionalStrings = ['project_dir','install_root','repo_revision','expected_version','observed_version','first_failed_phase','error_code'];
  for (const field of optionalStrings) {
    const v = payload[field];
    if (v !== undefined) {
      if (typeof v !== 'string') return { ok: false, error: `Field '${field}' must be a string` };
      if (v.length > MAX_MESSAGE_LENGTH) return { ok: false, error: `Field '${field}' exceeds ${MAX_MESSAGE_LENGTH} characters` };
    }
  }
  ...
  for (let i = 0; i < payload.phases.length; i++) {
    ...
    if (!result.ok) return { ok: false, error: `phases[${i}]: ${result.error}` };
    phases.push(result.value);
  }
  if (payload.evidence_refs !== undefined) {
    if (!Array.isArray(payload.evidence_refs)) return { ok: false, error: "Field 'evidence_refs' must be an array" };
    for (let i = 0; i < payload.evidence_refs.length; i++) {
      if (typeof payload.evidence_refs[i] !== 'string') return { ok: false, error: `evidence_refs[${i}] must be a string` };
    }
  }
  const run = { run_id: payload.run_id, started_at: payload.started_at, status: payload.status, phases };
  ...
  if (Array.isArray(payload.evidence_refs)) run.evidence_refs = payload.evidence_refs;
...
test('validateBootstrapRun: rejects run_id over 128 chars', () => {
  const result = validateBootstrapRun({ ...VALID_RUN, run_id: 'x'.repeat(129) });
  assert.equal(result.ok, false);
  assert.match(result.error, /run_id/);
});

test('validateBootstrapRun: rejects invalid started_at', () => {
  const result = validateBootstrapRun({ ...VALID_RUN, started_at: 'not-a-date' });
  assert.equal(result.ok, false);
  assert.match(result.error, /started_at/);
});

test('validateBootstrapRun: rejects invalid finished_at', () => {
  const result = validateBootstrapRun({ ...VALID_RUN, finished_at: 'bad-date' });
  assert.equal(result.ok, false);
...
test('validateBootstrapRun: rejects forbidden field "authorization"', () => {
  const result = validateBootstrapRun({ ...VALID_RUN, authorization: 'Bearer xyz' });
  assert.equal(result.ok, false);
...
test('validateBootstrapRun: rejects overlong string fields', () => {
  ...
  for (const field of ['project_dir', 'install_root', 'repo_revision', 'expected_version', 'observed_version', 'first_failed_phase', 'error_code']) {
    const result = validateBootstrapRun({ ...VALID_RUN, [field]: longStr });
    assert.equal(result.ok, false, `Field '${field}' should be rejected when too long`);
...
test('validateBootstrapRun: phase with error_code is preserved', () => {
  const result = validateBootstrapRun({
    ...VALID_RUN,
    phases: [{ phase: 'worker_package_fetch', result: 'error', duration_ms: 100, error_code: 'WORKER_PACKAGE_NOT_PUBLISHED' }],
  });
  assert.equal(result.ok, true);
  assert.equal(result.value.phases[0].error_code, 'WORKER_PACKAGE_NOT_PUBLISHED');
});
