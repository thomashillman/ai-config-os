...
test("reflector produces template effectiveness insight when engagement differs", () => {
  ...
  const templateInsight = result.report.insights.find(
    (i) => i.type === "template_effectiveness",
  );
...
test("reflector produces upgrade acceptance insight", () => {
  const observations = [
    ...
    observationFixture({
      narrationPoint: "onUpgradeAvailable",
      responseType: "declined_upgrade",
    }),
  ];

  const result = reflect({ observations });

  const upgradeInsight = result.report.insights.find(
    (i) => i.type === "upgrade_acceptance",
  );
...
test("reflector produces intent coverage gap insight", () => {
  const observations = [
    observationFixture({
      ...
      followUpText: "scan this project",
    }),
    observationFixture({
      narrationPoint: "onStart",
      responseType: "follow_up",
      followUpText: "look at my code",
    }),
  ];

  const result = reflect({ observations });

  const intentInsight = result.report.insights.find(
    (i) => i.type === "intent_coverage",
  );
...
test("reflector produces response time insight", () => {
  ...
  const timeInsight = result.report.insights.find(
    (i) => i.type === "response_time",
  );
  assert.ok(timeInsight, "should produce response time insight");
  assert.ok(
    timeInsight.evidence.fastest_median_ms <
      timeInsight.evidence.slowest_median_ms,
  );
});
...
test("insight confidence scores are within [0, 1]", () => {
  ...
  for (const insight of result.report.insights) {
    if (insight.suggestion?.confidence !== undefined) {
      assert.ok(
        insight.suggestion.confidence >= 0,
        `confidence ${insight.suggestion.confidence} should be >= 0`,
      );
      assert.ok(
        insight.suggestion.confidence <= 1,
        `confidence ${insight.suggestion.confidence} should be <= 1`,
      );
    }
  }
});
