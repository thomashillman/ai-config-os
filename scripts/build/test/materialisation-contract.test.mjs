  ...
  await t.test('should have all skills in dist/clients/claude-code/skills/', () => {
    ...
    for (const skill of manifest.skills) {
      ...
      if (!existsSync(skillFilePath)) {
        failures.push(`  ${skill.name}: path not found in dist/: ${skill.path}`);
      }
      if (!skill.path.startsWith('skills/')) {
        failures.push(`  ${skill.name}: path must be relative to dist root (got '${skill.path}')`);
      }
    }
    assert.equal(failures.length, 0, `${failures.length} materialisation issue(s):\n${failures.join('\n')}`);
  });
  ...
  await t.test('should use only relative paths in plugin.json (no source references)', () => {
    ...
    for (const skill of manifest.skills) {
      if (skill.path.includes('shared/skills')) {
        failures.push(`  ${skill.name}: path must not reference shared/skills/ (got '${skill.path}')`);
      }
      if (skill.path.startsWith('/')) {
        failures.push(`  ${skill.name}: path must be relative, not absolute (got '${skill.path}')`);
      }
      if (skill.path.includes('..')) {
        failures.push(`  ${skill.name}: path must not escape dist/ root with ../ (got '${skill.path}')`);
      }
    }
    assert.equal(failures.length, 0, `${failures.length} path safety issue(s):\n${failures.join('\n')}`);
  });
  ...
  await t.test('should have complete SKILL.md files with all required sections', () => {
    ...
    for (const skill of manifest.skills) {
      ...
      if (!content.startsWith('---')) {
        failures.push(`  ${skill.name}: SKILL.md must start with --- frontmatter`);
      }
      if (!/^---[\s\S]*?skill:\s*["']?[\w-]+["']?/.test(content)) {
        failures.push(`  ${skill.name}: SKILL.md must have 'skill' field in frontmatter`);
      }
      if (!/^---[\s\S]*?description:/.test(content)) {
        failures.push(`  ${skill.name}: SKILL.md must have 'description' field in frontmatter`);
      }
      if (!/^---[\s\S]*?type:/.test(content)) {
        failures.push(`  ${skill.name}: SKILL.md must have 'type' field in frontmatter`);
      }
      if (!/^---[\s\S]*?status:/.test(content)) {
        failures.push(`  ${skill.name}: SKILL.md must have 'status' field in frontmatter`);
      }
    }
    ...
    for (const skill of manifest.skills) {
      ...
      for (const match of promptFileMatches) {
        ...
        if (!existsSync(fullPath)) {
          failures.push(`  ${skill.name}: prompt_file '${promptPath}' not found in dist/ (looked at ${fullPath})`);
        }
      }
    }
