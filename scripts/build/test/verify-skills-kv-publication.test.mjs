...
const modulePromise = safeImport('../verify-skills-kv-publication.mjs', import.meta.url);
...
test('verifyPublication_checks_versioned_and_latest_keys', async () => {
  ...
  verifyPublication({
    ...
    runner: (_cmd, args) => {
      ...
      return {
        status: 0,
        stdout: JSON.stringify({ version: '1.2.3', skills: { debug: { 'SKILL.md': '# Debug' } } }),
        stderr: '',
      };
    },
    logger: () => {},
...
test('verifyPublication_fails_on_version_mismatch', async () => {
  ...
  assert.throws(
    () => verifyPublication({
      ...
      runner: () => ({
        status: 0,
        stdout: JSON.stringify({ version: '9.9.9', skills: { debug: { 'SKILL.md': '# Debug' } } }),
        stderr: '',
      }),
      logger: () => {},
    }),
