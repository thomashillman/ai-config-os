...
const AUTOSPEC_DIR = resolve(REPO_ROOT, 'docs', 'autospec', 'review-repository');
...
test('spec defines review_repository routes, primitives, and metrics', () => {
  ...
  assert.deepEqual(routes, new Set(['github_pr', 'pasted_diff', 'uploaded_bundle', 'local_repo']));
...
test('tasks enumerate deterministic T001-T020 sequence with status guardrails', () => {
  ...
  assert.ok(Array.isArray(tasksDoc.tasks), 'tasks.yaml must contain a tasks array');
  assert.equal(tasksDoc.tasks.length, 20, 'Expected exactly 20 MVA tasks (T001-T020)');

  const expectedIds = Array.from({ length: 20 }, (_, index) => `T${String(index + 1).padStart(3, '0')}`);
  const actualIds = tasksDoc.tasks.map(task => task.id);
  assert.deepEqual(actualIds, expectedIds, 'Task IDs must be sequential and deterministic');

  const validStatuses = new Set(['todo', 'done']);
  for (const task of tasksDoc.tasks) {
    assert.ok(validStatuses.has(task.status), `Unexpected status for ${task.id}: ${task.status}`);
    assert.ok(['week_1', 'week_2', 'week_3'].includes(task.sprint), `${task.id} has invalid sprint`);
  }

  const doneIds = tasksDoc.tasks.filter(task => task.status === 'done').map(task => task.id);
  const firstTodoIndex = tasksDoc.tasks.findIndex(task => task.status === 'todo');
  const expectedDonePrefix = (firstTodoIndex === -1 ? tasksDoc.tasks : tasksDoc.tasks.slice(0, firstTodoIndex))
    .map(task => task.id);
  assert.deepEqual(doneIds, expectedDonePrefix, 'Done tasks must form a contiguous prefix from T001');
});
...
test('plan sprint task mapping stays consistent with tasks.yaml', () => {
  ...
  for (const sprint of plan.sprints) {
    assert.ok(Array.isArray(sprint.task_ids), `${sprint.id} must define task_ids`);
    assert.deepEqual(sprint.task_ids, bySprint.get(sprint.id), `${sprint.id} task_ids mismatch tasks.yaml`);
  }
});
