...
const SCHEMA_PATH = join(ROOT, 'shared/skills/post-merge-retrospective/schema/artifact.schema.json');
...
function validateAgainstSchema(schema, payload) {
  ...
  if (!Array.isArray(payload.friction_signals)) return 'friction_signals must be an array';
  ...
  for (let i = 0; i < payload.friction_signals.length; i++) {
    ...
    for (const f of signalSchema.required) {
      if (!(f in s)) return `friction_signals[${i}] missing required field: ${f}`;
    }
  ...
  if (!Array.isArray(payload.skill_recommendations)) return 'skill_recommendations must be an array';
  ...
  for (let i = 0; i < payload.skill_recommendations.length; i++) {
    ...
    for (const f of recSchema.required) {
      if (!(f in r)) return `skill_recommendations[${i}] missing required field: ${f}`;
    }
...
test('Retrospective artifact JSON Schema', async (t) => {
  ...
  await t.test('schema requires all expected top-level fields', () => {
    const schema = loadSchema();
    const expected = ['schema_version', 'generated_at', 'pr_ref', 'session_stats',
      'friction_signals', 'skill_recommendations', 'summary'];
    for (const field of expected) {
      assert.ok(schema.required.includes(field), `required includes "${field}"`);
    }
  });
  ...
  await t.test('friction_signals has expected signal type enums', () => {
    ...
    const expected = ['error', 'correction', 'loop', 'assumption_failure',
      'missing_context', 'inefficiency', 'capability_gap'];
    for (const v of expected) {
      assert.ok(enums.includes(v), `friction type enum includes "${v}"`);
    }
    assert.equal(enums.length, expected.length, 'No extra signal types in schema');
  });

  await t.test('skill_recommendations.name has kebab-case pattern', () => {
    const schema = loadSchema();
    const pattern = schema.properties.skill_recommendations.items.properties.name.pattern;
  ...
  await t.test('skill_recommendations has expected category enums', () => {
    const schema = loadSchema();
    const enums = schema.properties.skill_recommendations.items.properties.category.enum;
    const expected = ['library-api-reference', 'product-verification', 'data-fetching',
  ...
  await t.test('wrong schema_version fails', () => {
    const schema = loadSchema();
    const error = validateAgainstSchema(schema, validArtifact({ schema_version: '2.0' }));
    assert.ok(error !== null, 'Should fail with wrong schema_version');
    assert.ok(error.includes('schema_version'), `Error should mention field: ${error}`);
  });
  ...
  await t.test('invalid friction signal type fails', () => {
    ...
    assert.ok(error.includes('type'), `Error should mention type field: ${error}`);
  });
  ...
  await t.test('invalid impact level fails', () => {
    ...
    assert.ok(error.includes('impact'), `Error should mention impact field: ${error}`);
  });
  ...
  await t.test('recommendation name with uppercase fails', () => {
    ...
    assert.ok(error.includes('name'), `Error should mention name field: ${error}`);
  });
  ...
  await t.test('invalid recommendation category fails', () => {
    const schema = loadSchema();
    const artifact = validArtifact({
      skill_recommendations: [validRecommendation({ category: 'unknown-category' })],
      summary: { total_signals: 0, high_impact_signals: 0, recommendation_count: 1 },
    });
    const error = validateAgainstSchema(schema, artifact);
    assert.ok(error !== null, 'Should fail with invalid category');
    assert.ok(error.includes('category'), `Error should mention category: ${error}`);
  });
  ...
  await t.test('non-boolean repeatable in friction signal fails', () => {
    ...
    assert.ok(error.includes('repeatable'), `Error should mention repeatable: ${error}`);
  });

  await t.test('schema enums for priority and estimated_reuse are correct', () => {
    const schema = loadSchema();
    const recProps = schema.properties.skill_recommendations.items.properties;
    assert.deepEqual(recProps.priority.enum.sort(), ['high', 'low', 'medium']);
    assert.deepEqual(recProps.estimated_reuse.enum.sort(), ['frequent', 'occasional', 'once']);
  });
});
