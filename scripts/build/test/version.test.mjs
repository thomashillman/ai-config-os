...
test("validateReleaseVersion rejects invalid versions", () => {
  ...
  assert.throws(
    () => validateReleaseVersion("1.0.0-beta"),
    /Invalid release version/,
  );
  assert.throws(
    () => validateReleaseVersion("v1.0.0"),
    /Invalid release version/,
  );
...
test("local build emits release version without provenance", () => {
  ...
  assert.equal(
    pluginJson.version,
    expectedVersion,
    "Emitted version should match VERSION",
  );
  assert.equal(
    pluginJson.built_at,
    undefined,
    "Local build should not have built_at",
  );
  assert.equal(
    pluginJson.build_id,
    undefined,
    "Local build should not have build_id",
  );
  assert.equal(
    pluginJson.source_commit,
    undefined,
    "Local build should not have source_commit",
  );
  ...
  assert.equal(
    ...
    expectedVersion,
    "Registry version should match VERSION",
  );
  assert.equal(
    registryJson.built_at,
    undefined,
    "Registry local build should not have built_at",
  );
});
...
test("release build emits provenance fields", () => {
  ...
  assert.equal(
    pluginJson.version,
    expectedVersion,
    "Release version should still match VERSION",
  );
  assert.ok(pluginJson.built_at, "Release build should have built_at");
  assert.equal(
    pluginJson.build_id,
    "run-456",
    "Release build should have build_id from env",
  );
  assert.equal(
    pluginJson.source_commit,
    "test-sha-123",
    "Release build should have source_commit from env",
  );
});
...
test("version parity check passes when files are in sync", () => {
  ...
  assert.ok(
    result.stdout.includes("parity check passed"),
...
test("getReleaseVersion returns same value as readReleaseVersion", async () => {
  const { getReleaseVersion } =
    await import("../../../runtime/lib/release-version.mjs");
  const expected = readReleaseVersion(REPO_ROOT);
  assert.equal(
    getReleaseVersion(),
    expected,
    "getReleaseVersion() must match readReleaseVersion()",
  );
});

test("server.js does not hardcode a version string", () => {
  const serverSrc = readFileSync(
    join(REPO_ROOT, "runtime", "mcp", "server.js"),
    "utf8",
  );
  assert.ok(
...
test("getReleaseVersionFromRoot rejects invalid semver in VERSION", async () => {
  ...
  try {
    ...
    const { getReleaseVersionFromRoot } =
      await import("../../../runtime/lib/release-version.mjs");
