...
describe('schema contract — unknown fields rejected', () => {
  ...
  test('known fields in tests item are accepted', () => {
    ...
    const valid = validate(fm);
    assert.equal(valid, true, `Should accept known test fields: ${JSON.stringify(validate.errors)}`);
  });
  ...
  test('known fields in docs are accepted', () => {
    ...
    const valid = validate(fm);
    assert.equal(valid, true, `Should accept known docs fields: ${JSON.stringify(validate.errors)}`);
  });
