...
test("compiler-fixture: temp repo helper creates valid structure", () => {
  ...
  try {
    ...
    assert.ok(
      existsSync(join(repoPath, "VERSION")),
      "Should create VERSION file",
    );
    assert.ok(
    ...
    assert.ok(
      existsSync(join(repoPath, "shared", "skills", "test-skill", "SKILL.md")),
      "Should create SKILL.md",
    );
    assert.ok(
      existsSync(join(repoPath, "shared", "targets", "platforms")),
      "Should create platforms dir",
    );
  } finally {
    cleanup();
  }
...
test("compiler-fixture: createSkillMd generates valid frontmatter", () => {
  ...
  assert.ok(
    skillMd.includes("description: A test skill"),
...
test("compiler-fixture: createPlatformYaml generates valid YAML", () => {
  ...
  assert.ok(
    platformYaml.includes("status: supported"),
