...
test("Step 3.1: Admissibility filtering", async (t) => {
  ...
  await t.test(
    "returns empty frontier when no candidates meet constraints",
    () => {
      const input = {
        ...baseInput,
        route_compatibility_projection: {
...
test("Step 3.2: Non-dominated frontier construction", async (t) => {
  await t.test("computes correct non-dominated set", () => {
    ...
    assert.ok(
      result.non_dominated_frontier_size >= result.admissible_candidates.length,
    );
  });
  ...
  await t.test("respects dominance ordering", () => {
    ...
    assert.ok(
      result.admissible_candidates.some(
        (c) => c.model_id === "efficient-reliable",
      ),
    );
    assert.ok(
      !result.admissible_candidates.some(
        (c) => c.model_id === "expensive-unreliable",
      ),
    );
  });
});
...
test("Step 3.3: Representative selection and ordering", async (t) => {
  ...
  await t.test("deterministic ordering is stable", () => {
    ...
    assert.equal(
      result1.admissible_candidates.length,
      result2.admissible_candidates.length,
    );
...
test("Step 3.5: Input validation", async (t) => {
  await t.test("rejects missing registry_snapshot", () => {
    ...
    assert.throws(
      () => evaluateModelPaths(input),
      /registry_snapshot must be an array/,
    );
  });
  ...
  await t.test("rejects missing allowed_execution_modes", () => {
    ...
    assert.throws(
      () => evaluateModelPaths(input),
      /allowed_execution_modes must be an array/,
    );
  });
  ...
  await t.test("rejects missing minimum_model_tier", () => {
    ...
    assert.throws(
      () => evaluateModelPaths(input),
      /minimum_model_tier is required/,
    );
  });
});
