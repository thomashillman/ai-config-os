...
test("route input definitions cover all declared runtime routes with no extras", () => {
  ...
  for (const [taskType, taskDefinition] of Object.entries(routes.taskTypes)) {
    const definedRoutes = new Set(
      (taskDefinition.routes || []).map((route) => route.route_id),
    );
    const inputRoutes = new Set(
      Object.keys(inputs.taskTypes?.[taskType]?.routes || {}),
    );
...
`);
  ...
  assert.throws(
    () => loadTaskRouteInputDefinitions(filePath),
