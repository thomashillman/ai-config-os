...
const mod = await import('../../../worker/src/validation/tasks.ts').catch(async () => {
  ...
  const loaded = await import(pathToFileURL(join(temp, 'src', 'validation', 'tasks.js')).href);
...
test('validateTaskStatePayload rejects missing expected_version', () => {
  const r = validateTaskStatePayload({ next_state: 'active', next_action: 'review', updated_at: new Date().toISOString() });
  assert.equal(r.ok, false);
  assert.ok(r.error.includes('expected_version'));
});

test('validateTaskStatePayload rejects non-ISO updated_at', () => {
  const r = validateTaskStatePayload({ expected_version: 1, next_state: 'active', next_action: 'review', updated_at: 'not-a-date' });
  assert.equal(r.ok, false);
...
test('validateRouteSelectionPayload rejects empty route_id', () => {
  const r = validateRouteSelectionPayload({ expected_version: 1, route_id: '', selected_at: new Date().toISOString() });
  assert.equal(r.ok, false);
...
test('validateAppendFindingPayload rejects non-object finding', () => {
  const r = validateAppendFindingPayload({ expected_version: 1, finding: 'not-an-object', updated_at: new Date().toISOString() });
  assert.equal(r.ok, false);
