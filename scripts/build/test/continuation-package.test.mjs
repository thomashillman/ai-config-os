...
import {
  TaskStore,
  TaskNotFoundError,
} from "../../../runtime/lib/task-store.mjs";
...
function buildContract(overrides = {}) {
  return {
    ...
    stronger_host_guidance:
      "Upgrade to route 'local_repo' when host supports: repo_local_read.",
    ...overrides,
  };
}
...
test("createContinuationPackage builds validated payload", () => {
  ...
  assert.equal(
    continuationPackage.effective_execution_contract.task_id,
    task.task_id,
  );
...
test("TaskStore createContinuationPackage is idempotent for retries with same handoff token", () => {
  ...
  assert.equal(
    events[0].metadata?.continuation_package_created_at,
...
test("TaskStore createContinuationPackage stores canonical timestamp metadata per token", () => {
  ...
  const byToken = new Map(
    events.map((event) => [event.metadata?.handoff_token_id, event]),
  );

  assert.equal(
    byToken.get("handoff_001")?.metadata?.continuation_package_created_at,
    "2026-03-12T12:03:00.000Z",
  );
  assert.equal(
    byToken.get("handoff_002")?.metadata?.continuation_package_created_at,
