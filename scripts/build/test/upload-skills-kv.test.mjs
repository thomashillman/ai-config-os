...
test("buildSkillsPackage_embeds_skill_files_from_dist", async () => {
  const { buildSkillsPackage } = await safeImport(
    "../upload-skills-kv.mjs",
    import.meta.url,
  );
  ...
  try {
    ...
    assert.equal(
      result.package.skills.debug["SKILL.md"],
      "# Debug\nUse this skill.\n",
    );
    assert.equal(
      result.package.skills.debug["prompts/brief.md"],
      "Be brief.\n",
    );
...
test("uploadToKV_surfaces_cloudflare_error_when_runner_fails_without_stderr", async () => {
  const { uploadToKV } = await safeImport(
    "../upload-skills-kv.mjs",
    import.meta.url,
  );
