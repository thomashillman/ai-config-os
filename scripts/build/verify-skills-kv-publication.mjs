...
const PACKAGE_KEYS = (version) => [`claude-code-package:${version}`, 'claude-code-package:latest'];
...
function readProductionManifestNamespaceId() {
  ...
  const manifestNamespace = namespaces.find((entry) => entry?.binding === 'MANIFEST_KV');

  if (!manifestNamespace?.id) {
    throw new Error('worker/wrangler.toml missing production MANIFEST_KV binding id');
  }
...
export function verifyPublication({
  ...
  if (!accountId || !apiToken || !kvNamespaceId) {
    throw new Error('Missing required env vars: CLOUDFLARE_ACCOUNT_ID, CLOUDFLARE_API_TOKEN, MANIFEST_KV_NAMESPACE_ID');
  }
  ...
  for (const key of PACKAGE_KEYS(expectedVersion)) {
    const result = downloadKey(key, { accountId, apiToken, kvNamespaceId, runner });
    ...
    if (pkg.version !== expectedVersion) {
      throw new Error(`KV key ${key} has version ${pkg.version}; expected ${expectedVersion}`);
    }

    if (!pkg.skills || typeof pkg.skills !== 'object' || Array.isArray(pkg.skills)) {
...
export async function main({ argv = process.argv.slice(2), logger = console.log, errorLogger = console.error } = {}) {
  try {
    ...
    logger(`✓ Claude package publication verified for version ${expectedVersion}`);
  } catch (err) {
