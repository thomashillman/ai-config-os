...
import {
  ...
  getOutcomeValidator,
  getSkillSchema,
} from "./lib/validators-cache.mjs";
import {
  loadToolIds,
  loadRouteDefinitions,
  loadRouteInputDefinitions,
} from "./lib/load-runtime-data.mjs";
...
const TASK_ROUTE_DEFINITIONS_PATH = join(
  ROOT,
  "runtime",
  "task-route-definitions.yaml",
);
const TASK_ROUTE_INPUT_DEFINITIONS_PATH = join(
  ROOT,
  "runtime",
  "task-route-input-definitions.yaml",
);
...
const releaseMode =
  process.argv.includes("--release") || process.env.AI_CONFIG_RELEASE === "1";
...
async function loadValidators() {
  const [skillValidator, platformValidator, routeValidator, outcomeValidator] =
    await Promise.all([
      getSkillValidator(),
      getPlatformValidator(),
      getRouteValidator(),
...
async function main() {
  ...
  for (const [platformId, platformDef] of platforms) {
    ...
    const { errors: policyErrors } = validatePlatformPolicy(
      platformDef,
      platformId,
    );
  ...
  const knownCapabilityIds = new Set(
    skillSchema.$defs?.capabilityId?.enum || [],
  );
  ...
  for (const [outcomeId, outcomeDef] of outcomes) {
    ...
    if (!valid) {
      console.error(
        `  [error] outcome ${outcomeId}: schema validation failed:`,
      );
  ...
  if (zeroEmitSkills.length > 0) {
    ...
    for (const skillId of zeroEmitSkills) {
      console.error(
        `  - ${skillId}: check capabilities.required vs all platform definitions`,
      );
    }
    console.error(
  ...
  const actuallyEmittedPlatforms = selectEmittedPlatforms(
    platformSkills,
    emitterRegistry,
  );
  ...
  for (const [platformId, skills] of Object.entries(platformSkills)) {
    ...
    if (platformId === "claude-code") {
      ...
      console.log(
        `  [${platformId}] emitter not yet implemented — skipping (${skills.length} skill(s))`,
      );
    }
  }

  console.log("\n[registry]");
  emitRegistry(parsed, actuallyEmittedPlatforms, {
    distDir: DIST_DIR,
    releaseVersion,
    provenance,
    compatMatrix,
    platformDefs: platforms,
  });
  emitSummary(parsed, actuallyEmittedPlatforms, {
    distDir: DIST_DIR,
    releaseVersion,
  });

  console.log("\n[runtime]");
  const { taskTypes: taskRouteDefinitions } = loadRouteDefinitions(
    TASK_ROUTE_DEFINITIONS_PATH,
  );
  const { taskTypes: taskRouteInputDefinitions } = loadRouteInputDefinitions(
    TASK_ROUTE_INPUT_DEFINITIONS_PATH,
  );
