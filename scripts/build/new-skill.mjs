...
if (existsSync(lintScript)) {
  const lintResult = spawnSync(
    process.execPath,
    [lintScript, join(sharedDir, "SKILL.md")],
    {
      cwd: REPO_ROOT,
      encoding: "utf8",
      stdio: ["pipe", "pipe", "pipe"],
  ...
  if (lintResult.status === 0) {
    ...
    console.log(
      `  node scripts/lint/skill.mjs shared/skills/${skillName}/SKILL.md`,
    );
  }
}
...
console.log(
  "To bump the release: edit VERSION, then run npm run version:sync.",
);
