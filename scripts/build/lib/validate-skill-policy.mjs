...
export function validateSkillPolicy(
  ...
  if (
    ...
    errors.push(
      "Legacy flat capabilities array is forbidden. Use { required, optional, fallback_mode }.",
    );
  }
  ...
  if (
    ...
    if (overlap.length > 0) {
      errors.push(
        `Capability appears in both required and optional: ${overlap.join(", ")}`,
      );
    }
  }
  ...
  if (frontmatter.platforms && typeof frontmatter.platforms === "object") {
    for (const [pid, pOverride] of Object.entries(frontmatter.platforms)) {
      if (knownPlatforms.size > 0 && !knownPlatforms.has(pid)) {
        errors.push(
          `Unknown platform '${pid}'. Known: ${[...knownPlatforms].join(", ")}`,
        );
      }
      ...
      if (pOverride && typeof pOverride === "object") {
        ...
        if (
          pOverride.mode === "excluded" &&
          pOverride.allow_unverified === true
        ) {
          errors.push(
            `Platform '${pid}': mode=excluded cannot have allow_unverified=true.`,
          );
        }
      }
    }
  ...
  if (frontmatter.type === "hook") {
    ...
    const hasExplicitPlatforms =
      frontmatter.platforms && typeof frontmatter.platforms === "object";
    if (!hasExplicitPlatforms) {
      errors.push(
        `Hook skill must have explicit 'platforms' block with exclusions for non-hook surfaces.`,
      );
    } else {
  ...
  if (Array.isArray(declaredTools)) {
    for (const toolId of declaredTools) {
      if (registeredTools.size > 0 && !registeredTools.has(toolId)) {
        errors.push(
          `Unknown tool dependency '${toolId}'. Registered tools: ${[...registeredTools].join(", ")}`,
        );
      }
    }
  }
