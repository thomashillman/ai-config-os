...
const docsToCheck = ["PLAN.md", "README.md"].map((file) =>
...
try {
  ...
  console.error(
    "[skill-count-drift] ERROR: failed to inspect skills inventory.",
  );
...
console.log(
  `[skill-count-drift] Inventory count from shared/skills/*/SKILL.md: ${actualCount}`,
);
console.log(`[skill-count-drift] Canonical phrase: ${CANONICAL_PHRASE}`);

if (errors.length > 0) {
  console.error(
    "[skill-count-drift] ERROR: documented skill count drift detected.",
  );
...
console.log(
  "[skill-count-drift] OK: all declared skill counts match inventory.",
);
