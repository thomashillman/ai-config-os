...
export const CANONICAL_PHRASE = 'Installable skill count: <number> (source: shared/skills/*/SKILL.md; excluding _template).';
  ...
  return readdirSync(skillsRoot, { withFileTypes: true })
    ...
    .filter(name => {
      ...
      try {
        ...
        throw new Error(`Missing or unreadable skill file: ${skillPath}`, { cause: error });
      }
    })
    .length;
}
...
export function parseDeclaredCounts(docPath, content) {
  ...
  const hasMalformedPhrase = MALFORMED_PHRASE_PATTERN.test(content) && matches.length === 0;
