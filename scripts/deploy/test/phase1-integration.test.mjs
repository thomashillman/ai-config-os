...
test('Integration: Service binding path has priority over proxy', () => {
  ...
  assert.equal(result1, 'service-binding', 'Service binding should be preferred');
...
test('Integration: Error message clarity without executor configured', () => {
  ...
  assert.ok(msg.includes('Phase 1 requires service binding'), 'Message should emphasize Phase 1 requirement');
  assert.ok(msg.includes('EXECUTOR'), 'Message should name the binding');
  assert.ok(msg.includes('optional'), 'Message should indicate proxy is optional');
  assert.ok(!msg.includes('Phase 0'), 'Message should not confuse with Phase 0');
});
...
test('Integration: Timeout clamping respects Phase 1 15s limit', () => {
  ...
  assert.equal(clampTimeout(10000, true), 10000, 'Phase 1 should respect values under 15s');

  // Phase 0/Phase 2 proxy
  assert.equal(clampTimeout(20000, false), 20000, 'Proxy can exceed 15s');
  assert.equal(clampTimeout(120000, false), 120000, 'Proxy should clamp to 120s');
});
...
test('Integration: Request signature passes through both paths', () => {
  ...
  assert.equal(viaBinding.signature, 'sig123', 'Service binding should preserve signature');
  assert.equal(viaProxy.signature, 'sig123', 'Proxy should preserve signature');
  assert.equal(viaBinding.hasSignature, true, 'Binding should detect signature');
...
test('Integration: Phase 1 does not expose shell capability claims', () => {
  ...
  const phase1Tools = ['health_check', 'list_phase1_tools', 'get_skill_metadata', 'get_artifact', 'skill_stats_cached'];
  const phase0Tools = ['sync_tools', 'list_tools', 'get_config', 'context_cost', 'validate_all'];

  // Phase 1 should not include Phase 0 tools
  for (const tool of phase0Tools) {
    assert.ok(!phase1Tools.includes(tool), `Phase 1 should not claim support for ${tool}`);
  }
  ...
  for (const [tool, desc] of Object.entries(kvR2Tools)) {
    assert.ok(desc.includes('KV') || desc.includes('R2') || desc === 'Available Phase 1 tools' || desc === 'Worker health status',
...
test('Integration: Configuration changes do not require code changes', () => {
  ...
  function selectExecutor(env) {
    ...
    return env.EXECUTOR ? 'service-binding' : (env.EXECUTOR_PROXY_URL ? 'proxy' : 'none');
  }
