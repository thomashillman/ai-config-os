...
test("Configuration Consistency: Service binding name matches expected naming pattern", () => {
  ...
  assert.equal(
    result.valid,
    true,
    "Staging service binding name must follow convention",
  );
  ...
  assert.equal(
    prodResult.valid,
    true,
    "Production service binding name must follow convention",
  );
});
...
test("Error Guidance: Phase 0 backward compat message is clear", () => {
  ...
  assert.equal(
    result.valid,
    true,
    "Valid proxy URL alone should pass (Phase 0 compat)",
  );
});
...
test("Fallback Path: Proxy URL alone is accepted for Phase 0 backward compatibility", () => {
  ...
  assert.equal(result.valid, true, "Proxy URL alone is valid (Phase 0 compat)");
});
...
test("Fallback Path: Placeholder URLs are rejected (prevents accidental Phase 0 usage)", () => {
  ...
  assert.ok(
    isPlaceholder(placeholders[0], "production"),
    "Production placeholder should be detected",
  );
  assert.ok(
    isPlaceholder(placeholders[1], "staging"),
    "Staging placeholder should be detected",
  );
  assert.ok(
    !isPlaceholder("https://real-executor.mycompany.com", "production"),
    "Real URL should not be detected as placeholder",
  );
});
...
test("Phase 2 Seam: Both paths can be configured simultaneously without conflict", () => {
  ...
  assert.equal(
    result.valid,
    true,
    "Service binding + proxy URL should be valid (Phase 2 seam)",
  );
});
...
test("Phase 2 Seam: Proxy URL is optional, not required", () => {
  ...
  assert.equal(
    result.valid,
    true,
    "EXECUTOR_PROXY_URL is optional for Phase 1",
  );
  assert.equal(
...
test("Binding Validation: Service binding must use EXECUTOR binding name", () => {
  ...
  assert.equal(
    result.valid,
    false,
    "Service binding must use EXECUTOR binding name",
  );
  assert.ok(
...
test("KV/R2 Validation: Placeholder IDs are rejected", () => {
  ...
  const result = validateWranglerConfigForEnv(
    placeholderIdConfig,
    "production",
  );
...
test("Timeout Validation: EXECUTOR_TIMEOUT_MS can be omitted (uses default)", () => {
  ...
  assert.equal(
    result.valid,
    true,
    "Missing EXECUTOR_TIMEOUT_MS should use default",
  );
});
