...
test('skill-outcome-tracker - PreToolUse ignores non-Skill tools', async () => {
  ...
  const pendingFile = join(tmpDir, `${sessionId}-skill-pending.json`);
  assert.ok(!existsSync(pendingFile), 'Pending file should not exist for non-Skill tools');
...
test('skill-outcome-tracker - PostToolUse clears pending after outcome recorded', async () => {
  ...
  const pendingFile = join('/tmp/claude-sessions', `${sessionId}-skill-pending.json`);
  ...
  assert.ok(!existsSync(pendingFile), 'Pending file should be cleared after outcome recorded');
...
test('skill-outcome-tracker - PostToolUse ignores Edit/Write without pending skill', async () => {
  ...
  const outcomes = readOutcomesLog();
  assert.equal(outcomes.length, 0, 'Should not record outcome without pending skill');
...
test('skill-outcome-tracker - PostToolUse ignores non-Edit/Write tools', async () => {
  ...
  const outcomes = readOutcomesLog();
  assert.equal(outcomes.length, 0, 'Should not record outcome for non-Edit/Write tools');

  // Pending should still exist
  const pendingFile = join('/tmp/claude-sessions', `${sessionId}-skill-pending.json`);
  assert.ok(existsSync(pendingFile), 'Pending should remain after non-Edit/Write PostToolUse');
...
test('skill-outcome-tracker - Phase routing works correctly', async () => {
  ...
  assert.ok(existsSync(join('/tmp/claude-sessions', `${sessionId}-skill-pending.json`)));
...
test('skill-outcome-tracker - JSONL format is valid', async () => {
  ...
  assert.ok(['output_used', 'output_replaced'].includes(outcome.outcome), 'Outcome should be valid');
