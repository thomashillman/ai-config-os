...
test('config-integrity - PreToolUse routes to dispatcher only', () => {
  ...
  const dispatcherCount = commands.filter(c => c.includes('pre-tool-use.sh')).length;
  assert.equal(dispatcherCount, 1, 'PreToolUse should route to exactly one pre-tool-use.sh');
...
test('config-integrity - PostToolUse routes to dispatcher only', () => {
  ...
  const dispatcherCount = commands.filter(c => c.includes('post-tool-use.sh')).length;
  assert.equal(dispatcherCount, 1, 'PostToolUse should route to exactly one post-tool-use.sh');
...
test('config-integrity - dispatcher scripts exist', () => {
  const preToolUsePath = resolve(process.cwd(), '.claude', 'hooks', 'pre-tool-use.sh');
  const postToolUsePath = resolve(process.cwd(), '.claude', 'hooks', 'post-tool-use.sh');
...
test('config-integrity - dispatcher.mjs exists', () => {
  const dispatcherPath = resolve(process.cwd(), '.claude', 'hooks', 'dispatch.mjs');
...
test('config-integrity - single source of truth for each concern', () => {
  ...
  const rulesIndexPath = resolve(process.cwd(), '.claude', 'hooks', 'lib', 'rules', 'index.mjs');
