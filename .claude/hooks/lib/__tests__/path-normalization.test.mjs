...
test('normalizeFilePath - converts relative paths to absolute', () => {
  ...
  assert.ok(result.includes('src/index.js') || result.includes('src\\index.js'), 'Should contain relative path');
  assert.ok(result.startsWith('/home/user/project') || result.includes('project'), 'Should be absolute');
});
...
test('normalizeFilePath - prevents directory traversal with ../', () => {
  ...
  if (result !== '../../etc/passwd') {
    ...
    assert.ok(result.startsWith(projectDir) || !result.includes('etc/passwd'),
...
test('normalizeFilePath - allows safe relative paths', () => {
  ...
  assert.ok(result.includes('src') && result.includes('components') && result.includes('App.js'),
...
test('normalizeFilePath - handles paths with ./ prefix', () => {
  ...
  assert.ok(result.includes('src') && result.includes('index.js'),
...
test('normalizeFilePath - returns original if escape detected', () => {
  ...
  if (result !== traversalPath) {
    assert.ok(result.startsWith(projectDir),
