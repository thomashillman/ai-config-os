...
const WORKER_URL =
  process.env.AI_CONFIG_WORKER ?? "https://ai-config-os.workers.dev";
const EMIT_RUNS =
  (process.env.AI_CONFIG_BOOTSTRAP_EMIT_RUNS ?? "1") !== "0" &&
...
async function emit(run) {
  ...
  if (!token) {
    process.stderr.write(
      "[bootstrap-run-emitter] AI_CONFIG_TOKEN not set; skipping run emit\n",
    );
    process.exit(1);
  }
  ...
  if (!EMIT_RUNS) {
    process.stderr.write(
      "[bootstrap-run-emitter] Run emission disabled via AI_CONFIG_BOOTSTRAP_EMIT_RUNS\n",
    );
    process.exit(1);
  }
  ...
  if (!validation.ok) {
    process.stderr.write(
      `[bootstrap-run-emitter] Validation error: ${validation.error}\n`,
    );
    process.exit(2);
  }
  ...
  try {
    ...
    if (!resp.ok) {
      process.stderr.write(
        `[bootstrap-run-emitter] Worker returned HTTP ${resp.status}\n`,
      );
      process.exit(3);
    }

    process.stdout.write(
      `[bootstrap-run-emitter] Run ${run.run_id} emitted (HTTP ${resp.status})\n`,
    );
    process.exit(0);
  } catch (err) {
    // Network errors are non-fatal — log to stderr but do not abort bootstrap
    process.stderr.write(
      `[bootstrap-run-emitter] Network error: ${err.message}\n`,
    );
    process.exit(3);
  }
}
...
readFile(runFile, "utf8")
  .then((text) => {
    ...
    try {
      ...
      process.stderr.write(
        `[bootstrap-run-emitter] Invalid JSON in ${runFile}: ${e.message}\n`,
      );
      process.exit(2);
    }
    return emit(run);
