...
function validateExecutePayload(payload: unknown): { ok: true; value: ExecutePayload } | { ok: false; error: string } {
  ...
  if (data.timeout_ms !== undefined && (!Number.isInteger(data.timeout_ms) || Number(data.timeout_ms) <= 0)) {
  ...
  if (tool === 'get_skill_metadata') {
    ...
    if (!skillId) {
      return {
        ok: false,
        status: 400,
        error: { code: 'INVALID_REQUEST', message: 'get_skill_metadata requires skill_id argument' },
      };
    }
  ...
  if (tool === 'get_artifact') {
    ...
    if (!version || !name) {
      return {
        ok: false,
        status: 400,
        error: { code: 'INVALID_REQUEST', message: 'get_artifact requires version and name arguments' },
      };
    }
 ...
 */
export async function handleExecutePhase1(request: Request, env: ExecutorEnv): Promise<Response> {
