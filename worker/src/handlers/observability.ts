...
export async function handleObservabilityRunCreate(
    ...
    return contractErrorResponse(
      {
        ...
        error: {
          ...
          message: "MANIFEST_KV or ARTEFACTS_R2 not bound",
          hint: "Check Worker storage binding configuration",
        },
      },
      503,
    );
  ...
  try {
    const result = await writeBootstrapRun(
      validation.value,
      env.MANIFEST_KV,
      env.ARTEFACTS_R2,
    );
    ...
    return contractErrorResponse(
      {
        ...
        error: {
          code: "storage_unavailable",
          message: "ARTEFACTS_R2 not bound",
          hint: "Check Worker storage binding configuration",
        },
      },
      503,
    ...
    return contractErrorResponse(
      {
        ...
        error: {
          ...
          hint: "Check the run ID and retry",
        },
      },
      404,
    );
...
export async function handleObservabilitySettingsGet(
  env: Env,
): Promise<Response> {
    ...
    return contractErrorResponse(
      {
        ...
        error: {
          ...
          message: "Settings did not pass validation",
          hint: validation.errors?.join("; ") ?? "Check the settings values",
        },
      },
      400,
    );
