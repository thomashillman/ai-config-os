...
/**
 * Platform capability profiles for skill filtering.
 * Each profile is immutable and cached forever.
 */
const PLATFORM_CAPABILITIES: Record<string, object> = {
  'claude-web': {
    platform: 'claude-web',
    surface: 'browser',
    supported: ['network.http'],
    unsupported: ['fs.read', 'fs.write', 'shell.exec', 'mcp.client'],
    capabilities: {
      http: true,
      filesystem: false,
      shell: false,
  ...
  'claude-code': {
    ...
    surface: 'cli',
    supported: ['fs.read', 'fs.write', 'shell.exec', 'mcp.client', 'network.http'],
    unsupported: [],
    capabilities: {
      http: true,
      filesystem: true,
...
export function handleCapabilitiesForPlatform(platform: string): Response {
  const caps = PLATFORM_CAPABILITIES[platform];
  if (!caps) {
    return notFound(
      `Unknown platform: ${platform}. Available: ${Object.keys(PLATFORM_CAPABILITIES).join(', ')}`
    );
  }

  // Immutable by platform; never changes
  return versionedCachedResponse(caps, platform);
}

/**
 * Filter skills by required capabilities.
 * Returns only skills where all required capabilities are supported by the request.
 */
export function handleSkillsCompatible(registry: RegistryLike, requestedCapabilities: string[]): Response {
  const capSet = new Set(requestedCapabilities);

  const compatibleSkills = registry.skills
    .map((entry) => readAsObject(entry))
    .filter(
      (obj) =>
        obj &&
        Array.isArray(obj.capabilities?.required) &&
        (obj.capabilities.required as string[]).every((cap) => capSet.has(cap))
    );

  // Cache by capability set hash (stable across calls)
  const cacheKey = requestedCapabilities.sort().join(',');
