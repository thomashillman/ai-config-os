...
async function handlePublish(
    ...
    return contractErrorResponse(
      {
        ...
        error: {
          ...
          message: "MANIFEST_KV not bound",
          hint: "Check Worker KV binding configuration",
        },
      },
      503,
    );
    ...
    return contractErrorResponse(
      {
        ...
        error: {
          ...
          message: "Snapshot must include data, meta, and summary",
          hint: "See docs/DASHBOARD_TO_WORKER_PLAN.md for the required shape",
        },
      },
      400,
    );
  ...
  const snapshot: DashboardSnapshot = {
    ...
    source_stamp:
      (body.source_stamp as string) ??
      request.headers.get("X-Publisher-Surface") ??
      "unknown",
  };
  ...
  if (!env.MANIFEST_KV) {
    ...
    return contractSuccessResponse({
      ...
      suggestedActions: [
        {
          id: "publish",
          label: "Publish local snapshot",
          reason:
            "Run runtime/publish-dashboard-state.mjs to push state to the Worker",
          runnable_target: "runtime/publish-dashboard-state.mjs",
        },
      ],
    });
  }
...
function handleActionRequest(resource: string): Response {
  return contractSuccessResponse({
    ...
    suggestedActions: [
      {
        id: "wait",
        label: "Wait for local runtime to publish",
        reason: "The local runtime will publish a fresh snapshot shortly",
        runnable_target: "runtime/publish-dashboard-state.mjs",
      },
    ],
  });
...
export async function handleSkillsPublish(
  request: Request,
  env: Env,
): Promise<Response> {
...
export async function handleToolingStatusPublish(
  request: Request,
  env: Env,
): Promise<Response> {
  return handlePublish("tooling.status", request, env);
}
export async function handleToolingStatusRead(
  url: URL,
  env: Env,
): Promise<Response> {
...
export async function handleConfigSummaryPublish(
  request: Request,
  env: Env,
): Promise<Response> {
  return handlePublish("config.summary", request, env);
}
export async function handleConfigSummaryRead(
  url: URL,
  env: Env,
): Promise<Response> {
...
export async function handleContextCostPublish(
  request: Request,
  env: Env,
): Promise<Response> {
  return handlePublish("runtime.context_cost", request, env);
}
export async function handleContextCostRead(
  url: URL,
  env: Env,
): Promise<Response> {
...
export async function handleAuditPublish(
  request: Request,
  env: Env,
): Promise<Response> {
...
export async function handleAnalyticsToolUsagePublish(
  request: Request,
  env: Env,
): Promise<Response> {
  return handlePublish("analytics.tool_usage", request, env);
}
export async function handleAnalyticsToolUsageRead(
  url: URL,
  env: Env,
): Promise<Response> {
...
export async function handleAnalyticsSkillEffectivenessPublish(
  request: Request,
  env: Env,
): Promise<Response> {
  return handlePublish("analytics.skill_effectiveness", request, env);
}
export async function handleAnalyticsSkillEffectivenessRead(
  url: URL,
  env: Env,
): Promise<Response> {
...
export async function handleAnalyticsAutoresearchRunsPublish(
  request: Request,
  env: Env,
): Promise<Response> {
  return handlePublish("analytics.autoresearch_runs", request, env);
}
export async function handleAnalyticsAutoresearchRunsRead(
  url: URL,
  env: Env,
): Promise<Response> {
...
export async function handleAnalyticsFrictionSignalsPublish(
  request: Request,
  env: Env,
): Promise<Response> {
  return handlePublish("analytics.friction_signals", request, env);
}
export async function handleAnalyticsFrictionSignalsRead(
  url: URL,
  env: Env,
): Promise<Response> {
