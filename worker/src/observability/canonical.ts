...
function summarizeFailureReason(run: { status: BootstrapRun['status']; first_failed_phase?: string; error_code?: string; expected_version?: string; observed_version?: string }): string {
  ...
  if (run.expected_version && run.observed_version && run.expected_version !== run.observed_version) {
...
function inferNextActions(run: { status: BootstrapRun['status']; first_failed_phase?: string; error_code?: string; expected_version?: string; observed_version?: string }): string[] {
...
export function withRunSignals<T extends RunSummary | BootstrapRun>(run: T): RunWithSignals<T> {
