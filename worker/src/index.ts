...
const _handler = createWorkerHandler(
  REGISTRY_JSON as { version: string; built_at?: string; skills: unknown[] },
  CLAUDE_CODE_PLUGIN_JSON,
);
...
export default {
  ...
  async scheduled(
    ...
    if (env.MANIFEST_KV && env.ARTEFACTS_R2) {
      ctx.waitUntil(
        cleanupExpiredRetrospectives(env.MANIFEST_KV, env.ARTEFACTS_R2, 60),
      );
    }
  },
