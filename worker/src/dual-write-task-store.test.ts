...
function makeMockKvStore() {
  ...
  return {
    ...
    transitionState: vi.fn().mockResolvedValue({ ...task, state: 'completed', version: 3 }),
    appendFinding: vi.fn().mockResolvedValue({ ...task, version: 3 }),
    transitionFindingsForRouteUpgrade: vi.fn().mockResolvedValue({ ...task, version: 3 }),
...
describe('DualWriteTaskStore', () => {
  ...
  describe('mutation methods call KV first, then DO', () => {
    ...
    it('transitionState', async () => {
      const payload = { expectedVersion: 2, nextState: 'completed', nextAction: 'done', updatedAt: '2026-01-01T00:00:00Z' };
    ...
    it('appendFinding', async () => {
      const payload = { expectedVersion: 2, finding: { finding_id: 'f1' }, updatedAt: '2026-01-01T00:00:00Z' };
    ...
    it('selectRoute', async () => {
      const payload = { routeId: 'local_repo', expectedVersion: 2, selectedAt: '2026-01-01T00:00:00Z' };
    ...
    it('update', async () => {
      const payload = { expectedVersion: 2, changes: { next_action: 'review' } };
    ...
    it('transitionFindingsForRouteUpgrade', async () => {
      const payload = { expectedVersion: 2, toRouteId: 'local_repo', upgradedAt: '2026-01-01T00:00:00Z', toEquivalenceLevel: 'equal' };
      await store.transitionFindingsForRouteUpgrade('task_1', payload);
      expect(kvStore.transitionFindingsForRouteUpgrade).toHaveBeenCalledWith('task_1', payload);
      await new Promise(r => setTimeout(r, 10));
      expect(doMock.fetchFn).toHaveBeenCalled();
    });

    it('createContinuationPackage', async () => {
      const payload = { handoffToken: { token_id: 'tok_1' }, effectiveExecutionContract: { task_id: 'task_1' } };
      await store.createContinuationPackage('task_1', payload);
      expect(kvStore.createContinuationPackage).toHaveBeenCalledWith('task_1', payload);
      await new Promise(r => setTimeout(r, 10));
      expect(doMock.fetchFn).toHaveBeenCalled();
    });
  });
  ...
  describe('error handling', () => {
    it('propagates KV errors without calling DO', async () => {
      ...
      await expect(store.create({ task_id: 'task_1' })).rejects.toThrow('KV write failed');
      expect(doMock.namespace.idFromName).not.toHaveBeenCalled();
    });
    ...
    it('swallows DO errors and returns KV result', async () => {
      ...
      await new Promise(r => setTimeout(r, 10));
      expect(warnSpy).toHaveBeenCalledWith(expect.stringContaining('[DualWrite]'));
      warnSpy.mockRestore();
    });

    it('swallows DO stub creation errors', async () => {
      doMock.namespace.idFromName.mockImplementation(() => { throw new Error('bad ID'); });
      const warnSpy = vi.spyOn(console, 'warn').mockImplementation(() => {});

      const result = await store.create({ task_id: 'task_1', version: 1 });
      expect(result.version).toBe(1);
      expect(warnSpy).toHaveBeenCalledWith(expect.stringContaining('[DualWrite] DO stub creation failed'));
      warnSpy.mockRestore();
    });
  });
