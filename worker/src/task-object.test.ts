...
describe('TaskObject', () => {
  ...
  describe('PUT /put-state', () => {
    ...
    it('stores events, snapshots, log, and fingerprints when provided', async () => {
      ...
      expect(state._store.get('continuation_fingerprints')).toEqual({ tok_1: 'fp_1' });
    });

    it('returns 409 on version conflict', async () => {
      // Store initial state at version 1
      await obj.fetch(makeRequest('POST', '/put-state', {
  ...
  describe('fingerprint management', () => {
    it('round-trips set and get', async () => {
      await obj.fetch(makeRequest('POST', '/set-fingerprint', {
        tokenId: 'tok_1',
        fingerprint: 'fp_abc',
      }));

      const res = await obj.fetch(makeRequest('GET', '/get-fingerprint?tokenId=tok_1'));
      const body = await res.json() as { tokenId: string; fingerprint: string | null };
    ...
    it('returns null for unknown tokenId', async () => {
      const res = await obj.fetch(makeRequest('GET', '/get-fingerprint?tokenId=unknown'));
    ...
    it('rejects missing tokenId on set', async () => {
      const res = await obj.fetch(makeRequest('POST', '/set-fingerprint', { tokenId: 123 }));
      expect(res.status).toBe(400);
    });
