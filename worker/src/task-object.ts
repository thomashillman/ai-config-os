...
export class TaskObject implements DurableObject {
  ...
  async fetch(request: Request): Promise<Response> {
    ...
    try {
      ...
      return jsonResponse({ error: 'not_found', message: `Unknown route: ${request.method} ${path}` }, 404);
    } catch (err) {
  ...
  private async handlePutState(request: Request): Promise<Response> {
    ...
    if (!body.task || typeof body.task !== 'object') {
      return jsonResponse({ error: 'validation_error', message: 'Missing or invalid task' }, 400);
    }

    const incomingVersion = typeof body.task.version === 'number' ? body.task.version : null;
    ...
    if (storedVersion !== undefined && incomingVersion !== null && incomingVersion !== storedVersion + 1) {
      console.warn(
    ...
    if (Array.isArray(body.snapshots) && body.snapshots.length > 0) {
      const existing = (await this.state.storage.get<unknown[]>('snapshots')) ?? [];
      await this.state.storage.put('snapshots', existing.concat(body.snapshots));
    }
    ...
    if (body.continuationFingerprints && typeof body.continuationFingerprints === 'object') {
      const existing = (await this.state.storage.get<Record<string, string>>('continuation_fingerprints')) ?? {};
      await this.state.storage.put('continuation_fingerprints', { ...existing, ...body.continuationFingerprints });
    }

    return jsonResponse({ ok: true, version: incomingVersion ?? (storedVersion ?? 0) + 1 });
  }
  ...
  private async handleGetState(): Promise<Response> {
    ...
    if (task === undefined) {
      return jsonResponse({ error: 'not_found', message: 'No state stored in this TaskObject' }, 404);
    }
  ...
  private async handleSetFingerprint(request: Request): Promise<Response> {
    const body = await request.json() as { tokenId: string; fingerprint: string };
    if (typeof body.tokenId !== 'string' || typeof body.fingerprint !== 'string') {
      return jsonResponse({ error: 'validation_error', message: 'tokenId and fingerprint must be strings' }, 400);
    }
    const existing = (await this.state.storage.get<Record<string, string>>('continuation_fingerprints')) ?? {};
  ...
  private async handleGetFingerprint(url: URL): Promise<Response> {
    ...
    if (!tokenId) {
      return jsonResponse({ error: 'validation_error', message: 'Missing tokenId query parameter' }, 400);
    }
    const fingerprints = (await this.state.storage.get<Record<string, string>>('continuation_fingerprints')) ?? {};
