...
const VALID_SIGNAL_TYPES: FrictionSignalType[] = [
  'error', 'correction', 'loop', 'assumption_failure',
  'missing_context', 'inefficiency', 'capability_gap',
];
const VALID_IMPACT_LEVELS: ImpactLevel[] = ['low', 'medium', 'high'];

function validateFrictionSignal(s: unknown, index: number): { ok: true; value: FrictionSignal } | { ok: false; error: string } {
  if (!isObject(s)) return { ok: false, error: `friction_signals[${index}] must be an object` };

  if (!VALID_SIGNAL_TYPES.includes(s.type as FrictionSignalType)) {
    return { ok: false, error: `friction_signals[${index}].type must be one of: ${VALID_SIGNAL_TYPES.join(', ')}` };
  }
  if (typeof s.turn_index !== 'number' || !Number.isInteger(s.turn_index) || s.turn_index < 0) {
    return { ok: false, error: `friction_signals[${index}].turn_index must be a non-negative integer` };
  }
  if (typeof s.description !== 'string' || s.description.length === 0) {
    return { ok: false, error: `friction_signals[${index}].description must be a non-empty string` };
  }
  if (s.description.length > MAX_MESSAGE_LENGTH) {
    return { ok: false, error: `friction_signals[${index}].description exceeds ${MAX_MESSAGE_LENGTH} characters` };
  }
  if (!VALID_IMPACT_LEVELS.includes(s.impact as ImpactLevel)) {
    return { ok: false, error: `friction_signals[${index}].impact must be one of: ${VALID_IMPACT_LEVELS.join(', ')}` };
  }
  if (typeof s.repeatable !== 'boolean') {
    return { ok: false, error: `friction_signals[${index}].repeatable must be a boolean` };
  }
...
const VALID_CATEGORIES: RecommendationCategory[] = [
  'library-api-reference', 'product-verification', 'data-fetching',
  'business-automation', 'scaffolding', 'code-quality', 'ci-cd', 'runbook',
];
...
function validateSkillRecommendation(r: unknown, index: number): { ok: true; value: SkillRecommendation } | { ok: false; error: string } {
  if (!isObject(r)) return { ok: false, error: `skill_recommendations[${index}] must be an object` };

  if (typeof r.name !== 'string' || r.name.length === 0) {
    return { ok: false, error: `skill_recommendations[${index}].name must be a non-empty string` };
  }
  if (!KEBAB_CASE_RE.test(r.name)) {
    return { ok: false, error: `skill_recommendations[${index}].name must be kebab-case (lowercase letters, digits, hyphens only)` };
  }
  if (!VALID_CATEGORIES.includes(r.category as RecommendationCategory)) {
    return { ok: false, error: `skill_recommendations[${index}].category must be one of: ${VALID_CATEGORIES.join(', ')}` };
  }
  for (const field of ['rationale', 'trigger_description'] as const) {
    if (typeof r[field] !== 'string' || (r[field] as string).length === 0) {
      return { ok: false, error: `skill_recommendations[${index}].${field} must be a non-empty string` };
    }
    if ((r[field] as string).length > MAX_MESSAGE_LENGTH) {
      return { ok: false, error: `skill_recommendations[${index}].${field} exceeds ${MAX_MESSAGE_LENGTH} characters` };
    }
  }
  if (!VALID_PRIORITIES.includes(r.priority as RecommendationPriority)) {
    return { ok: false, error: `skill_recommendations[${index}].priority must be one of: ${VALID_PRIORITIES.join(', ')}` };
  }
  if (!VALID_REUSE.includes(r.estimated_reuse as EstimatedReuse)) {
    return { ok: false, error: `skill_recommendations[${index}].estimated_reuse must be one of: ${VALID_REUSE.join(', ')}` };
  }
 ...
 */
export function validateRetrospectiveArtifact(payload: unknown): ValidationResult {
  if (!isObject(payload)) {
  ...
  if (forbidden !== null) {
    return { ok: false, error: `Field '${forbidden}' is not permitted in a retrospective` };
  }
  ...
  if (!isIsoDateTime(payload.generated_at)) {
    return { ok: false, error: "Field 'generated_at' must be a valid ISO 8601 timestamp" };
  }
  ...
  if (typeof stats.turn_count !== 'number' || !Number.isInteger(stats.turn_count) || stats.turn_count < 0) {
    return { ok: false, error: "session_stats.turn_count must be a non-negative integer" };
  }
  if (typeof stats.tool_calls !== 'number' || !Number.isInteger(stats.tool_calls) || stats.tool_calls < 0) {
    return { ok: false, error: "session_stats.tool_calls must be a non-negative integer" };
  }
  ...
  if (stats.duration_hint.length > 128) {
    return { ok: false, error: "session_stats.duration_hint exceeds 128 characters" };
  }
  ...
  if (payload.friction_signals.length > MAX_FRICTION_SIGNALS) {
    return { ok: false, error: `Field 'friction_signals' exceeds maximum of ${MAX_FRICTION_SIGNALS} entries` };
  }
  ...
  if (!Array.isArray(payload.skill_recommendations)) {
    return { ok: false, error: "Field 'skill_recommendations' must be an array" };
  }
  if (payload.skill_recommendations.length > MAX_RECOMMENDATIONS) {
    return { ok: false, error: `Field 'skill_recommendations' exceeds maximum of ${MAX_RECOMMENDATIONS} entries` };
  }
  const skillRecommendations: SkillRecommendation[] = [];
  for (let i = 0; i < payload.skill_recommendations.length; i++) {
    const result = validateSkillRecommendation(payload.skill_recommendations[i], i);
    if (!result.ok) return result;
  ...
  for (const field of ['total_signals', 'high_impact_signals', 'recommendation_count'] as const) {
    if (typeof sum[field] !== 'number' || !Number.isInteger(sum[field] as number) || (sum[field] as number) < 0) {
      return { ok: false, error: `summary.${field} must be a non-negative integer` };
    }
  }

  // Cross-field consistency checks
  if ((sum.total_signals as number) !== frictionSignals.length) {
    return { ok: false, error: `summary.total_signals (${sum.total_signals}) must equal friction_signals.length (${frictionSignals.length})` };
  }
  if ((sum.recommendation_count as number) !== skillRecommendations.length) {
    return { ok: false, error: `summary.recommendation_count (${sum.recommendation_count}) must equal skill_recommendations.length (${skillRecommendations.length})` };
  }
