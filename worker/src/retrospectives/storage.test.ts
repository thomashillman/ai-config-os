...
function makeArtifact(overrides: Partial<RetrospectiveArtifact> = {}): RetrospectiveArtifact {
  return {
    schema_version: '1.0',
    ...
    friction_signals: [
      { type: 'loop', turn_index: 2, description: 'Repeated the same read', impact: 'low', repeatable: true },
      { type: 'error', turn_index: 4, description: 'Type error on assignment', impact: 'high', repeatable: false },
    ],
    ...
    summary: { total_signals: 2, high_impact_signals: 1, recommendation_count: 1 },
    ...overrides,
  };
}
...
describe('deriveRetrospectiveId', () => {
  ...
  it('normalises a numeric PR ref', () => {
    expect(deriveRetrospectiveId('42', '2026-03-23T10:00:00Z')).toBe('2026-03-23-42');
  });

  it('converts slashes and special chars in branch-style refs to hyphens', () => {
    expect(deriveRetrospectiveId('feat/my-branch', '2026-03-23T10:00:00Z')).toBe('2026-03-23-feat-my-branch');
  });

  it('collapses consecutive separators and trims leading/trailing hyphens', () => {
    const id = deriveRetrospectiveId('--fix--something--', '2026-01-01T00:00:00Z');
...
describe('writeRetrospectiveArtifact', () => {
  ...
  it('prepends the new id to the KV index', async () => {
    const kv = makeMockKv({ [RETRO_INDEX_KV_KEY]: JSON.stringify(['2025-01-01-old']) });
    const r2 = makeMockR2();
...
describe('listRetrospectives', () => {
  ...
  it('returns metas for live entries and prunes stale ones', async () => {
    ...
    const liveMeta = JSON.stringify({
      id: liveId, pr_ref: '42', generated_at: '2026-03-23T10:00:00Z',
      schema_version: '1.0', total_signals: 1, high_impact_signals: 0,
      recommendation_count: 0, signal_types: ['loop'], recommendations_compact: [],
    });
  ...
  it('respects the limit parameter', async () => {
    ...
    for (const id of ids) {
      initial[artifactMetaKvKey(id)] = JSON.stringify({
        id, pr_ref: id, generated_at: '2026-03-23T00:00:00Z', schema_version: '1.0',
        total_signals: 0, high_impact_signals: 0, recommendation_count: 0,
        signal_types: [], recommendations_compact: [],
      });
    }
...
describe('aggregateRetrospectives', () => {
  ...
  beforeEach(() => {
    ...
    const meta1 = JSON.stringify({
      id: id1, pr_ref: 'pr-10', generated_at: '2026-03-01T00:00:00Z', schema_version: '1.0',
      total_signals: 2, high_impact_signals: 1, recommendation_count: 1,
      signal_types: ['loop', 'error'],
      recommendations_compact: [{ name: 'git-ops', category: 'code-quality', priority: 'high' }],
    });
    const meta2 = JSON.stringify({
      id: id2, pr_ref: 'pr-20', generated_at: '2026-03-15T00:00:00Z', schema_version: '1.0',
      total_signals: 1, high_impact_signals: 0, recommendation_count: 2,
      signal_types: ['loop'],
