...
export const signedExecutionRequestEnvelopeSchema = {
  ...
  type: 'object',
  additionalProperties: false,
  required: ['contractVersion', 'requestId', 'issuedAt', 'signature', 'payload'],
  properties: {
...
export const executionResultSchema = {
  ...
  type: 'object',
  additionalProperties: false,
  required: ['ok', 'stdout', 'stderr', 'exitCode', 'startedAt', 'finishedAt', 'durationMs'],
  properties: {
    ok: { type: 'boolean' },
    stdout: { type: 'string' },
    stderr: { type: 'string' },
...
export const validateToolInvocationPayload = ajv.compile(toolInvocationPayloadSchema);
export const validateSignedExecutionRequestEnvelope = ajv.compile(signedExecutionRequestEnvelopeSchema);
...
function validationError(prefix, validate) {
  ...
  return new Error(`${prefix}: ${details || 'unknown schema validation error'}`);
}

export function assertToolInvocationPayload(value) {
  if (!validateToolInvocationPayload(value)) {
    throw validationError('Invalid tool invocation payload', validateToolInvocationPayload);
  }
  return value;
}
...
export function assertSignedExecutionRequestEnvelope(value) {
  if (!validateSignedExecutionRequestEnvelope(value)) {
    throw validationError('Invalid execution request envelope', validateSignedExecutionRequestEnvelope);
  }
  return value;
}
...
export function makeErrorResponse({ code, message, details, requestId = null }) {
  return {
    ok: false,
