...
export function canonicalSigningInput({ method, path, timestamp, nonce, bodyHash }) {
  return [method.toUpperCase(), path, timestamp, nonce, bodyHash.toLowerCase()].join('\n');
}
...
export async function verifySignedRequest({
  ...
  if (!secret) {
    return structuredError(401, 'signing_secret_missing', 'Request signing secret is not configured');
  }
  ...
  if (!timestamp || !nonce || !bodyHash || !signatureHeader) {
    return structuredError(401, 'missing_signature_headers', 'Required request-signing headers are missing', {
  ...
  if (!Number.isFinite(parsedTs)) {
    return structuredError(401, 'invalid_timestamp', 'Timestamp must be epoch milliseconds');
  }

  if (Math.abs(nowMs - parsedTs) > clockSkewMs) {
    return structuredError(403, 'stale_timestamp', 'Timestamp is outside allowed clock-skew window', {
      clock_skew_ms: clockSkewMs,
    });
  }

  if (!/^[a-f0-9]{64}$/i.test(bodyHash)) {
    return structuredError(401, 'invalid_body_hash', 'Body hash must be a lowercase SHA-256 hex digest');
  }
