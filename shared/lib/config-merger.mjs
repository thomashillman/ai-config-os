...
const log = (msg) => { if (DEBUG) process.stderr.write(`[config-merger] ${msg}\n`); };
...
function loadYaml(filePath) {
  ...
  try {
    ...
    process.stderr.write(`[error] Could not read ${filePath}: ${err.message}\n`);
    process.exit(1);
  }
  let data;
  try {
    ...
    process.stderr.write(`[error] YAML parse error in ${filePath}: ${err.message}\n`);
    process.exit(1);
  }
  if (data !== null && typeof data !== 'object' || Array.isArray(data)) {
    process.stderr.write(`[error] ${filePath} must be a YAML mapping, not a scalar or list\n`);
    process.exit(1);
  }
  return data ?? {};
}
