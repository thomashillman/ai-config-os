...
Guarantees that Claude Code can discover and use skills reliably, even when the Worker is unavailable or manifests are stale.
...
4. **Capability probe** - Detect local platform capabilities and cache at `~/.ai-config-os/probe-report.json`
