...
if (!workerUrl) {
  console.error(
    "[publish] Error: AI_CONFIG_OS_WORKER_URL (or WORKER_URL) is required",
  );
  process.exit(1);
}
if (!token) {
  console.error(
    "[publish] Error: AI_CONFIG_OS_WORKER_TOKEN (or AUTH_TOKEN) is required",
  );
  process.exit(1);
}
...
for (const r of results) {
  ...
  console.error(
    `[publish] ${status.padEnd(4)} ${r.resource.padEnd(40)} ${detail}`,
  );
  if (!r.ok) failures++;
}
