...
const VALID_ROUTE_KINDS = ['repository_local', 'repository_remote', 'artifact_bundle', 'artifact_diff'];

const VALID_ARTIFACT_COMPLETENESS = ['repo_complete', 'repo_partial', 'artifact_complete', 'diff_only'];

const VALID_HISTORY_AVAILABILITY = ['repo_history', 'change_history', 'artifact_limited_history', 'no_history'];

const VALID_LOCALITY_CONFIDENCE = ['repo_local', 'repo_remote_bound', 'artifact_scoped', 'diff_scoped'];
...
const VALID_TASK_CLASSES = ['repository_review', 'patch_review', 'artifact_review'];
...
const VALID_LATENCY_RISK = ['interactive_safe', 'interactive_tolerable', 'background_biased'];
...
export function validateRouteProfiles(profiles) {
  ...
  profiles.forEach((profile, idx) => {
    ...
    if (!profile.identity.route_id || typeof profile.identity.route_id !== 'string') {
      throw new Error(`${context}.identity: 'route_id' must be a non-empty string`);
    }
    ...
    if (!Array.isArray(caps.allowed_task_classes) || caps.allowed_task_classes.length === 0) {
      throw new Error(`${context}.default_capabilities: 'allowed_task_classes' must be a non-empty array`);
    }
    ...
    if (limits.max_input_tokens !== undefined && typeof limits.max_input_tokens !== 'number') {
      throw new Error(`${context}.static_limits: 'max_input_tokens' must be a number`);
    }
    if (limits.max_output_tokens !== undefined && typeof limits.max_output_tokens !== 'number') {
      throw new Error(`${context}.static_limits: 'max_output_tokens' must be a number`);
    }
    if (limits.max_total_tokens !== undefined && typeof limits.max_total_tokens !== 'number') {
      throw new Error(`${context}.static_limits: 'max_total_tokens' must be a number`);
    }
    if (limits.max_latency_ms !== undefined && typeof limits.max_latency_ms !== 'number') {
      throw new Error(`${context}.static_limits: 'max_latency_ms' must be a number`);
    }
    ...
    if (profile.static_preferences && typeof profile.static_preferences === 'object') {
      if (profile.static_preferences.preferred_model_tier) {
        if (!VALID_MODEL_TIERS.includes(profile.static_preferences.preferred_model_tier)) {
          throw new Error(
    ...
    Object.keys(profile).forEach(key => {
      if (forbiddenFields.some(ff => key.toLowerCase().includes(ff))) {
        throw new Error(`${context}: forbidden dynamic field detected: '${key}'`);
      }
    });
  });
}
...
export function validateModelPathRegistry(models) {
  ...
  models.forEach((model, idx) => {
    ...
    if (!model.identity.provider || typeof model.identity.provider !== 'string') {
      throw new Error(`${context}.identity: 'provider' must be a non-empty string`);
    }
    if (!model.identity.model_id || typeof model.identity.model_id !== 'string') {
      throw new Error(`${context}.identity: 'model_id' must be a non-empty string`);
    }
    ...
    if (!Array.isArray(model.compatibility.supported_execution_modes)) {
      throw new Error(`${context}.compatibility: 'supported_execution_modes' must be an array`);
    }
    if (model.compatibility.supported_execution_modes.length === 0) {
      throw new Error(`${context}.compatibility: 'supported_execution_modes' must not be empty`);
    }
    ...
    // Check for forbidden dynamic fields
    const forbiddenFields = ['runtime_state', 'availability', 'live_condition', 'pricing'];
...
export function validateRouteProfileUniqueness(profiles) {
  ...
  profiles.forEach((p, idx) => {
    if (ids.has(p.identity.route_id)) {
      throw new Error(`Duplicate route_id '${p.identity.route_id}' at index ${idx}`);
    }
