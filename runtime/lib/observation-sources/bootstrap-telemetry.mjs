...
export async function loadBootstrapTelemetry({ home = process.env.HOME || process.env.USERPROFILE } = {}) {
  if (!home) return [];
  ...
  try {
    const files = readdirSync(logsDir).filter(f => f.startsWith('bootstrap-') && f.endsWith('.jsonl'));
