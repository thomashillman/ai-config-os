...
test('T005 alt: scoring is deterministic across explicit route permutations with tied scores', () => {
  ...
  const permutations = [
    [
      { id: 'r1', channel: 'script', equivalence: 'exact', requiredCapabilities: ['shell.exec'] },
      { id: 'r2', channel: 'script', equivalence: 'exact', requiredCapabilities: ['shell.exec'] },
      { id: 'r3', channel: 'script', equivalence: 'high', requiredCapabilities: ['shell.exec'] },
    ],
    [
      { id: 'r2', channel: 'script', equivalence: 'exact', requiredCapabilities: ['shell.exec'] },
      { id: 'r1', channel: 'script', equivalence: 'exact', requiredCapabilities: ['shell.exec'] },
  ...
  for (const permuted of permutations) {
    ...
    const exactIds = scored.filter((r) => r.equivalence === 'exact').map((r) => r.id);
...
test('T005 alt: resolver accepts dictionary objects with null prototype', () => {
  ...
  const contract = resolveEffectiveOutcomeContract({ toolName: 'null_proto_tool' });
