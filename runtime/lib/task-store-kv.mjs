...
} from "./portable-task-lifecycle-worker.mjs";
import {
  appendFindingToTask,
  transitionFindingsForRouteUpgrade,
} from "./findings-ledger-worker.mjs";
import {
  TaskConflictError,
  TaskNotFoundError,
  createReadinessView as toTaskReadinessView,
} from "./task-shared.mjs";
import {
  KvPersistence,
  normaliseSlug,
  generateShortCode,
} from "./kv-persistence.mjs";
...
export class KvTaskStore {
  ...
  _taskKey(taskId) {
    return this.kvp._taskKey(taskId);
  }
  _logKey(taskId) {
    return this.kvp._logKey(taskId);
  }
  _eventsKey(taskId) {
    return this.kvp._eventsKey(taskId);
  }
  _snapshotsKey(taskId) {
    return this.kvp._snapshotsKey(taskId);
  }
  _shortCodeKey(code) {
    return this.kvp._shortCodeKey(code);
  }
  _nameSlugKey(slug) {
    return this.kvp._nameSlugKey(slug);
  }
  _indexKey() {
    return this.kvp._indexKey();
  }
  async _get(key) {
    return this.kvp._get(key);
  }
  async _put(key, value) {
    return this.kvp._put(key, value);
  }
  async _append(key, item) {
    return this.kvp._append(key, item);
  }
  async _loadIndex() {
    return this.kvp._loadIndex();
  }
  async _flushIndex() {
    return this.kvp._flushIndex();
  }
  async _updateIndex(taskId, meta) {
    return this.kvp._updateIndex(taskId, meta);
  }
  ...
  async create(task) {
    ...
    if (existing) {
      throw new TaskConflictError(`Task already exists: ${validated.task_id}`, {
    ...
    const samePrefix = index.filter((t) =>
      (t.short_code || "").startsWith(prefix),
    ).length;
    ...
    await this._append(
      this._snapshotsKey(validated.task_id),
      createSnapshot(validated),
    );
    ...
    await this._updateIndex(validated.task_id, {
      ...
      stronger_route_available:
        validated.task_type === "review_repository" &&
        validated.current_route !== "local_repo",
  ...
  async loadByName(nameOrSlug) {
    ...
    const match = index.find((t) =>
      normaliseSlug(t.name || "").includes(slug.slice(0, 8)),
    );
  ...
  async update(taskId, { expectedVersion, changes }) {
    ...
    await this._updateIndex(taskId, {
      ...
      stronger_route_available:
        next.task_type === "review_repository" &&
        next.current_route !== "local_repo",
  ...
  async appendFinding(taskId, { expectedVersion, finding, updatedAt }) {
    ...
    await this._append(this._eventsKey(taskId), {
      ...
      created_at: updatedAt,
      metadata: {
        finding_id: lastFinding.finding_id,
        provenance_status: lastFinding.provenance.status,
      },
    });
    await this._append(this._logKey(taskId), {
      type: "finding_added",
      ...
      data: {
        finding_id: lastFinding.finding_id,
        status: lastFinding.provenance.status,
      },
    });
  ...
  async transitionFindingsForRouteUpgrade(
    ...
    const reclassifiedCount = transitionedFindings.reduce((count, nf, idx) => {
      ...
      return !pf ||
        pf.provenance.status !== nf.provenance.status ||
        pf.provenance.recorded_by_route !== nf.provenance.recorded_by_route
        ? count + 1
        : count;
    }, 0);
    await this._append(this._eventsKey(taskId), {
    ...
    await this._updateIndex(taskId, {
      ...
      stronger_route_available:
        next.task_type === "review_repository" && toRouteId !== "local_repo",
      updated_at: upgradedAt,
    });
    await this._flushIndex();
    return clone(next);
  ...
  async selectRoute(taskId, { routeId, expectedVersion, selectedAt }) {
    ...
    await this._updateIndex(taskId, {
      ...
      stronger_route_available:
        next.task_type === "review_repository" && routeId !== "local_repo",
  ...
  async createContinuationPackage(
    ...
    const existingEvent = events.find(
      (e) =>
        e.type === "continuation_created" &&
        e.metadata?.handoff_token_id === tokenId,
    );
    const canonicalCreatedAt =
      existingEvent?.metadata?.continuation_package_created_at ||
  ...
  async getSnapshot(taskId, version) {
    ...
    if (!snapshot)
      throw new TaskNotFoundError(
        `snapshot version ${version} for task ${taskId}`,
      );
  ...
  async listRecentTasks({ status, limit = 10, updatedWithinSeconds } = {}) {
    ...
    if (updatedWithinSeconds) {
      const cutoff = new Date(
        Date.now() - updatedWithinSeconds * 1000,
      ).toISOString();
