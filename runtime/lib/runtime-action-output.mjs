...
function parseManifestStatus(rawOutput) {
  ...
  const lastSynced =
    rawOutput.match(/^Last synced:\s*(.+)$/m)?.[1]?.trim() ?? null;
...
function parseValidateAllOutput(rawOutput) {
  ...
  const nonBlockingFailCount = (
    rawOutput.match(/✗ Fail \(non-blocking\)/g) || []
  ).length;
...
function parseContextCostOutput(rawOutput) {
  const totalTokens = Number(
    rawOutput.match(/Total tokens:\s*([0-9]+)/)?.[1] || 0,
  );
  const threshold = Number(rawOutput.match(/Threshold:\s*([0-9]+)/)?.[1] || 0);
  const overThreshold = Number(
    rawOutput.match(/Skills over threshold:\s*([0-9]+)/)?.[1] || 0,
  );

  const skillEntries = [];
  for (const line of rawOutput.split("\n")) {
    const match = line.match(
      /^([a-z0-9_-]+)\s+([0-9]+)\s+([0-9]+)\s+([0-9]+)%$/i,
    );
    if (match) {
      skillEntries.push({
        skill: match[1],
...
export function parseRuntimeActionOutput(
  actionName,
  rawOutput,
  { normalizedArgs = {} } = {},
) {
