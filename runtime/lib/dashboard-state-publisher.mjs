  ...
  try {
    const res = await fetch(url, {
      ...
      headers: {
        ...
        'X-Publisher-Surface': payload.meta?.publisher_surface ?? 'local_runtime',
      },
    ...
    return { resource, ok: false, status: 0, error: err instanceof Error ? err.message : String(err) };
  }
}

// ── Publisher functions ───────────────────────────────────────────────────────

export async function publishSkillsList({ workerUrl, token, scope, repoRoot = resolveRepoRoot() }) {
...
export async function publishToolingStatus({ workerUrl, token, scope, repoRoot = resolveRepoRoot() }) {
  const result = runScript(repoRoot, 'runtime/manifest.sh', ['status']);
  return postSnapshot(workerUrl, token, 'tooling.status', '/v1/tooling/status/publish', {
    data: { raw_output: result.output, success: result.success },
    meta: freshnessMeta(scope),
    summary: result.success ? 'tooling.status: manifest loaded' : 'tooling.status: script error',
  });
}

export async function publishConfigSummary({ workerUrl, token, scope, repoRoot = resolveRepoRoot() }) {
  const result = runScript(repoRoot, 'shared/lib/config-merger.sh', []);
  return postSnapshot(workerUrl, token, 'config.summary', '/v1/config/summary/publish', {
    data: { raw_output: result.output, success: result.success },
    meta: freshnessMeta(scope),
    summary: result.success ? 'config.summary: config loaded' : 'config.summary: script error',
  });
}

export async function publishContextCost({ workerUrl, token, scope, repoRoot = resolveRepoRoot(), threshold = 2000 }) {
  const result = runScript(repoRoot, 'ops/context-cost.sh', ['--threshold', String(threshold)]);
  return postSnapshot(workerUrl, token, 'runtime.context_cost', '/v1/runtime/context-cost/publish', {
    data: { raw_output: result.output, success: result.success, threshold },
    meta: freshnessMeta(scope),
    summary: result.success ? 'runtime.context_cost: computed' : 'runtime.context_cost: script error',
  });
}

export async function publishAuditValidateAll({ workerUrl, token, scope, repoRoot = resolveRepoRoot() }) {
  const result = runScript(repoRoot, 'ops/validate-all.sh', []);
  return postSnapshot(workerUrl, token, 'audit.validate_all', '/v1/audit/validate-all/publish', {
    data: { raw_output: result.output, success: result.success },
    meta: freshnessMeta(scope),
    summary: result.success ? 'audit.validate_all: passed' : 'audit.validate_all: failures detected',
  });
}

export async function publishAnalyticsToolUsage({ workerUrl, token, scope, repoRoot = resolveRepoRoot() }) {
  let contractData;
  try {
  ...
  return postSnapshot(workerUrl, token, 'analytics.tool_usage', '/v1/analytics/tool-usage/publish', {
    data: contractData,
    meta: {
      ...freshnessMeta(scope),
      interpretation: contractData.interpretation,
...
export async function publishAnalyticsSkillEffectiveness({ workerUrl, token, scope, repoRoot = resolveRepoRoot() }) {
  let contractData;
  try {
    ...
    contractData = buildSkillEffectivenessContract(skills, outcomeEvents.length);
  } catch {
    contractData = buildSkillEffectivenessContract([], 0);
  }
  return postSnapshot(workerUrl, token, 'analytics.skill_effectiveness', '/v1/analytics/skill-effectiveness/publish', {
    data: contractData,
    meta: {
      ...freshnessMeta(scope),
      interpretation: contractData.interpretation,
...
export async function publishAnalyticsAutoresearchRuns({ workerUrl, token, scope, repoRoot = resolveRepoRoot() }) {
  const runs = readAutoresearchRuns(repoRoot);
  const contractData = buildAutoresearchRunsContract(runs);
  return postSnapshot(workerUrl, token, 'analytics.autoresearch_runs', '/v1/analytics/autoresearch-runs/publish', {
    data: contractData,
    meta: {
      ...freshnessMeta(scope),
      interpretation: contractData.interpretation,
...
export async function publishAnalyticsFrictionSignals({ workerUrl, token, scope }) {
  ...
  return postSnapshot(workerUrl, token, 'analytics.friction_signals', '/v1/analytics/friction-signals/publish', {
    data: contractData,
    meta: {
      ...freshnessMeta(scope),
      interpretation: contractData.interpretation,
...
export async function publishAll({ workerUrl, token, scope, repoRoot = resolveRepoRoot() }) {
  ...
  const publishers = [
    ...
    { name: 'analytics.skill_effectiveness', fn: publishAnalyticsSkillEffectiveness },
    { name: 'analytics.friction_signals', fn: publishAnalyticsFrictionSignals },
    { name: 'analytics.autoresearch_runs', fn: publishAnalyticsAutoresearchRuns },
  ];
  ...
  for (const { name, fn } of publishers) {
    try {
      ...
      results.push({ resource: name, ok: false, status: 0, error: err instanceof Error ? err.message : String(err) });
    }
  }
