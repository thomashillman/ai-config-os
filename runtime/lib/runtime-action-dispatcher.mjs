import {
  getRuntimeActionMeta,
  isScriptWrapperAction,
} from "./runtime-action-matrix.mjs";
...
export function createRuntimeActionDispatcher({ runScript, validateNumber }) {
  ...
  const normalizeNumber =
    typeof validateNumber === "function"
      ? validateNumber
    ...
    dispatch(actionName, actionArgs = {}) {
      ...
      if (!isScriptWrapperAction(actionName)) {
        throw new ActionValidationError(
          `Action '${actionName}' is not a script-wrapper action`,
        );
      }
      ...
      switch (actionName) {
        ...
        case "context_cost": {
          ...
          try {
            ...
            throw new ActionValidationError(
              error instanceof Error ? error.message : "Invalid threshold",
            );
          }
      ...
      const parsed = parseRuntimeActionOutput(actionName, result.output, {
        normalizedArgs,
      });
      return {
        ...result,
