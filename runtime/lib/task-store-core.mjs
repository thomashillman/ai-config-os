...
function createReadinessView(task, progressEvents = []) {
  const totalSteps = task.progress?.total_steps ?? 0;
  const completedSteps = task.progress?.completed_steps ?? 0;

  const findingsProvenance = (Array.isArray(task.findings) ? task.findings : []).reduce((summary, finding) => {
  ...
  return {
    ...
    findings_provenance: findingsProvenance,
