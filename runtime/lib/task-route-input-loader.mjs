...
const DEFAULT_INPUT_DEFINITIONS_PATH = path.resolve(
  process.cwd(),
...
function validateRequiredInputs(taskType, routeId, requiredInputs) {
  if (!Array.isArray(requiredInputs) || requiredInputs.length === 0) {
    throw new Error(
      `Task route input definitions for '${taskType}.${routeId}' require non-empty required_inputs array`,
    );
  }
  ...
  for (const input of requiredInputs) {
    if (typeof input !== "string" || input.trim().length === 0) {
      throw new Error(
        `Task route input definitions for '${taskType}.${routeId}' contain an invalid required input`,
      );
    }
    if (seen.has(input)) {
      throw new Error(
        `Task route input definitions for '${taskType}.${routeId}' contain duplicate required input '${input}'`,
      );
    }
    seen.add(input);
  }
...
export function loadTaskRouteInputDefinitions(
  filePath = DEFAULT_INPUT_DEFINITIONS_PATH,
) {
  ...
  for (const [taskType, taskDefinition] of Object.entries(taskTypes)) {
    ...
    if (!taskDefinitionObject) {
      throw new Error(
        `Task route input definitions for '${taskType}' must be a plain object`,
      );
    }
    ...
    if (!routes) {
      throw new Error(
        `Task route input definitions for '${taskType}' missing routes object`,
      );
    }
    ...
    for (const [routeId, routeDefinition] of Object.entries(routes)) {
      ...
      if (!routeDefinitionObject) {
        throw new Error(
          `Task route input definitions for '${taskType}.${routeId}' must be a plain object`,
        );
      }
      validateRequiredInputs(
        taskType,
        routeId,
        routeDefinitionObject.required_inputs,
      );
    }
  }
