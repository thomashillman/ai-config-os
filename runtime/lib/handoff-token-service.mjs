...
function toEpochMs(value, fieldName) {
  ...
  if (!Number.isFinite(parsed)) {
    throw new Error(
      `Invalid ${fieldName}: expected ISO date-time string or epoch milliseconds`,
    );
  }
  return parsed;
}
...
function signaturesMatch(actualHex, expectedHex) {
  if (
    !SIGNATURE_HEX_PATTERN.test(actualHex) ||
    !SIGNATURE_HEX_PATTERN.test(expectedHex)
  ) {
    return false;
  }
...
export function createHandoffTokenService({
  ...
  function issueToken({ taskId, ttlSeconds, now: nowInput } = {}) {
    ...
    return validateContract("handoffToken", {
      ...unsignedToken,
      signature,
    });
  }
  ...
  function verifyToken({ token, expectedTaskId, now: nowInput } = {}) {
    ...
    const validatedToken = validateContract("handoffToken", token);
    if (validatedToken.task_id !== expectedTaskId) {
      throw new Error(
        `Token task id mismatch: expected '${expectedTaskId}', got '${validatedToken.task_id}'`,
      );
    }
    ...
    if (
      typeof replayStore?.isConsumed === "function" &&
      replayStore.isConsumed({
