...
export function getEnv() {
  ...
  return {
    ...
    signaturePublicKey:
      process.env.REMOTE_EXECUTOR_SIGNATURE_PUBLIC_KEY_PEM ?? "",
...
function validateContract(body) {
  ...
  if (
    body.args !== undefined &&
    (!Array.isArray(body.args) || body.args.some((v) => typeof v !== "string"))
  ) {
    throw new ExecutorHttpError(
      400,
      "BAD_REQUEST",
  ...
  if (
    ...
    (!Number.isInteger(body.timeout_ms) || body.timeout_ms <= 0)
  ) {
    throw new ExecutorHttpError(
      400,
      "BAD_REQUEST",
...
function resolveScript(tool) {
  ...
  if (!scriptPath.startsWith(REPO_ROOT + path.sep)) {
    throw new ExecutorHttpError(
      500,
      "EXECUTOR_ERROR",
      "Resolved script path is outside repository root",
    );
  }
  return scriptPath;
}
...
async function executeTool(body, timeoutMs) {
  ...
  const parsed = parseRuntimeActionOutput(body.tool, rawOutput, {
    normalizedArgs: {},
  });
  ...
  return {
    ...
    result: {
      ...
      capability: {
        local_only: parsed.capability.local_only,
        worker_backed: true,
      },
...
export function createRemoteExecutorHandler({
  ...
  return async function handler(req, res) {
    ...
    if (proxySecret !== env.sharedSecret) {
      return json(res, 401, {
        ok: false,
        error: { code: "UNAUTHORIZED", message: "Invalid proxy secret" },
      });
    }
    ...
    try {
      ...
      const signature =
        typeof req.headers["x-request-signature"] === "string"
