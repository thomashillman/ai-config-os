...
export const TOOL_REGISTRY = {
  sync_tools: {
    ...
    run: ({ runScript }, args) =>
      runScript("runtime/sync.sh", args.dry_run ? ["--dry-run"] : []),
  },
  list_tools: {
    description:
  ...
  context_cost: {
    ...
    run: ({ runScript }, args) => {
      ...
      return runScript("ops/context-cost.sh", [
        "--threshold",
        String(threshold),
      ]);
    },
  ...
  mcp_list: {
    ...
    schema: EMPTY_OBJECT_SCHEMA,
    run: ({ runScript }) =>
      runScript("runtime/adapters/mcp-adapter.sh", ["list"]),
  },
  mcp_add: {
    ...
    run: ({ runScript }, args) =>
      runScript("runtime/adapters/mcp-adapter.sh", [
        "add",
        args.name,
        args.command,
        ...(args.args ?? []),
  ...
  mcp_remove: {
    ...
    run: ({ runScript }, args) =>
      runScript("runtime/adapters/mcp-adapter.sh", ["remove", args.name]),
  },
  task_start_review_repository: {
    ...
    run: ({ taskService }, args) =>
      taskService.startReviewRepositoryTask({
        taskId: args.task_id,
        goal: args.goal,
  ...
  task_resume_review_repository: {
    ...
    run: ({ taskService }, args) =>
      taskService.resumeReviewRepositoryTask({
        taskId: args.task_id,
        capabilityProfile: args.capability_profile,
      }),
  },
