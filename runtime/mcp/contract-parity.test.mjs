...
test("MCP and dashboard contract-resolution surfaces agree on route identity", async () => {
  ...
  assert.equal(
    mcpContract.preferredRoute.id,
    jsonPayload.data.effectiveOutcomeContract.preferredRoute.id,
  );
  assert.deepEqual(
    mcpContract.preferredRoute.args,
    jsonPayload.data.effectiveOutcomeContract.preferredRoute.args,
  );
});
...
test("MCP and dashboard script-wrapper actions resolve to equivalent command mappings", async () => {
  ...
  const fixtures = [
    ...
    },
    {
      tool: "context_cost",
      route: "GET",
      path: "/api/context-cost",
      mcpArgs: { threshold: 3000 },
      req: { query: { threshold: 3000 } },
    },
    {
      tool: "validate_all",
      route: "GET",
  ...
  for (const fixture of fixtures) {
    await handler({
      params: { name: fixture.tool, arguments: fixture.mcpArgs },
    });
